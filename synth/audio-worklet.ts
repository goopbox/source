...
class SynthProcessor extends AudioWorkletProcessor {
  ...
  private setLiveInput(state: LiveInputState): void {
    ...
    const acceptsNotes: boolean =
      ...
      pitches: readonly number[] = acceptsNotes ? state.pitches : [],
      instruments: readonly number[] = acceptsNotes ? state.instruments : [];
  ...
  private setLiveInputInstruments(instruments: readonly number[]): void {
    if (this.synth.song?.getChannelIsAutomation(this.synth.liveInputChannel)) {
      instruments = [];
    }
    this.synth.liveInputInstruments.length = instruments.length;
    for (let i = 0; i < instruments.length; i++) {
  ...
  private setSong(serializedSong: Uint8Array, mutedChannels: readonly boolean[]): void {
    this.synth.setSong(serializedSong);
    const { song } = this.synth;
    if (song == null) {
      return;
    }
  ...
  private snapshot(): TransportSnapshot {
    return {
      ...
      tempo: this.synth.song?.automationChannelCount
  ...
  public process(_inputs: Float32Array[][], outputs: Float32Array[][]): boolean {
    ...
    const output: Float32Array[] | undefined = outputs[0]!,
      left: Float32Array | undefined = output?.[0],
      right: Float32Array | undefined = output?.[1];
    if (left == null || right == null) {
      return true;
    }

    const wasPlaying: boolean = this.synth.playing;
    if (this.initialized && (wasPlaying || this.renderedFrames < this.liveInputDeadline)) {
