...
import { decodeBinaryValue, encodeBinaryValue } from "./binary-codec.js";

const magic: readonly number[] = [0x53, 0x4c, 0x44, 0x47], // SLDG
  currentVersion = 1,
  headerLength = 5,
  checksumLength = 4,
  maximumBodyLength: number = 64 * 1024 * 1024,
  maximumCompressedBodyLength: number = maximumBodyLength + 64 * 1024,
  compressionDictionaryText: string = [
    "asset:https://",
  ...
  compressionDictionary: Uint8Array = Uint8Array.from(
...
const crcTable: Uint32Array = (() => {
  ...
  for (let index = 0; index < table.length; index++) {
    ...
    for (let bit = 0; bit < 8; bit++) {
      value = (value & 1) === 0 ? value >>> 1 : 0xed_b8_83_20 ^ (value >>> 1);
    }
    table[index] = value >>> 0;
  }
  return table;
...
function inflateBody(compressed: Uint8Array): Uint8Array {
  ...
  const inflater: Inflate = new Inflate(
    ...
    inputChunkLength: number = 4 * 1024;
  for (let offset = 0; offset < compressed.length; offset += inputChunkLength) {
...
export function encodeSongBinary(value: unknown): Uint8Array {
  ...
  const result: Uint8Array = new Uint8Array(headerLength + compressed.length + checksumLength);
  result.set(magic, 0);
...
export function extractCompressedSongBody(bytes: Uint8Array): Uint8Array | null {
  ...
  const checksumOffset: number = bytes.length - checksumLength,
    view: DataView = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
  if (view.getUint32(checksumOffset) !== crc32(bytes.subarray(0, checksumOffset))) {
...
export function decodeSongBinary(bytes: Uint8Array): unknown {
  ...
  const checksumOffset: number = bytes.length - checksumLength,
    view: DataView = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength),
    expectedChecksum: number = view.getUint32(checksumOffset),
    actualChecksum: number = crc32(bytes.subarray(0, checksumOffset));
  if (actualChecksum !== expectedChecksum) {
    throw new Error("Corrupt .goop file checksum.");
  }
  return decodeBinaryValue(inflateBody(bytes.subarray(headerLength, checksumOffset)));
}
