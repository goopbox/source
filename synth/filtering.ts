...
The original editor used simpler low-pass filters that I adapted from SFXR:
...
export class FilterCoefficients {
  ...
  public highShelf1stOrder(cornerRadiansPerSample: number, shelfLinearGain: number): void {
    ...
    const tan: number = Math.tan(cornerRadiansPerSample * 0.5),
      sqrtGain: number = Math.sqrt(shelfLinearGain),
      g: number = (tan * sqrtGain - 1) / (tan * sqrtGain + 1.0),
      a0 = 1.0;
    this.a[1] = g / a0;
  ...
  public lowPass2ndOrderButterworth(cornerRadiansPerSample: number, peakLinearGain: number): void {
    ...
    const alpha: number = Math.sin(cornerRadiansPerSample) / (2.0 * peakLinearGain),
      cos: number = Math.cos(cornerRadiansPerSample),
      a0: number = 1.0 + alpha;
    this.a[1] = (-2.0 * cos) / a0;
    this.a[2] = (1 - alpha) / a0;
  ...
  public lowPass2ndOrderSimplified(cornerRadiansPerSample: number, peakLinearGain: number): void {
    ...
    const g: number = 2.0 * Math.sin(cornerRadiansPerSample / 2.0),
      filterResonance: number = 1.0 - 1.0 / (2.0 * peakLinearGain),
      feedback: number = filterResonance + filterResonance / (1.0 - g);
  ...
  public highShelf2ndOrder(
    ...
    const A: number = Math.sqrt(shelfLinearGain),
      c: number = Math.cos(cornerRadiansPerSample),
      Aplus: number = A + 1.0,
      Aminus: number = A - 1.0,
      alpha: number =
        Math.sin(cornerRadiansPerSample) * 0.5 * Math.sqrt((Aplus / A) * (1.0 / slope - 1.0) + 2.0),
      sqrtA2Alpha: number = 2.0 * Math.sqrt(A) * alpha,
      a0: number = Aplus - Aminus * c + sqrtA2Alpha;
  ...
  public peak2ndOrder(
    ...
    const sqrtGain: number = Math.sqrt(peakLinearGain),
      ...
      alpha: number = Math.tan(bandWidth * 0.5),
      a0: number = 1.0 + alpha / sqrtGain;
  ...
  /*
	...
	public scaledDifference(other: FilterCoefficients, scale: number): void {
		if (other.order !== this.order) {throw new Error("Filter orders must match.");}
		for (let i: number = 0; i <= this.order; i++) {
...
export class FrequencyResponse {
  ...
  public analyzeComplex(filter: FilterCoefficients, real: number, imag: number): void {
    ...
    let realNum: number = b[0]! + b[1]! * realZ1,
      imagNum: number = b[1]! * imagZ1,
      realDenom: number = 1.0 + a[1]! * realZ1,
      imagDenom: number = a[1]! * imagZ1,
      realZ: number = realZ1,
      imagZ: number = imagZ1;
    for (let i = 2; i <= filter.order; i++) {
      const realTemp: number = realZ * realZ1 - imagZ * imagZ1,
        imagTemp: number = realZ * imagZ1 + imagZ * realZ1;
      realZ = realTemp;
      imagZ = imagTemp;
      realNum += b[i]! * realZ;
      imagNum += b[i]! * imagZ;
...
export class DynamicBiquadFilter {
  ...
  public loadCoefficientsWithGradient(
    ...
    if (start.order !== 2 || end.order !== 2) {
      throw new Error("Filter order must be 2.");
    }
    this.a1 = start.a[1]!;
    this.a2 = start.a[2]!;
    this.b0 = start.b[0]!;
