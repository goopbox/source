...
import { TimeStretch, advanceChipWavePhase, interpolateChipWaveSample } from "./time-stretch.js";
import { decodeSongBinary, encodeSongBinary } from "./song-binary.js";
import {
...
const epsilon = 1.0e-24, // For detecting and avoiding float denormals, which have poor performance.
  maximumSongAssetCount = 64,
  maximumAssetSourceLength = 8192,
  maximumSoundFontIdLength: number = "asset:".length + maximumAssetSourceLength,
  compactSongValueVersion = 2,
  legacyCompactSongValueVersion = 1,
  compactInstrumentLowFieldCount = 31,
  compactInstrumentFields: readonly string[] = [
    "chipWave",
    "chipNoise",
    "fadeIn",
    "fadeOut",
  ...
  compactInstrumentFieldSet: ReadonlySet<string> = new Set(compactInstrumentFields);
...
class CompactBitWriter {
  ...
  public write(bitCount: number, value: number): void {
    ...
    for (let bit: number = bitCount - 1; bit >= 0; bit--) {
      this.#bits[this.#length++] = Math.floor(value / 2 ** bit) % 2;
    }
  }
  ...
  public writeLongTail(minValue: number, minBits: number, value: number): void {
    ...
    this.#bits[this.#length++] = 0;
    while (bitCount > 0) {
      bitCount--;
      this.#bits[this.#length++] = Math.floor(value / 2 ** bitCount) % 2;
    }
  }

  public writePartDuration(value: number, divisor: number, minBits: number): void {
    if (divisor > 1) {
      if (value % divisor === 0) {
        this.write(1, 0);
  ...
  public writePitchToken(recentPitchIndex: number): void {
    if (!Number.isInteger(recentPitchIndex) || recentPitchIndex < -1 || recentPitchIndex >= 8) {
  ...
  public writeNoteSize(value: number, commonSize: number | null, useHuffman: boolean): void {
    ...
    const buffer: ArrayBuffer = new ArrayBuffer(8),
      view: DataView = new DataView(buffer);
  ...
  public concat(other: CompactBitWriter): void {
    for (let index = 0; index < other.#length; index++) {
      this.#bits[this.#length++] = other.#bits[index]!;
    }
  }
  ...
  public key(): string {
    ...
    for (let index = 0; index < this.#length; index++) {
      result += this.#bits[index] === 0 ? "0" : "1";
    }
    return result;
  }

  public finish(): Uint8Array {
    const result: Uint8Array = new Uint8Array(Math.ceil(this.#length / 8));
    for (let index = 0; index < this.#length; index++) {
...
class CompactBitReader {
  ...
  #readBit(): number {
    if (this.#index >= this.#bytes.length * 8) {
      throw new Error("Truncated compact .goop bit data.");
    }
    const result: number = (this.#bytes[this.#index >> 3]! >> (7 - (this.#index & 7))) & 1;
    this.#index++;
    return result;
  }
  ...
  public readPitchInterval(minBits: number): number {
    const negative: boolean = this.read(1) !== 0,
      magnitude: number = this.readLongTail(1, minBits);
  ...
  public readShapeToken(): number {
    ...
    if (shapeIndex >= 10) {
      throw new Error("Invalid compact .goop shape token.");
    }
    return shapeIndex;
  }
  ...
  public readNoteSize(commonSize: number | null, useHuffman: boolean): number {
    if (useHuffman) {
      ...
      if (pair === 2) {
        return this.read(1) === 0 ? 9 : 6;
      }
      if (this.read(1) === 0) {
        return this.read(1) === 0 ? 5 : 10;
      }
    } else if (commonSize != null && this.read(1) === 0) {
      return commonSize;
    }
    const encoded: number = this.read(4);
    if (encoded <= Config.noteSizeMax) {
      return encoded;
    }
    if (encoded !== 15) {
  ...
  public assertCanonicalPadding(): void {
    const remaining: number = this.#bytes.length * 8 - this.#index;
    if (remaining >= 8) {
      throw new Error("Invalid compact .goop bit length.");
    }
    while (this.#index < this.#bytes.length * 8) {
      if (this.#readBit() !== 0) {
...
function interpolateSetting(values: readonly number[], setting: number): number {
  const lowerIndex: number = Math.max(0, Math.min(values.length - 2, Math.floor(setting)));
  return (
    values[lowerIndex]! + (values[lowerIndex + 1]! - values[lowerIndex]!) * (setting - lowerIndex)
  );
}
...
export function sanitizeAutomationValue(value: number, domain: AutomationValueDomain): number {
  if (!Number.isFinite(value)) {
    value = value > 0 ? domain.max : domain.min;
  }
...
export function mapAutomationValueBetweenDomains(
  ...
  const sourceSpan: number = source.max - source.min,
    normalized: number = sourceSpan <= 0 ? 0 : (value - source.min) / sourceSpan;
  return sanitizeAutomationValue(
    destination.min + Math.max(0, Math.min(1, normalized)) * (destination.max - destination.min),
    destination,
  );
}
...
export function mapAutomationClipboardValue(
  ...
  if (source == null) {
    return sanitizeAutomationValue(0, destination);
  }
...
export class Event {
  ...
  public constructor(
    ...
    this.#points = points.map((point: EventPoint): EventPoint => point.clone());
  }
  ...
  public cloneWith(start: number, end: number, points: readonly EventPoint[]): this {
  ...
  public getFinalValue(): number {
    return this.points.at(-1)?.value ?? 0;
  }
    ...
    for (let index = 1; index < this.points.length; index++) {
      ...
      if (eventTime <= next.time) {
        const previous: EventPoint = this.points[index - 1]!,
          duration: number = next.time - previous.time;
        if (duration <= 0) {
          return next.value;
        }
  ...
  public getPointAt(part: number): EventPoint {
    const eventTime: number = Math.max(0, Math.min(this.end - this.start, part - this.start));
    if (this.points.length === 0) {
      return new EventPoint(eventTime, 0);
    }
    if (eventTime <= this.points[0]!.time) {
    ...
    for (let index = 1; index < this.points.length; index++) {
      ...
      const previous: EventPoint = this.points[index - 1]!,
        duration: number = next.time - previous.time,
        ratio: number = duration <= 0 ? 1 : (eventTime - previous.time) / duration;
      return new EventPoint(eventTime, previous.value + (next.value - previous.value) * ratio);
    }
...
export class AutomationRow {
  ...
  public targetChannel = -1;
  public targetChannelKind: ChannelKind = ChannelKind.pitch;
  public targetInstrument = -1;
  public targetId = "tempo";
  public targetIndex = 0;
  public targetChannelMissing = false;
  public targetInstrumentMissing = false;
  public targetElementMissing = false;
  ...
  public setSongTarget(targetId = "tempo", targetIndex = 0): void {
  ...
  public getTarget(): ModulationTarget | null {
    return Config.modulationTargets.dictionary[this.targetId] ?? null;
  }

  public getValueDomain(): AutomationValueDomain | null {
    const target: ModulationTarget | null = this.getTarget();
    return target == null ? null : Config.getAutomationValueDomain(target);
  }

  public isTargetValid(song: Song): boolean {
    const target: ModulationTarget | null = this.getTarget();
    if (target == null || target.supportsAutomation !== true || this.targetElementMissing) {
      return false;
    }
    ...
    if (this.targetInstrument < 0 || this.targetInstrument >= channel.instruments.length) {
      return false;
    }
...
export class Note extends Event {
  ...
  public constructor(pitch: number, start: number, end: number, size: number, fadeout = false) {
    super(start, end, [makeNotePin(0, 0, size), makeNotePin(0, end - start, fadeout ? 0 : size)]);
    this.pitches = [pitch];
  ...
  public pickMainInterval(): number {
    ...
    for (let pinIndex = 1; pinIndex < this.pins.length; pinIndex++) {
      const pinA: NotePin = this.pins[pinIndex - 1]!,
        pinB: NotePin = this.pins[pinIndex]!;
  ...
  public override cloneWith(start: number, end: number, points: readonly EventPoint[]): this {
    const note: this = super.cloneWith(start, end, points),
      firstInterval: number = note.pins[0]?.interval ?? 0;
    if (firstInterval !== 0) {
      for (let index = 0; index < note.pitches.length; index++) {
  ...
  public override getPointAt(part: number): NotePin {
    ...
    for (let index = 1; index < this.pins.length; index++) {
      ...
      const previous: NotePin = this.pins[index - 1]!,
        ...
        ratio: number = duration <= 0 ? 1 : (eventTime - previous.time) / duration;
      return new NotePin(
        Math.round(previous.interval + (next.interval - previous.interval) * ratio),
        eventTime,
        Math.round(previous.size + (next.size - previous.size) * ratio),
      );
    }
    const last: NotePin = this.pins.at(-1)!;
...
export class Pattern {
  ...
  public cloneAutomationEvents(): Event[][] {
    return this.automationEvents.map((events: Event[]): Event[] =>
  ...
  public hasContent(kind?: ChannelKind): boolean {
    if (kind === ChannelKind.automation) {
      return this.automationEvents.some((events: Event[]): boolean => events.length > 0);
    }
  ...
  public contentEquals(pattern: Pattern, kind: ChannelKind): boolean {
    if (kind !== ChannelKind.automation) {
      ...
      for (let noteIndex = 0; noteIndex < this.notes.length; noteIndex++) {
        ...
        if (
          ...
          left.pitches.some(
            (pitch: number, index: number): boolean => pitch !== right.pitches[index]!,
          ) ||
          left.pins.some((pin: NotePin, index: number): boolean => {
            const other: NotePin = right.pins[index]!;
            return (
              pin.interval !== other.interval || pin.time !== other.time || pin.size !== other.size
            );
          })
        ) {
          return false;
    ...
    for (let rowIndex = 0; rowIndex < this.automationEvents.length; rowIndex++) {
      const leftEvents: Event[] = this.automationEvents[rowIndex]!,
        rightEvents: Event[] = pattern.automationEvents[rowIndex]!;
      if (leftEvents.length !== rightEvents.length) {
        return false;
      }
      for (let eventIndex = 0; eventIndex < leftEvents.length; eventIndex++) {
        const left: Event = leftEvents[eventIndex]!,
          right: Event = rightEvents[eventIndex]!;
        if (
          left.start !== right.start ||
          left.end !== right.end ||
  ...
  public toAutomationBinaryObject(): object {
    return {
      rows: this.automationEvents.map((events: Event[]): object[] =>
  ...
  public fromBinaryObject(patternObject: DynamicValue, song: Song, isNoiseChannel: boolean): void {
    if (
      patternObject == null ||
    ...
    const maximumPitch: number = isNoiseChannel ? Config.drumCount - 1 : Config.maxPitch;
    let previousEnd = 0;
        ...
        if (
          ...
          pitches.some(
            (pitch: number): boolean =>
              pitch + pinObject.interval < 0 || pitch + pinObject.interval > maximumPitch,
          )
        ) {
          throw new Error("Invalid .goop note pin interval.");
        }
        pins.push(makeNotePin(pinObject.interval, pinObject.time, pinObject.size));
        previousTime = pinObject.time;
      }

      const end: number = start + pins.at(-1)!.time;
  ...
  public fromAutomationBinaryObject(
    ...
    for (let rowIndex = 0; rowIndex < rows.length; rowIndex++) {
      ...
      if (!Array.isArray(eventObjects) || eventObjects.length > Config.automationEventsPerRowMax) {
      ...
      for (const eventObject of eventObjects) {
        ...
        for (const pointObject of eventObject.points) {
          if (
            ...
            (domain != null && (pointObject.value < domain.min || pointObject.value > domain.max))
          ) {
...
export class ChipWaveSettings {
  ...
  public fromSettingsObject(settingsObject: DynamicValue): void {
    const pitch = Number(settingsObject?.["pitchPercent"]),
      tempo = Number(settingsObject?.["tempoPercent"]);
    this.pitch = Number.isFinite(pitch) ? pitch : 100;
    this.tempo = Number.isFinite(tempo) ? tempo : 100;
    const offset = Number(settingsObject?.["sampleOffset"]),
      loopStart = Number(settingsObject?.["sampleLoopStart"]),
      loopEnd = Number(settingsObject?.["sampleLoopEnd"]);
    this.offset = Number.isFinite(offset) ? Math.max(0, Math.min(1, offset)) : 0;
    this.loopStart = Number.isFinite(loopStart) ? Math.max(0, Math.min(1, loopStart)) : this.offset;
    this.loopEnd = Number.isFinite(loopEnd) ? Math.max(this.loopStart, Math.min(1, loopEnd)) : 1;
...
// oxlint-disable no-use-before-define -- The synth model and Synth static utilities require each other.
export class SpectrumWave {
  public spectrum: number[] = [];
  public hash = -1;

  public constructor(isNoiseChannel: boolean) {
...
class SpectrumWaveState {
  ...
  public getCustomWave(settings: SpectrumWave, lowestOctave: number): Float32Array {
    ...
    const highestOctave = 14,
      ...
      pitchTweak: number[] = [0, 1 / 7, Math.log2(5 / 4), 3 / 7, Math.log2(3 / 2), 5 / 7, 6 / 7];
    function controlPointToOctave(point: number): number {
      return (
        lowestOctave +
        Math.floor(point / Config.spectrumControlPointsPerOctave) +
        pitchTweak[
          (point + Config.spectrumControlPointsPerOctave) % Config.spectrumControlPointsPerOctave
        ]!
      );
    }
      ...
      const value1: number = i <= 0 ? 0 : settings.spectrum[i - 1]!,
        value2: number =
          i >= Config.spectrumControlPoints
            ...
            : settings.spectrum[i]!,
        octave1: number = controlPointToOctave(i - 1);
    ...
    if (settings.spectrum[Config.spectrumControlPoints - 1]! > 0) {
      combinedAmplitude +=
        ...
        drawNoiseSpectrum(
          ...
          waveLength,
          highestOctave +
            (controlPointToOctave(Config.spectrumControlPoints) - highestOctave) * falloffRatio,
          highestOctave,
...
class HarmonicsWaveState {
  public wave: Float32Array | null = null;
  #hash = -1;
  #generatedForType!: InstrumentType;

  public getCustomWave(settings: HarmonicsWave, instrumentType: InstrumentType): Float32Array {
    if (this.#hash === settings.hash && this.#generatedForType === instrumentType) {
      return this.wave!;
    }
    this.#hash = settings.hash;
    this.#generatedForType = instrumentType;
    ...
    const harmonicsRendered: number =
      ...
      waveLength: number = Config.harmonicsWavelength,
...
export class FilterControlPoint {
  ...
  public getLinearGain(peakMult = 1.0): number {
    const power: number = (this.gain - Config.filterGainCenter) * Config.filterGainStep,
      neutral: number = this.type === FilterType.peak ? 0.0 : -0.5,
      interpolatedPower: number = neutral + (power - neutral) * peakMult;
  ...
  public toCoefficients(
    ...
    const cornerRadiansPerSample: number =
        ...
        sampleRate,
      linearGain: number = this.getLinearGain(peakMult);
    switch (this.type) {
  ...
  public getVolumeCompensationMult(): number {
    const octave: number = (this.freq - Config.filterFreqReferenceSetting) * Config.filterFreqStep,
      gainPow: number = (this.gain - Config.filterGainCenter) * Config.filterGainStep;
    switch (this.type) {
      case FilterType.lowPass: {
        const freqRelativeTo8khz: number = (2.0 ** octave * Config.filterFreqReferenceHz) / 8000.0,
          ...
          warpedFreq: number = (Math.sqrt(1.0 + 4.0 * freqRelativeTo8khz) - 1.0) / 2.0,
          warpedOctave: number = Math.log2(warpedFreq);
        return (
          0.5 **
          (0.2 * Math.max(0.0, gainPow + 1.0) +
            Math.min(
              0.0,
              Math.max(-3.0, 0.595 * warpedOctave + 0.35 * Math.min(0.0, gainPow + 1.0)),
            ))
        );
      }
      case FilterType.highPass: {
      ...
      case FilterType.peak: {
        const distanceFromCenter: number =
          ...
          freqLoudness: number = (1.0 / (1.0 + (distanceFromCenter / 3.0) ** 2.0)) ** 2.0;
        return (
          0.5 ** (0.125 * Math.max(0.0, gainPow) + 0.1 * freqLoudness * Math.min(0.0, gainPow))
        );
      }
      default: {
        throw new Error("Unsupported filter gain.");
      }
    }
  }
}
...
export class FilterSettings {
  ...
  public addPoint(type: FilterType, freqSetting: number, gainSetting: number): void {
  ...
  public toSettingsObject(): object {
    const filterArray: DynamicValue[] = [];
    for (let i = 0; i < this.controlPointCount; i++) {
      const point: FilterControlPoint = this.controlPoints[i]!;
      filterArray.push({
        type: Config.filterTypeNames[point.type]!,
        cutoffHz: Math.round(point.getHz() * 100) / 100,
        linearGain: Math.round(point.getLinearGain() * 10_000) / 10_000,
      });
    }
    return filterArray;
  }

  public fromSettingsObject(filterObject: DynamicValue): void {
    this.controlPoints.length = 0;
    if (filterObject) {
      for (const pointObject of filterObject) {
        const point: FilterControlPoint = new FilterControlPoint(),
          typeIndex: number = Config.filterTypeNames.indexOf(pointObject["type"]);
        point.type = typeIndex;
        if (typeIndex === -1) {
...
export class EnvelopeSettings {
  ...
  public toSettingsObject(): object {
    const envelopeObject: DynamicValue = {
      target: Config.modulationTargets[this.target]!.name,
      envelope: Config.envelopes[this.envelope]!.name,
      speed: this.speed,
      a: this.a,
      b: this.b,
    };
    if (Config.modulationTargets[this.target]!.maxCount > 1) {
      envelopeObject["index"] = this.index;
    }
    return envelopeObject;
  }

  public fromSettingsObject(envelopeObject: DynamicValue): void {
    this.reset();

    let target: ModulationTarget = Config.modulationTargets.dictionary[envelopeObject["target"]]!;
    if (target == null) {
      target = Config.modulationTargets.dictionary["noteVolume"]!;
    }
    this.target = target.index;

    const envelopeValue: DynamicValue = envelopeObject["envelope"];
    ...
    if (envelopeObject["index"] === undefined) {
      ...
      this.index = clamp(
        0,
        Config.modulationTargets[this.target]!.maxCount,
...
export class Instrument {
  ...
  public supersawShape = 0;
  public soundFontId: string | null = null;
  public soundFontPreset = 0;
  ...
  public constructor(isNoiseChannel: boolean) {
    ...
    for (let i = 0; i < Config.drumCount; i++) {
      ...
      this.drumsetEnvelopeSpeeds[i] = Config.envelopes.dictionary["twang"]!.speed;
  ...
  public setTypeAndReset(type: InstrumentType, isNoiseChannel: boolean): void {
    ...
    if (this.chord !== Config.chords.dictionary["simultaneous"]!.index) {
      ...
      this.effects |= 1 << EffectType.chord;
    }
  }

  public toSettingsObject(): object {
    const instrumentObject: DynamicValue = {
      type: Config.instrumentTypeNames[this.type]!,
      volume: (5 - this.volume) * 20,
      pan: Math.round((100 * (this.pan - Config.panCenter)) / Config.panCenter),
      eqFilter: effectsIncludeEqFilter(this.effects) ? this.eqFilter.toSettingsObject() : [],
    };
    ...
    if (effectsIncludeDistortion(this.effects)) {
      instrumentObject["distortion"] = (100 * this.distortion) / (Config.distortionRange - 1);
    }
    if (effectsIncludeBitcrusher(this.effects)) {
      instrumentObject["bitcrusherOctave"] =
        (Config.bitcrusherFreqRange - 1 - this.bitcrusherFreq) * Config.bitcrusherOctaveStep;
      instrumentObject["bitcrusherQuantization"] =
        (100 * this.bitcrusherQuantization) / (Config.bitcrusherQuantizationRange - 1);
    }
    if (effectsIncludeChorus(this.effects)) {
      instrumentObject["chorus"] = (100 * this.chorus) / (Config.chorusRange - 1);
    }
    if (effectsIncludeEcho(this.effects)) {
      instrumentObject["echoSustain"] = (100 * this.echoSustain) / (Config.echoSustainRange - 1);
      instrumentObject["echoDelayBeats"] =
        ((this.echoDelay + 1) * Config.echoDelayStepTicks) /
        (Config.ticksPerPart * Config.partsPerBeat);
    }
    if (effectsIncludeReverb(this.effects)) {
      instrumentObject["reverb"] = (100 * this.reverb) / (Config.reverbRange - 1);
    }

    instrumentObject["fadeInSeconds"] =
      Math.round(10_000 * Synth.fadeInSettingToSeconds(this.fadeIn)) / 10_000;
    ...
    if (this.type === InstrumentType.noise) {
      ...
      for (let j = 0; j < Config.drumCount; j++) {
        ...
        for (let i = 0; i < Config.spectrumControlPoints; i++) {
          spectrum[i] = Math.round(
            (100 * this.drumsetSpectrumWaves[j]!.spectrum[i]!) / Config.spectrumMax,
          );
        }
      ...
      instrumentObject["pulseWidth"] =
        Math.round(getPulseWidthRatio(this.pulseWidth) * 100 * 100_000) / 100_000;
      ...
      for (const operator of this.operators) {
        const chipWave = operator.wave === 0 ? undefined : Config.chipWaves[operator.wave - 1]!;
        operatorArray.push({
          frequency: operator.frequency,
          amplitude: operator.amplitude,
          wave: operator.wave === 0 ? "sine" : chipWave!.name,
          ...(chipWave?.sampleId === undefined ? {} : { sampleId: chipWave.sampleId }),
  ...
  public fromSettingsObject(instrumentObject: DynamicValue, isNoiseChannel: boolean): void {
    if (instrumentObject === undefined) {
      instrumentObject = {};
    }

    const typeIndex: number = Config.instrumentTypeNames.indexOf(instrumentObject["type"]);
    let type: InstrumentType = typeIndex;
    if (typeIndex === -1) {
    ...
    for (const effect of instrumentObject["effects"] ?? []) {
      ...
      if (effectIndex !== -1) {
        effects |= 1 << effectIndex;
      }
    }
    this.effects = effects;

    this.transition = Config.transitions.dictionary["normal"]!.index; // Default value.
    const transitionProperty: DynamicValue = instrumentObject["transition"];
    if (transitionProperty !== undefined) {
      const transition: Transition | undefined = Config.transitions.dictionary[transitionProperty]!;
      ...
      if (this.transition !== Config.transitions.dictionary["normal"]!.index) {
        ...
        this.effects |= 1 << EffectType.transition;
      }
    }

    if (instrumentObject["fadeInSeconds"] !== undefined) {
      this.fadeIn = Synth.secondsToFadeInSetting(Number(instrumentObject["fadeInSeconds"]));
    }
    if (instrumentObject["fadeOutTicks"] !== undefined) {
      this.fadeOut = Synth.ticksToFadeOutSetting(Number(instrumentObject["fadeOutTicks"]));
    }

    {
      // Note that the chord setting may be overridden by instrumentObject["chorus"] below.
      const chordProperty: DynamicValue = instrumentObject["chord"],
        chord: Chord | undefined = Config.chords.dictionary[chordProperty]!;
      if (chord === undefined) {
    ...
    const unisonProperty: DynamicValue = instrumentObject["unison"];
    if (unisonProperty !== undefined) {
      const unison: Unison | undefined = Config.unisons.dictionary[unisonProperty]!;
      if (unison !== undefined) {
    ...
    if (instrumentObject["pitchShiftSemitones"] !== undefined) {
      this.pitchShift = Number(instrumentObject["pitchShiftSemitones"]);
    }
    if (instrumentObject["detuneCents"] !== undefined) {
      this.detune =
        Config.detuneCenter + Synth.centsToDetune(Number(instrumentObject["detuneCents"]));
    }

    this.vibrato = Config.vibratos.dictionary["none"]!.index; // Default value.
    const vibratoProperty: DynamicValue = instrumentObject["vibrato"];
    if (vibratoProperty !== undefined) {
      const vibrato: Vibrato | undefined = Config.vibratos.dictionary[vibratoProperty]!;
    ...
    if (instrumentObject["pan"] === undefined) {
      ...
      this.pan = clamp(
        0,
        Config.panMax + 1,
        Math.round(Config.panCenter + ((instrumentObject["pan"] | 0) * Config.panCenter) / 100),
      );
    }

    if (instrumentObject["distortion"] !== undefined) {
      this.distortion =
        ((Config.distortionRange - 1) * Number(instrumentObject["distortion"])) / 100;
    }
    ...
    if (instrumentObject["bitcrusherOctave"] !== undefined) {
      this.bitcrusherFreq =
        ...
        1 -
        Number(instrumentObject["bitcrusherOctave"]) / Config.bitcrusherOctaveStep;
    }
    if (instrumentObject["bitcrusherQuantization"] !== undefined) {
      this.bitcrusherQuantization =
        ((Config.bitcrusherQuantizationRange - 1) *
          Number(instrumentObject["bitcrusherQuantization"])) /
        100;
    }

    if (instrumentObject["echoSustain"] !== undefined) {
      this.echoSustain =
        ((Config.echoSustainRange - 1) * Number(instrumentObject["echoSustain"])) / 100;
    }
    if (instrumentObject["echoDelayBeats"] !== undefined) {
      this.echoDelay =
        (Number(instrumentObject["echoDelayBeats"]) * (Config.ticksPerPart * Config.partsPerBeat)) /
          Config.echoDelayStepTicks -
        1.0;
    }

    if (!isNaN(instrumentObject["chorus"])) {
      this.chorus = ((Config.chorusRange - 1) * Number(instrumentObject["chorus"])) / 100;
    }

    if (instrumentObject["reverb"] !== undefined) {
      this.reverb = ((Config.reverbRange - 1) * Number(instrumentObject["reverb"])) / 100;
    }
    ...
    if (instrumentObject["pulseWidth"] === undefined) {
      ...
      this.pulseWidth = clamp(
        ...
        Math.round(Math.log2(Number(instrumentObject["pulseWidth"]) / 50) / 0.5 - 1 + 8),
      );
    }
    ...
    if (instrumentObject["spread"] === undefined) {
      ...
      this.supersawSpread = clamp(
        ...
        Math.round((Config.supersawSpreadMax * (instrumentObject["spread"] | 0)) / 100),
      );
    }
    if (instrumentObject["shape"] === undefined) {
      ...
      this.supersawShape = clamp(
        ...
        Math.round((Config.supersawShapeMax * (instrumentObject["shape"] | 0)) / 100),
      );
    }
    ...
    if (instrumentObject["harmonics"] === undefined) {
      ...
      for (let i = 0; i < Config.harmonicsControlPoints; i++) {
        this.harmonicsWave.harmonics[i] = Math.max(
          ...
          Math.min(
            Config.harmonicsMax,
            Math.round((Config.harmonicsMax * Number(instrumentObject["harmonics"][i])) / 100),
          ),
        );
      }
    ...
    if (instrumentObject["spectrum"] === undefined) {
      ...
      for (let i = 0; i < Config.spectrumControlPoints; i++) {
        this.spectrumWave.spectrum[i] = Math.max(
          ...
          Math.min(
            Config.spectrumMax,
            Math.round((Config.spectrumMax * Number(instrumentObject["spectrum"][i])) / 100),
          ),
        );
      }
    ...
    if (instrumentObject["stringSustain"] === undefined) {
      ...
      this.stringSustain = clamp(
        ...
        Math.round(
          ((Config.stringSustainRange - 1) * (instrumentObject["stringSustain"] | 0)) / 100,
        ),
      );
    }
    const stringSustainType: number = Config.enableAcousticSustain
      ? Config.sustainTypeNames.indexOf(instrumentObject["stringSustainType"])
      : SustainType.bright;
    this.stringSustainType = stringSustainType === -1 ? SustainType.bright : stringSustainType;
    ...
    if (this.type === InstrumentType.drumset) {
      if (instrumentObject["drums"] !== undefined) {
        for (let j = 0; j < Config.drumCount; j++) {
          const drum: DynamicValue = instrumentObject["drums"][j];
          if (drum === undefined) {
            continue;
          }
          ...
          if (drum["filterEnvelope"] !== undefined) {
            ...
            if (envelope !== undefined) {
              ...
              this.drumsetEnvelopeSpeeds[j] = Number.isFinite(Number(drum["filterEnvelopeSpeed"]))
                ? Number(drum["filterEnvelopeSpeed"])
                : envelope.speed;
              this.drumsetEnvelopeAs[j] = Number.isFinite(Number(drum["filterEnvelopeA"]))
                ? Number(drum["filterEnvelopeA"])
                : envelope.a;
              this.drumsetEnvelopeBs[j] = Number.isFinite(Number(drum["filterEnvelopeB"]))
          ...
          if (drum["spectrum"] !== undefined) {
            for (let i = 0; i < Config.spectrumControlPoints; i++) {
              this.drumsetSpectrumWaves[j]!.spectrum[i] = Math.max(
                ...
                Math.min(
                  Config.spectrumMax,
                  Math.round((Config.spectrumMax * Number(drum["spectrum"][i])) / 100),
                ),
              );
            }
          }
    ...
    if (this.type === InstrumentType.chip) {
      ...
      this.chipWave =
        typeof sampleId === "string"
          ...
          : Config.chipWaves.findIndex((wave) => wave.name === instrumentObject["wave"]);
      if (this.chipWave === -1) {
        this.chipWave = 1;
      }
    ...
    if (this.type === InstrumentType.fm) {
      ...
      for (let j = 0; j < Config.operatorCount; j++) {
        ...
        let operatorObject: DynamicValue = undefined;
        ...
        const sampleId: unknown = operatorObject["sampleId"],
          chipWave: number =
            typeof sampleId === "string"
              ? Config.chipWaves.findIndex((wave) => wave.sampleId === sampleId)
              : Config.chipWaves.findIndex((wave) => wave.name === operatorObject["wave"]);
    ...
    if (this.type === InstrumentType.soundFont) {
      ...
      this.soundFontPreset = Math.max(
        0,
        Math.min(0x3_ff_ff, instrumentObject["soundFontPreset"] | 0),
      );
    }
    ...
    if (Array.isArray(instrumentObject["envelopes"])) {
      const envelopeArray: DynamicValue[] = instrumentObject["envelopes"];
  ...
  public addEnvelope(
    ...
    if (!this.supportsEnvelopeTarget(target, index)) {
      throw new Error("Unsupported envelope target.");
    }
    if (this.envelopeCount >= Config.maxEnvelopeCount) {
      throw new Error("Maximum envelope count reached.");
    }
  ...
  public supportsEnvelopeTarget(target: number, index: number): boolean {
    ...
    if (modulationTarget.supportsEnvelope !== true) {
      return false;
    }
    if (index < 0 || index >= modulationTarget.maxCount) {
      return false;
    }
    if (
      modulationTarget.compatibleInstruments != null &&
      modulationTarget.compatibleInstruments.indexOf(this.type) === -1
    ) {
      return false;
    }
    if (modulationTarget.effect != null && (this.effects & (1 << modulationTarget.effect)) === 0) {
      return false;
    }
    if (modulationTarget.isFilter) {
      const filter: FilterSettings =
        modulationTarget.perNote === false ? this.eqFilter : this.noteFilter;
      if (index >= filter.controlPointCount) {
        return false;
      }
    }
    return true;
  ...
  public clearInvalidEnvelopeTargets(): void {
    for (let envelopeIndex = 0; envelopeIndex < this.envelopeCount; envelopeIndex++) {
      const target: number = this.envelopes[envelopeIndex]!.target,
        index: number = this.envelopes[envelopeIndex]!.index;
      if (!this.supportsEnvelopeTarget(target, index)) {
        this.envelopes[envelopeIndex]!.target = Config.modulationTargets.dictionary["none"]!.index;
  ...
  public toBinaryState(): object {
    const filterState = (filter: FilterSettings): object[] => {
      const points: object[] = [];
      for (let i = 0; i < filter.controlPointCount; i++) {
    ...
    return {
      ...
      drumsetSpectra: this.drumsetSpectrumWaves.map((wave: SpectrumWave): number[] =>
        wave.spectrum.concat(),
      ),
    };
  }

  public fromBinaryState(binaryState: DynamicValue): void {
    if (
      binaryState == null ||
    ...
    const numberValue = (value: unknown): number => {
      ...
      integerValue = (value: unknown): number => {
      ...
      indexValue = (value: unknown, length: number): number => {
      ...
      rangeValue = (value: unknown, min: number, max: number): number => {
        const result: number = numberValue(value);
      ...
      numberArray = (value: unknown, expectedLength: number): number[] => {
      ...
      boundedNumberArray = (
        ...
        numberArray(value, expectedLength).map((item: number): number =>
    ...
    const chipWavePitch: number = numberValue(binaryState.chipWavePitch),
      chipWaveTempo: number = numberValue(binaryState.chipWaveTempo),
      chipWaveOffset: number = rangeValue(binaryState.chipWaveOffset, 0, 1),
      chipWaveLoopStart: number = rangeValue(binaryState.chipWaveLoopStart, 0, 1),
      chipWaveLoopEnd: number = rangeValue(binaryState.chipWaveLoopEnd, chipWaveLoopStart, 1);
    ...
    indexValue(scalarValues["stringSustainType"]!, Config.sustainTypeNames.length);
    ...
    if (effects !== this.effects || effects < 0 || effects >= 2 ** Config.effectNames.length) {
      throw new Error("Invalid .goop instrument effects.");
    }
    const soundFontPreset: number = integerValue(scalarValues["soundFontPreset"]!);
    if (soundFontPreset < 0 || soundFontPreset > 0x3_ff_ff) {
    ...
    rangeValue(scalarValues["feedbackAmplitude"]!, 0, Config.operatorAmplitudeMax);
    ...
    const filterState = (value: unknown): BinaryFilterPoint[] => {
        ...
        for (const candidate of value) {
          ...
          const point: DynamicValue = candidate,
            type: FilterType = indexValue(point.type, Config.filterTypeNames.length);
          points.push({
            type,
      ...
      eqFilterState: BinaryFilterPoint[] = filterState(binaryState.eqFilter),
      noteFilterState: BinaryFilterPoint[] = filterState(binaryState.noteFilter);
    ...
    for (const candidate of binaryState.envelopes) {
      ...
      const envelope: DynamicValue = candidate,
        target: number = indexValue(envelope.target, Config.modulationTargets.length),
        index: number = integerValue(envelope.index),
        modulationTarget: ModulationTarget = Config.modulationTargets[target]!;
      if (
        index < 0 ||
        index >= modulationTarget.maxCount ||
        modulationTarget.supportsEnvelope !== true ||
        (modulationTarget.compatibleInstruments != null &&
          modulationTarget.compatibleInstruments.indexOf(this.type) === -1) ||
        (modulationTarget.effect != null && (effects & (1 << modulationTarget.effect)) === 0) ||
        (modulationTarget.isFilter &&
          index >=
            (modulationTarget.perNote === false ? eqFilterState.length : noteFilterState.length))
      ) {
      ...
      const parameterLimit: number =
        modulationTarget.isFilter || modulationTarget.name === "none" ? 2 ** 28 : 4;
      envelopeStates.push({
        target,
        index,
    ...
    for (let i = 0; i < this.operators.length; i++) {
      const candidate: DynamicValue = binaryState.operators[i];
      if (candidate == null || typeof candidate !== "object" || Array.isArray(candidate)) {
        throw new Error("Invalid .goop instrument operator.");
      }
      const offset: number =
          candidate.offset === undefined ? 0 : rangeValue(candidate.offset, 0, 1),
        loopStart: number =
          candidate.loopStart === undefined ? offset : rangeValue(candidate.loopStart, 0, 1),
        loopEnd: number =
          candidate.loopEnd === undefined ? 1 : rangeValue(candidate.loopEnd, loopStart, 1);
      if (candidate.oneshot !== undefined && typeof candidate.oneshot !== "boolean") {
        throw new Error("Invalid .goop operator oneshot setting.");
      }
      operatorStates.push({
        frequency: rangeValue(candidate.frequency, 0, Config.operatorFrequencyMax),
        amplitude: rangeValue(candidate.amplitude, 0, Config.operatorAmplitudeMax),
    ...
    const spectrumState: number[] = boundedNumberArray(
      ...
      harmonicsState: number[] = boundedNumberArray(
        binaryState.harmonics,
        Config.harmonicsControlPoints,
        0,
        Config.harmonicsMax,
      ),
      drumsetEnvelopesState: number[] = numberArray(
        binaryState.drumsetEnvelopes,
        Config.drumCount,
      ).map((value: number): number => indexValue(value, Config.envelopes.length)),
      drumsetEnvelopeSpeedsState: number[] = boundedNumberArray(
      ...
      filterEnvelopeParameterLimit: number = 2 ** 28,
      drumsetEnvelopeAsState: number[] = boundedNumberArray(
        binaryState.drumsetEnvelopeAs,
        Config.drumCount,
        -filterEnvelopeParameterLimit,
        filterEnvelopeParameterLimit,
      ),
      drumsetEnvelopeBsState: number[] = boundedNumberArray(
    ...
    const drumsetSpectraState: number[][] = binaryState.drumsetSpectra.map(
      (spectrum: unknown): number[] =>
        boundedNumberArray(spectrum, Config.spectrumControlPoints, 0, Config.spectrumMax),
    );
    ...
    for (const name of scalarNames) {
      (this as DynamicValue)[name] = scalarValues[name]!;
    }
    ...
    const restoreFilter = (filter: FilterSettings, points: BinaryFilterPoint[]): void => {
      filter.reset();
    ...
    for (let i = 0; i < envelopeStates.length; i++) {
      const source: BinaryEnvelope = envelopeStates[i]!,
        destination: EnvelopeSettings = this.envelopes[i]!;
    ...
    for (let i = 0; i < this.operators.length; i++) {
      ...
      this.operators[i]!.chipWaveSettings.loopStart = operatorStates[i]!.loopStart;
    ...
    this.harmonicsWave.markCustomWaveDirty();
    this.drumsetEnvelopes.splice(0, this.drumsetEnvelopes.length, ...drumsetEnvelopesState);
    this.drumsetEnvelopeSpeeds.splice(
      0,
      this.drumsetEnvelopeSpeeds.length,
      ...drumsetEnvelopeSpeedsState,
    );
    this.drumsetEnvelopeAs.splice(0, this.drumsetEnvelopeAs.length, ...drumsetEnvelopeAsState);
    this.drumsetEnvelopeBs.splice(0, this.drumsetEnvelopeBs.length, ...drumsetEnvelopeBsState);
    for (let i = 0; i < Config.drumCount; i++) {
...
export class Song {
  ...
  public getChannelCount(): number {
    return this.pitchChannelCount + this.noiseChannelCount + this.automationChannelCount;
  }
  ...
  public getChannelKind(channelIndex: number): ChannelKind {
    if (this.getChannelIsPitch(channelIndex)) {
      return ChannelKind.pitch;
    }
    if (this.getChannelIsNoise(channelIndex)) {
      return ChannelKind.noise;
    }
  ...
  public createChannel(kind: ChannelKind): Channel {
    ...
    if (kind === ChannelKind.automation) {
      for (let rowIndex = 0; rowIndex < Config.automationRowCountDefault; rowIndex++) {
      ...
      for (
        ...
        instrument.setTypeAndReset(isNoise ? InstrumentType.noise : InstrumentType.chip, isNoise);
        channel.instruments.push(instrument);
      }
    }
    for (let patternIndex = 0; patternIndex < this.patternsPerChannel; patternIndex++) {
      const pattern: Pattern = new Pattern();
      if (kind === ChannelKind.automation) {
  ...
  public forEachAutomationRow(
    ...
    const firstAutomation: number = this.pitchChannelCount + this.noiseChannelCount;
    for (
  ...
  public remapAutomationChannelReferences(oldToNew: readonly (number | null)[]): void {
    this.forEachAutomationRow((row: AutomationRow): void => {
      if (row.targetChannel < 0 || row.targetChannelMissing) {
        return;
      }
  ...
  public remapAutomationInstrumentReferences(
    ...
    this.forEachAutomationRow((row: AutomationRow): void => {
      ...
      const replacement: number | null | undefined = oldToNew[row.targetInstrument]!;
      if (replacement == null) {
        row.targetInstrumentMissing = true;
      } else {
  ...
  public referencesChannel(channelIndex: number): AutomationRow[] {
    ...
    this.forEachAutomationRow((row: AutomationRow): void => {
      if (row.targetChannel === channelIndex) {
        result.push(row);
      }
    });
    return result;
  ...
  public initToDefault(andResetChannels = true): void {
    ...
    if (andResetChannels) {
      ...
      for (let channelIndex = 0; channelIndex < this.getChannelCount(); channelIndex++) {
        const kind: ChannelKind = this.getChannelKind(channelIndex),
          isNoiseChannel: boolean = kind === ChannelKind.noise;
        ...
        for (let pattern = 0; pattern < this.patternsPerChannel; pattern++) {
          ...
          if (kind === ChannelKind.automation) {
            channel.patterns[pattern]!.ensureAutomationRowCount(Config.automationRowCountDefault);
          }
        }
  ...
  #configureAssets(sources: readonly string[]): void {
    this.assets.length = 0;
  ...
  public toBinary(): Uint8Array {
    return encodeSongBinary(this.#toCompactBinaryValue());
  }
  ...
  public toBinaryObject(): object {
    ...
    for (let channelIndex = 0; channelIndex < this.getChannelCount(); channelIndex++) {
      const channel: Channel = this.channels[channelIndex]!,
        kind: ChannelKind = this.getChannelKind(channelIndex),
        instrumentArray: object[] =
          kind === ChannelKind.automation
            ? []
            : channel.instruments.map((instrument: Instrument): object =>
                instrument.toBinaryState(),
              ),
        patternArray: object[] = [];
      for (const pattern of channel.patterns) {
        patternArray.push(
      ...
      const channelObject: DynamicValue = {
      ...
      if (kind === ChannelKind.automation) {
        channelObject.automationRows = channel.automationRows.map((row: AutomationRow): object => ({
  ...
  static #binaryValuesEqual(left: DynamicValue, right: DynamicValue): boolean {
    ...
    if (Array.isArray(left)) {
      ...
      for (let i = 0; i < left.length; i++) {
        if (!Song.#binaryValuesEqual(left[i], right[i])) {
          return false;
        }
      }
      return true;
    ...
    if (left != null && right != null && typeof left === "object" && typeof right === "object") {
      const leftKeys: string[] = Object.keys(left),
        rightKeys: string[] = Object.keys(right);
      if (leftKeys.length !== rightKeys.length) {
        return false;
      }
      for (const key of leftKeys) {
        if (!Object.hasOwn(right, key) || !Song.#binaryValuesEqual(left[key], right[key])) {
          return false;
        }
      }
      return true;
  ...
  #toCompactBinaryValue(): unknown[] {
    const defaultInstrumentStates = new Map<string, DynamicValue>(),
      assetSources: string[] = this.assets.map((asset: AssetDefinition): string => asset.source),
      channels: unknown[] = [],
      barBits: CompactBitWriter = new CompactBitWriter(),
      patternBits: CompactBitWriter = new CompactBitWriter(),
      shapeBits: CompactBitWriter = new CompactBitWriter(),
      barBitCount: number = Song.getNeededBits(this.patternsPerChannel),
      noteSizeCounts: number[] = new Array<number>(Config.noteSizeMax + 1).fill(0);
    let integerNoteSizeCount = 0,
      fractionalNoteSizeCount = 0;
    const durationDivisors: readonly number[] = [1, 2, 3, 4, 6, 8, 12],
      durationCounts = new Map<number, number>();
    for (let channelIndex = 0; channelIndex < this.getChannelCount(); channelIndex++) {
      if (this.getChannelIsAutomation(channelIndex)) {
        continue;
      }
      const channel: Channel = this.channels[channelIndex]!,
        recentShapeKeys: string[] = [];
      for (const pattern of channel.patterns) {
        let currentPart = 0;
        for (const note of pattern.notes) {
          ...
          const shapeValues: unknown[] = [
            ...
            shapeDurations: number[] = [];
          let pinTime = 0,
            currentPitch: number = note.pitches[0]!;
          for (let pinIndex = 1; pinIndex < note.pins.length; pinIndex++) {
            const pin: NotePin = note.pins[pinIndex]!,
              nextPitch: number = note.pitches[0]! + pin.interval,
              duration: number = pin.time - pinTime;
            shapeValues.push(currentPitch === nextPitch ? 0 : 1, duration, pin.size);
            shapeDurations.push(duration);
            currentPitch = nextPitch;
            pinTime = pin.time;
          }
          const shapeKey: string = JSON.stringify(shapeValues),
            shapeIndex: number = recentShapeKeys.indexOf(shapeKey);
          if (shapeIndex === -1) {
    ...
    let commonNoteSize = 0;
    for (let size = 1; size < noteSizeCounts.length; size++) {
      if (noteSizeCounts[size]! > noteSizeCounts[commonNoteSize]!) {
        commonNoteSize = size;
      }
    }
    const commonCount: number = noteSizeCounts[commonNoteSize]!,
      fixedSizeBits: number = 1 + integerNoteSizeCount * 4 + fractionalNoteSizeCount * 68,
      commonSizeBits: number =
        5 + commonCount + (integerNoteSizeCount - commonCount) * 5 + fractionalNoteSizeCount * 69,
      useCommonNoteSize: boolean = integerNoteSizeCount > 0 && commonSizeBits < fixedSizeBits;
    let huffmanNoteSizeBits: number = fractionalNoteSizeCount * 71;
    for (let size = 0; size < noteSizeCounts.length; size++) {
    ...
    const useHuffmanNoteSize: boolean =
        huffmanNoteSizeBits < (useCommonNoteSize ? commonSizeBits : fixedSizeBits),
      longTailBitCount = (minimum: number, minBits: number, value: number): number => {
        value -= minimum;
        let bitCount: number = minBits,
          result: number = 1 + bitCount;
        while (value >= 2 ** bitCount) {
          value -= 2 ** bitCount++;
          result += 2;
        }
    ...
    let durationDivisorIndex = 0,
      durationMinBits = 0,
      bestDurationBits: number = Number.POSITIVE_INFINITY;
    for (let divisorIndex = 0; divisorIndex < durationDivisors.length; divisorIndex++) {
      const divisor: number = durationDivisors[divisorIndex]!;
      for (let minBits = 0; minBits <= 4; minBits++) {
        let bits = 0;
    ...
    if (!useHuffmanNoteSize) {
      ...
      if (useCommonNoteSize) {
        patternBits.write(4, commonNoteSize);
      }
    }

    for (let channelIndex = 0; channelIndex < this.getChannelCount(); channelIndex++) {
      const channel: Channel = this.channels[channelIndex]!,
        isNoiseChannel: boolean = this.getChannelIsNoise(channelIndex),
        isAutomationChannel: boolean = this.getChannelIsAutomation(channelIndex),
        instruments: unknown[] = [];
      for (const instrument of channel.instruments) {
        const state: DynamicValue = instrument.toBinaryState();
        for (const key of Object.keys(state)) {
          if (key === "type" || key === "preset" || key === "effects") {
            continue;
          }
          if (!compactInstrumentFieldSet.has(key)) {
            throw new Error(`Compact song codec is missing instrument field: ${key}.`);
          }
        }

        const defaultsKey = `${isNoiseChannel ? 1 : 0}:${instrument.type}`;
        let defaultState: DynamicValue = defaultInstrumentStates.get(defaultsKey);
        ...
        for (let fieldIndex = 0; fieldIndex < compactInstrumentFields.length; fieldIndex++) {
          const field: string = compactInstrumentFields[fieldIndex]!;
          if (Song.#binaryValuesEqual(state[field], defaultState[field])) {
            continue;
          }
          if (fieldIndex < compactInstrumentLowFieldCount) {
            lowMask += 2 ** fieldIndex;
          } else {
            highMask += 2 ** (fieldIndex - compactInstrumentLowFieldCount);
          }
          ...
          if (
            ...
            const assetIndex: number = assetSources.indexOf(fieldValue.slice("asset:".length));
            values.push(assetIndex !== -1 ? assetIndex : fieldValue);
          } else {
      ...
      const recentPitches: number[] = isNoiseChannel
          ...
          : [0, 7, 12, 19, 24, -5, -12],
        recentShapes: string[] = [];
      for (let index = 0; index < recentPitches.length; index++) {
      ...
      for (const pattern of channel.patterns) {
        ...
        for (const note of pattern.notes) {
          ...
          shapeBits.clear();
          for (let pitchIndex = 1; pitchIndex < note.pitches.length; pitchIndex++) {
            shapeBits.write(1, 1);
          }
          shapeBits.write(1, 0);
          ...
          for (let pinIndex = 1; pinIndex < note.pins.length; pinIndex++) {
            const pin: NotePin = note.pins[pinIndex]!,
              nextPitch: number = startPitch + pin.interval;
            ...
            shapeBits.writePartDuration(pin.time - shapePart, durationDivisor, durationMinBits);
            shapePart = pin.time;
            shapeBits.writeNoteSize(
              pin.size,
              useCommonNoteSize ? commonNoteSize : null,
          ...
          const shapeKey: string = shapeBits.key(),
            shapeIndex: number = recentShapes.indexOf(shapeKey);
          ...
          for (let pitchIndex = 0; pitchIndex < allPitches.length; pitchIndex++) {
            const pitch: number = allPitches[pitchIndex]!,
              recentPitchIndex: number = recentPitches.indexOf(pitch);
            patternBits.writePitchToken(recentPitchIndex);
            if (recentPitchIndex === -1) {
              let interval = 0,
                pitchIterator: number = lastPitch;
              if (pitchIterator < pitch) {
                while (pitchIterator !== pitch) {
                  pitchIterator++;
                  if (recentPitches.indexOf(pitchIterator) === -1) {
                    interval++;
                  }
                }
              } else {
            ...
            lastPitch = pitchIndex === note.pitches.length - 1 ? note.pitches[0]! : pitch;
          }
    ...
    const commonPrefix: unknown[] = [
      this.automationChannelCount === 0 ? legacyCompactSongValueVersion : compactSongValueVersion,
      this.scale,
      this.key,
      this.composingKey,
      this.tempo,
    ...
    if (this.automationChannelCount === 0) {
      return [...commonPrefix, assetSources, channels, barBits.finish(), patternBits.finish()];
    }
    return [
      ...commonPrefix,
      ...
      barBits.finish(),
      patternBits.finish(),
      this.#toCompactAutomationValue(),
    ];
  }

  #toCompactAutomationValue(): unknown[] {
    const result: unknown[] = [],
      firstAutomation: number = this.pitchChannelCount + this.noiseChannelCount;
    for (
      let channelIndex: number = firstAutomation;
      channelIndex < this.getChannelCount();
      channelIndex++
      ...
      const channel: Channel = this.channels[channelIndex]!,
        rows: unknown[] = channel.automationRows.map((row: AutomationRow): unknown[] => [
          row.targetChannel,
        ...
        patterns: unknown[] = channel.patterns.map((pattern: Pattern): unknown[] =>
          pattern.automationEvents.map((events: Event[]): unknown[] =>
            events.map((event: Event): unknown[] => [
              event.start,
              event.end,
              event.points.flatMap((point: EventPoint): number[] => [point.time, point.value]),
            ]),
          ),
        );
  ...
  static #expandCompactInstruments(compactInstruments: unknown, assets: unknown): object[] {
    if (!Array.isArray(compactInstruments)) {
      throw new Error("Invalid compact .goop instrument data.");
    }
    const highFieldCount: number = compactInstrumentFields.length - compactInstrumentLowFieldCount,
      maximumLowMask: number = 2 ** compactInstrumentLowFieldCount - 1,
      maximumHighMask: number = 2 ** highFieldCount - 1;
    return compactInstruments.map((compactInstrument: unknown): object => {
      ...
      const lowMask: unknown = compactInstrument[3],
        highMask: unknown = compactInstrument[4];
      if (
      ...
      let valueIndex = 5;
      for (let fieldIndex = 0; fieldIndex < compactInstrumentFields.length; fieldIndex++) {
        const mask: number = fieldIndex < compactInstrumentLowFieldCount ? lowMask : highMask,
          bitIndex: number =
            fieldIndex < compactInstrumentLowFieldCount
              ? fieldIndex
              : fieldIndex - compactInstrumentLowFieldCount;
        if (Math.floor(mask / 2 ** bitIndex) % 2 === 0) {
          continue;
        }
        ...
        const field: string = compactInstrumentFields[fieldIndex]!,
          fieldValue: unknown = compactInstrument[valueIndex++];
  ...
  static #decodeCompactBars(
    ...
    const reader: CompactBitReader = new CompactBitReader(bytes),
      bitCount: number = Song.getNeededBits(patternsPerChannel),
      channels: number[][] = [];
  ...
  static #decodeCompactPatterns(
    ...
    const reader: CompactBitReader = new CompactBitReader(bytes),
      decodedChannels: object[][] = [],
      partsPerBar: number = beatsPerBar * Config.partsPerBeat,
      durationDivisors: readonly number[] = [1, 2, 3, 4, 6, 8, 12],
      durationDivisorIndex: number = reader.read(3);
    if (durationDivisorIndex >= durationDivisors.length) {
      throw new Error("Invalid compact .goop duration divisor.");
    }
    const durationDivisor: number = durationDivisors[durationDivisorIndex]!,
      durationMinBits: number = reader.read(3);
    ...
    const useHuffmanNoteSize: boolean = reader.read(1) !== 0,
      commonNoteSize: number | null = useHuffmanNoteSize
        ? null
        : reader.read(1) !== 0
          ? reader.read(4)
          : null;
    ...
    for (let channelIndex = 0; channelIndex < compactChannels.length; channelIndex++) {
      ...
      const isNoiseChannel: boolean = channelIndex >= pitchChannelCount,
        octaveOffset: number = isNoiseChannel ? 0 : octave * Config.pitchesPerOctave,
        maximumPitch: number = isNoiseChannel ? Config.drumCount - 1 : Config.maxPitch;
      let lastPitch: number = isNoiseChannel ? 4 : octaveOffset;
      const recentPitches: number[] = isNoiseChannel
          ? [4, 6, 7, 2, 3, 8, 0, 10]
          : [0, 7, 12, 19, 24, -5, -12],
        recentShapes: DynamicValue[] = [];
      for (let index = 0; index < recentPitches.length; index++) {
        recentPitches[index]! += octaveOffset;
      }
      ...
      for (let patternIndex = 0; patternIndex < patternsPerChannel; patternIndex++) {
        ...
        while (currentPart < partsPerBar) {
          ...
          if (shapeToken >= 0) {
            ...
            currentPart += reader.readPartDuration(durationDivisor, durationMinBits);
            if (currentPart > partsPerBar) {
            ...
            shape.initialSize = reader.readNoteSize(commonNoteSize, useHuffmanNoteSize);
            for (let pinIndex = 0; pinIndex < shape.pinCount; pinIndex++) {
              const pitchBend: boolean = reader.read(1) !== 0;
              if (pitchBend) {
                shape.bendCount++;
              }
              shape.length += reader.readPartDuration(durationDivisor, durationMinBits);
          ...
          const pitches: number[] = [],
            pitchBends: number[] = [],
            totalPitchCount: number = shape.pitchCount + shape.bendCount;
          if (totalPitchCount > Config.maxChordSize + partsPerBar) {
            throw new Error("Invalid compact .goop pitch count.");
          }
          for (let pitchIndex = 0; pitchIndex < totalPitchCount; pitchIndex++) {
            let pitch: number;
            ...
            if (pitchIndex < shape.pitchCount) {
              pitches.push(pitch);
            } else {
          ...
          const pins: object[] = [{ interval: 0, time: 0, size: shape.initialSize }];
          for (const shapePin of shape.pins) {
            if (shapePin.pitchBend) {
              pitchBends.shift();
            }
          ...
          const continuesLastPattern: boolean = currentPart === 0 ? reader.read(1) !== 0 : false;
          notes.push({
            pitches,
            pins,
            start: currentPart,
  ...
  static #expandCompactAutomationValue(
    value: unknown,
    ...
    return value.map((channelValue: unknown): object => {
      ...
      const rowValues: unknown = channelValue[0],
        patternValues: unknown = channelValue[1];
      if (
      ...
      const automationRows: object[] = rowValues.map((rowValue: unknown): object => {
          if (!Array.isArray(rowValue) || rowValue.length !== 6) {
            throw new Error("Invalid compact .goop automation row.");
          }
          const [targetChannel, targetKind, targetInstrument, targetId, targetIndex, flags] =
            rowValue;
          if (
          ...
          return {
            targetChannel,
            targetChannelKind: targetKind === 0 ? ChannelKind.pitch : ChannelKind.noise,
            targetInstrument,
            targetId,
            targetIndex,
        ...
        patterns: object[] = patternValues.map((patternValue: unknown): object => {
          ...
          const rows: object[][] = patternValue.map((eventsValue: unknown): object[] => {
            if (
            ...
            return eventsValue.map((eventValue: unknown): object => {
              ...
              const points: object[] = [];
              for (let index = 0; index < pointValues.length; index += 2) {
                const time: unknown = pointValues[index],
                  operand: unknown = pointValues[index + 1];
                if (
                  typeof time !== "number" ||
  ...
  static #expandCompactBinaryValue(value: unknown): object {
    if (!Array.isArray(value) || value.length === 0) {
      throw new Error("Invalid compact .goop song data.");
    }
    const version: unknown = value[0],
      isVersion1: boolean = version === legacyCompactSongValueVersion,
      isVersion2: boolean = version === compactSongValueVersion;
    if ((!isVersion1 && !isVersion2) || value.length !== (isVersion1 ? 17 : 19)) {
      throw new Error("Invalid compact .goop song data.");
    }
    const compactInteger = (
      ...
      beatsPerBar: number = compactInteger(
        value[5],
        Config.beatsPerBarMin,
        Config.beatsPerBarMax,
        "beats per bar",
      ),
      barCount: number = compactInteger(
        value[6],
        Config.barCountMin,
        Config.barCountMax,
        "bar count",
      ),
      patternsPerChannel: number = compactInteger(value[7], 1, Config.barCountMax, "pattern count"),
      pitchChannelCount: number = compactInteger(
        value[11],
        Config.pitchChannelCountMin,
        Config.pitchChannelCountMax,
        "pitch channel count",
      ),
      noiseChannelCount: number = compactInteger(
        value[12],
        Config.noiseChannelCountMin,
        Config.noiseChannelCountMax,
        "noise channel count",
      ),
      automationChannelCount: number = isVersion2
        ? compactInteger(
            value[13],
        ...
        : 0,
      assetsIndex: number = isVersion2 ? 14 : 13,
      channelsIndex: number = isVersion2 ? 15 : 14,
      barsIndex: number = isVersion2 ? 16 : 15,
      patternsIndex: number = isVersion2 ? 17 : 16,
      compactChannels: unknown = value[channelsIndex],
      barData: unknown = value[barsIndex],
      patternData: unknown = value[patternsIndex];
    if (
    ...
    const musicalChannelCount: number = pitchChannelCount + noiseChannelCount,
      channelCount: number = musicalChannelCount + automationChannelCount;
    if (compactChannels.length !== channelCount) {
      throw new Error("Invalid compact .goop channel count.");
    }
    const bars: number[][] = Song.#decodeCompactBars(
        barData,
        channelCount,
        barCount,
        patternsPerChannel,
      ),
      patterns: object[][] = Song.#decodeCompactPatterns(
        patternData,
      ...
      compactAutomationChannels: object[] = isVersion2
        ? Song.#expandCompactAutomationValue(value[18], automationChannelCount, patternsPerChannel)
        : [],
      channels: object[] = compactChannels.map(
        (compactChannel: unknown, channelIndex: number): object => {
          ...
          if (channelIndex >= musicalChannelCount) {
            ...
            const automationChannel: DynamicValue =
          ...
          return {
            ...
            instruments: Song.#expandCompactInstruments(compactChannel[1], value[assetsIndex]),
  ...
  public fromBinary(data: Uint8Array): void {
    ...
    try {
      const decodedSong: unknown = decodeSongBinary(data),
        songObject: unknown = Array.isArray(decodedSong)
          ? Song.#expandCompactBinaryValue(decodedSong)
          : decodedSong;
      if (songObject == null || typeof songObject !== "object" || Array.isArray(songObject)) {
        throw new Error("Invalid .goop song data.");
      }
      const parsedSong: Song = new Song();
      parsedSong.#parseBinaryObject(songObject);
      this.#replaceWith(parsedSong);
    } catch (error) {
  ...
  #parseBinaryObject(songObject: DynamicValue): void {
    ...
    if (
      ...
      !songObject.assets.every(
        (source: unknown): source is string =>
          typeof source === "string" && source.length <= maximumAssetSourceLength,
      )
    ) {
    ...
    this.beatsPerBar = integer("beatsPerBar", Config.beatsPerBarMin, Config.beatsPerBarMax);
    this.barCount = integer("barCount", Config.barCountMin, Config.barCountMax);
    this.patternsPerChannel = integer("patternsPerChannel", 1, Config.barCountMax);
    ...
    this.#configureAssets(songObject.assets);
    this.channels.length = 0;
    for (let channelIndex = 0; channelIndex < songObject.channels.length; channelIndex++) {
      const channelObject: DynamicValue = songObject.channels[channelIndex],
        kind: ChannelKind = this.getChannelKind(channelIndex),
        isAutomationChannel: boolean = kind === ChannelKind.automation;
      if (
        channelObject == null ||
      ...
      const isNoiseChannel: boolean = kind === ChannelKind.noise,
        octave: unknown = channelObject.octave;
      if (
        typeof octave !== "number" ||
        !Number.isInteger(octave) ||
        octave < 0 ||
      ...
      if (isAutomationChannel) {
        ...
        for (const rowObject of channelObject.automationRows) {
          if (
            ...
            rowObject.targetChannel >= Config.pitchChannelCountMax + Config.noiseChannelCountMax ||
      ...
      for (const instrumentState of channelObject.instruments) {
        if (
          ...
          instrumentState.preset > 0xff_ff_ff_ff ||
        ...
        const expandedState: DynamicValue = instrument.toBinaryState();
      ...
      for (const patternObject of channelObject.patterns) {
        const pattern: Pattern = new Pattern();
        if (isAutomationChannel) {
          pattern.fromAutomationBinaryObject(patternObject, this, channel.automationRows);
        } else {
...
class PickedString {
  ...
  public allPassG = 0.0;
  public allPassGDelta = 0.0;
  public sustainFilterA1 = 0.0;
  public sustainFilterA1Delta = 0.0;
  public sustainFilterA2 = 0.0;
  public sustainFilterA2Delta = 0.0;
  public sustainFilterB0 = 0.0;
  public sustainFilterB0Delta = 0.0;
  public sustainFilterB1 = 0.0;
  public sustainFilterB1Delta = 0.0;
  public sustainFilterB2 = 0.0;
  public sustainFilterB2Delta = 0.0;
  ...
  public update(
    ...
    const allPassCenter: number =
      ...
      prevDelayLength: number = this.prevDelayLength,
      phaseDeltaStart: number = tone.phaseDeltas[stringIndex]!,
      phaseDeltaScale: number = tone.phaseDeltaScales[stringIndex]!,
      phaseDeltaEnd: number = phaseDeltaStart * phaseDeltaScale ** roundedSamplesPerTick,
      radiansPerSampleStart: number = Math.PI * 2.0 * phaseDeltaStart,
      radiansPerSampleEnd: number = Math.PI * 2.0 * phaseDeltaEnd,
      centerHarmonicStart: number = radiansPerSampleStart * 2.0,
      centerHarmonicEnd: number = radiansPerSampleEnd * 2.0,
      allPassRadiansStart: number = Math.min(
        Math.PI,
      ...
      allPassRadiansEnd: number = Math.min(
        Math.PI,
        radiansPerSampleEnd *
          Config.pickedStringDispersionFreqMult *
          (allPassCenter / radiansPerSampleEnd) ** Config.pickedStringDispersionFreqScale,
      ),
      shelfRadians: number = (2.0 * Math.PI * Config.pickedStringShelfHz) / synth.samplesPerSecond,
      decayCurveStart: number = (100.0 ** stringDecayStart - 1.0) / 99.0,
      decayCurveEnd: number = (100.0 ** stringDecayEnd - 1.0) / 99.0,
      register: number = sustainType === SustainType.acoustic ? 0.25 : 0.0,
      registerShelfCenter = 15.6,
      registerLowpassCenter: number = (3.0 * synth.samplesPerSecond) / 48_000,
      ...
      decayRateStart: number =
        0.5 **
        (decayCurveStart *
          (shelfRadians / (radiansPerSampleStart * registerShelfCenter)) ** (1.0 + 2.0 * register) *
          registerShelfCenter),
      decayRateEnd: number =
        0.5 **
        (decayCurveEnd *
          (shelfRadians / (radiansPerSampleEnd * registerShelfCenter)) ** (1.0 + 2.0 * register) *
          registerShelfCenter),
      expressionDecayStart: number = decayRateStart ** 0.002,
      expressionDecayEnd: number = decayRateEnd ** 0.002;

    Synth.tempFilterStartCoefficients.allPass1stOrderInvertPhaseAbove(allPassRadiansStart);
    synth.tempFrequencyResponse.analyze(Synth.tempFilterStartCoefficients, centerHarmonicStart);
    const allPassGStart: number = Synth.tempFilterStartCoefficients.b[0]! /* Same as a[1] */,
      allPassPhaseDelayStart: number = -synth.tempFrequencyResponse.angle() / centerHarmonicStart;

    Synth.tempFilterEndCoefficients.allPass1stOrderInvertPhaseAbove(allPassRadiansEnd);
    synth.tempFrequencyResponse.analyze(Synth.tempFilterEndCoefficients, centerHarmonicEnd);
    const allPassGEnd: number = Synth.tempFilterEndCoefficients.b[0]! /* Same as a[1] */,
      allPassPhaseDelayEnd: number = -synth.tempFrequencyResponse.angle() / centerHarmonicEnd;
    ...
    const brightnessType: PickedStringBrightnessType =
      sustainType === SustainType.bright
    ...
    if (brightnessType === PickedStringBrightnessType.bright) {
      const shelfGainStart: number = decayRateStart ** Config.stringDecayRate,
        shelfGainEnd: number = decayRateEnd ** Config.stringDecayRate;
      ...
      const cornerHardness: number =
        ...
        lowpass1stOrderCutoffRadiansStart: number =
          ((registerLowpassCenter * registerLowpassCenter * radiansPerSampleStart * 3.3 * 48_000) /
            synth.samplesPerSecond) **
            (0.5 + register) /
          registerLowpassCenter /
          decayCurveStart ** 0.5,
        lowpass1stOrderCutoffRadiansEnd: number =
          ((registerLowpassCenter * registerLowpassCenter * radiansPerSampleEnd * 3.3 * 48_000) /
            synth.samplesPerSecond) **
            (0.5 + register) /
          registerLowpassCenter /
          decayCurveEnd ** 0.5,
        lowpass2ndOrderCutoffRadiansStart: number =
          lowpass1stOrderCutoffRadiansStart *
          2.0 ** (0.5 - 1.75 * (1.0 - (1.0 - cornerHardness) ** 0.85)),
        lowpass2ndOrderCutoffRadiansEnd: number =
          lowpass1stOrderCutoffRadiansEnd *
          2.0 ** (0.5 - 1.75 * (1.0 - (1.0 - cornerHardness) ** 0.85)),
        lowpass2ndOrderGainStart: number = 2.0 ** -(2.0 ** -(cornerHardness ** 0.9)),
        lowpass2ndOrderGainEnd: number = 2.0 ** -(2.0 ** -(cornerHardness ** 0.9));
    ...
    const sustainFilterA1Start: number = Synth.tempFilterStartCoefficients.a[1]!,
      sustainFilterA2Start: number = Synth.tempFilterStartCoefficients.a[2]!,
      sustainFilterB0Start: number = Synth.tempFilterStartCoefficients.b[0]! * expressionDecayStart,
      sustainFilterB1Start: number = Synth.tempFilterStartCoefficients.b[1]! * expressionDecayStart,
      sustainFilterB2Start: number = Synth.tempFilterStartCoefficients.b[2]! * expressionDecayStart,
      sustainFilterPhaseDelayStart: number =
        -synth.tempFrequencyResponse.angle() / centerHarmonicStart;

    synth.tempFrequencyResponse.analyze(Synth.tempFilterEndCoefficients, centerHarmonicEnd);
    const sustainFilterA1End: number = Synth.tempFilterEndCoefficients.a[1]!,
      sustainFilterA2End: number = Synth.tempFilterEndCoefficients.a[2]!,
      sustainFilterB0End: number = Synth.tempFilterEndCoefficients.b[0]! * expressionDecayEnd,
      sustainFilterB1End: number = Synth.tempFilterEndCoefficients.b[1]! * expressionDecayEnd,
      sustainFilterB2End: number = Synth.tempFilterEndCoefficients.b[2]! * expressionDecayEnd,
      sustainFilterPhaseDelayEnd: number = -synth.tempFrequencyResponse.angle() / centerHarmonicEnd,
      periodLengthStart: number = 1.0 / phaseDeltaStart,
      periodLengthEnd: number = 1.0 / phaseDeltaEnd,
      minBufferLength: number = Math.ceil(Math.max(periodLengthStart, periodLengthEnd) * 2),
      delayLength: number =
        periodLengthStart - allPassPhaseDelayStart - sustainFilterPhaseDelayStart,
      delayLengthEnd: number = periodLengthEnd - allPassPhaseDelayEnd - sustainFilterPhaseDelayEnd;
    ...
    const pitchChanged: boolean = Math.abs(Math.log2(delayLength / prevDelayLength)) > 0.01,
      reinitializeImpulse: boolean = this.delayIndex === -1 || pitchChanged;
    if (this.delayLine == null || this.delayLine.length <= minBufferLength) {
      ...
      const likelyMaximumLength: number = Math.ceil(
        ...
        newDelayLine: Float32Array = new Float32Array(
      ...
      if (!reinitializeImpulse && this.delayLine != null) {
        ...
        const oldDelayBufferMask: number = (this.delayLine.length - 1) >> 0,
          startCopyingFromIndex: number = this.delayIndex + this.delayResetOffset;
    ...
    const delayLine: Float32Array = this.delayLine,
      delayBufferMask: number = (delayLine.length - 1) >> 0;
    ...
    if (reinitializeImpulse) {
      ...
      const startImpulseFrom: number = -delayLength,
        startZerosFrom: number = Math.floor(startImpulseFrom - periodLengthStart / 2),
        stopZerosAt: number = Math.ceil(startZerosFrom + periodLengthStart * 2);
      ...
      const impulseWave: Float32Array = instrumentState.wave!,
        impulseWaveLength: number = impulseWave.length - 1, // The first sample is duplicated at the end, don't double-count it.
        impulsePhaseDelta: number = impulseWaveLength / periodLengthStart,
        fadeDuration: number = Math.min(periodLengthStart * 0.2, synth.samplesPerSecond * 0.003),
        startImpulseFromSample: number = Math.ceil(startImpulseFrom),
        stopImpulseAt: number = startImpulseFrom + periodLengthStart + fadeDuration,
        stopImpulseAtSample: number = stopImpulseAt;
      ...
      for (let i: number = startImpulseFromSample; i <= stopImpulseAtSample; i++) {
        const impulsePhaseInt: number = impulsePhase | 0,
          index: number = impulsePhaseInt % impulseWaveLength;
        ...
        const sample: number = (nextWaveIntegral - prevWaveIntegral) / impulsePhaseDelta,
          fadeIn: number = Math.min(1.0, (i - startImpulseFrom) / fadeDuration),
          fadeOut: number = Math.min(1.0, (stopImpulseAt - i) / fadeDuration),
          combinedFade: number = fadeIn * fadeOut,
          curvedFade: number = combinedFade * combinedFade * (3.0 - 2.0 * combinedFade); // A cubic sigmoid from 0 to 1.
...
export class EnvelopeComputer {
  ...
  readonly #modifiedEnvelopeIndices: number[] = [];
  #modifiedEnvelopeCount = 0;
  public lowpassCutoffDecayVolumeCompensation = 1.0;

  public constructor(_perNote = true) {
    this.#perNote = _perNote;
    const length: number = this.#perNote
      ? EnvelopeComputeIndex.length
      : InstrumentAutomationIndex.length;
  ...
  public reset(): void {
    ...
    this.#noteSizeFinal = Config.noteSizeMax;
    this.prevNoteSecondsEnd = 0.0;
    this.prevNoteTicksEnd = 0.0;
    this.#prevNoteSizeFinal = Config.noteSizeMax;
    this.#modifiedEnvelopeCount = 0;
  }
  ...
  public computeEnvelopes(
    ...
    if (tone != null && tone.atNoteStart && !transition.continues && !tone.forceContinueAtStart) {
      ...
      this.#prevNoteSizeFinal = this.#noteSizeFinal;
      this.noteSecondsEnd = 0.0;
      this.noteTicksEnd = 0.0;
    }
    if (tone != null) {
      if (tone.note == null) {
        ...
        this.#noteSizeFinal = tone.note.pins[tone.note.pins.length - 1]!.size;
      }
    }

    const tickTimeEnd: number = tickTimeStart + 1.0,
      noteSecondsStart: number = this.noteSecondsEnd,
      noteSecondsEnd: number = noteSecondsStart + secondsPerTick,
      noteTicksStart: number = this.noteTicksEnd,
      noteTicksEnd: number = noteTicksStart + 1.0,
      prevNoteSecondsStart: number = this.prevNoteSecondsEnd,
      prevNoteSecondsEnd: number = prevNoteSecondsStart + secondsPerTick,
      prevNoteTicksStart: number = this.prevNoteTicksEnd,
      prevNoteTicksEnd: number = prevNoteTicksStart + 1.0,
      beatsPerTick: number = 1.0 / (Config.ticksPerPart * Config.partsPerBeat),
      beatTimeStart: number = beatsPerTick * tickTimeStart,
      beatTimeEnd: number = beatsPerTick * tickTimeEnd;

    let noteSizeStart: number = this.#noteSizeFinal,
      noteSizeEnd: number = this.#noteSizeFinal;
    const prevNoteSize: number = this.#prevNoteSizeFinal;
    let nextNoteSize = 0,
    ...
    if (tone != null && tone.note != null && !tone.passedEndOfNote) {
      const endPinIndex: number = tone.note.getEndPinIndex(currentPart),
        startPin: NotePin = tone.note.pins[endPinIndex - 1]!,
        endPin: NotePin = tone.note.pins[endPinIndex]!,
        startPinTick: number = (tone.note.start + startPin.time) * Config.ticksPerPart,
        endPinTick: number = (tone.note.start + endPin.time) * Config.ticksPerPart,
        ratioStart: number = (tickTimeStart - startPinTick) / (endPinTick - startPinTick),
        ratioEnd: number = (tickTimeEnd - startPinTick) / (endPinTick - startPinTick);
      noteSizeStart = startPin.size + (endPin.size - startPin.size) * ratioStart;
      noteSizeEnd = startPin.size + (endPin.size - startPin.size) * ratioEnd;

      if (transition.slides) {
        const noteStartTick: number = tone.noteStartPart * Config.ticksPerPart,
          noteEndTick: number = tone.noteEndPart * Config.ticksPerPart,
          noteLengthTicks: number = noteEndTick - noteStartTick,
          maximumSlideTicks: number = noteLengthTicks * 0.5,
          slideTicks: number = Math.min(maximumSlideTicks, transition.slideTicks);
    ...
    for (let envelopeIndex = 0; envelopeIndex <= instrument.envelopeCount; envelopeIndex++) {
      let modulationTarget: ModulationTarget,
        targetIndex: number,
        envelope: Envelope,
        speed: number,
        a: number,
      ...
      if (envelopeIndex === instrument.envelopeCount) {
        ...
        modulationTarget = Config.modulationTargets.dictionary["noteVolume"]!;
        targetIndex = 0;
        ...
        modulationTarget = Config.modulationTargets[envelopeSettings.target]!;
        ...
        if (envelope.type === EnvelopeType.noteSize) {
          usedNoteSize = true;
        }
      }
      if (
        (modulationTarget.perNote !== false) === this.#perNote &&
        modulationTarget.computeIndex != null
      ) {
        const computeIndex: number = modulationTarget.computeIndex + targetIndex;
        let envelopeStart: number = EnvelopeComputer.computeEnvelope(
          ...
          envelopeEnd: number = EnvelopeComputer.computeEnvelope(
            envelope,
            noteSecondsEnd,
            beatTimeEnd,
            noteSizeEnd,
        ...
        this.#modifiedEnvelopeIndices[this.#modifiedEnvelopeCount++] = computeIndex;

        if (modulationTarget.isFilter) {
          const filterSettings: FilterSettings = this.#perNote
          ...
          if (
            ...
            lowpassCutoffDecayVolumeCompensation = Math.max(
              ...
              EnvelopeComputer.getLowpassCutoffDecayVolumeCompensation(envelope, speed),
            );
          }
        }
      }
  ...
  public clearEnvelopes(): void {
    for (let envelopeIndex = 0; envelopeIndex < this.#modifiedEnvelopeCount; envelopeIndex++) {
      const computeIndex: number = this.#modifiedEnvelopeIndices[envelopeIndex]!;
  ...
  public static computeEnvelope(
    ...
    switch (envelope.type) {
      ...
      case EnvelopeType.tremolo: {
        return a + (b - a) * (0.5 - Math.cos(beats * 2.0 * Math.PI * speed) * 0.5);
      }
      ...
      case EnvelopeType.flare: {
        const attack: number = 0.25 / Math.sqrt(speed),
          flare: number = time < attack ? time / attack : 1.0 / (1.0 + (time - attack) * speed);
        return a + (b - a) * flare;
      }
      case EnvelopeType.decay: {
        return b + (a - b) * 2 ** (-speed * time);
      }
      default: {
  ...
  public static getLowpassCutoffDecayVolumeCompensation(
    ...
    if (envelope.type === EnvelopeType.decay) {
      return 1.25 + 0.025 * speed;
    }
    if (envelope.type === EnvelopeType.twang) {
      return 1.0 + 0.02 * speed;
    }
    return 1.0;
  }
}
...
class Tone {
  ...
  public readonly pitches: number[] = new Array<number>(Config.maxChordSize).fill(0);
  public pitchCount = 0;
  public chordSize = 0;
  ...
  public noiseSample = 0.0;
  public noiseSampleB = 0.0;
  ...
  public expressionDelta = 0.0;
  public readonly operatorExpressions: number[] = [];
  public readonly operatorExpressionDeltas: number[] = [];
  public readonly prevPitchExpressions: (number | null)[] = new Array<number | null>(
  ...
  public supersawDynamism = 0.0;
  public supersawDynamismDelta = 0.0;
  ...
  public supersawShape = 0.0;
  public supersawShapeDelta = 0.0;
  public supersawDelayLength = 0.0;
  public supersawDelayLengthDelta = 0.0;
  public supersawDelayLine: Float32Array | null = null;
  public supersawDelayIndex = -1;
  ...
  public soundFontPresetIndex = -1;
  public soundFontKey = 60;
  public soundFontInitialized = false;
...
class InstrumentState {
  ...
  public assetRootKey = 60;
  public noisePitchFilterMult = 1.0;
  public unison: Unison | null = null;
  public usesUnison = false;
  public chord: Chord | null = null;
  public effects = 0;
  public chipWavePitchFactor = 1.0;
  public chipWaveTempoFactor = 1.0;
  public chipWaveOffset = 0.0;
  public chipWaveLoopStart = 0.0;
  public chipWaveLoopEnd = 1.0;
  public chipWaveOneshot = false;
  ...
  public eqFilterVolume = 1.0;
  public eqFilterVolumeDelta = 0.0;
  public mixVolume = 1.0;
  public mixVolumeDelta = 0.0;
  public delayInputMult = 0.0;
  public delayInputMultDelta = 0.0;

  public distortion = 0.0;
  public distortionDelta = 0.0;
  public distortionDrive = 0.0;
  public distortionDriveDelta = 0.0;
  ...
  public panningVolumeL = 0.0;
  public panningVolumeR = 0.0;
  public panningOffsetL = 0.0;
  public panningOffsetR = 0.0;
  ...
  public chorusVoiceMult = 0;
  public chorusVoiceMultDelta = 0;
  public chorusCombinedMult = 0;
  public chorusCombinedMultDelta = 0;
  ...
  public allocateNecessaryBuffers(
    ...
    if (effectsIncludeEcho(instrument.effects)) {
      const safeEchoDelaySteps: number = Config.echoDelayRange,
        maximumSamplesPerTick: number =
          synth.samplesPerSecond /
          ((Config.tempoMin / 60) * Config.partsPerBeat * Config.ticksPerPart),
        baseEchoDelayBufferSize: number = Synth.fittingPowerOfTwo(
          safeEchoDelaySteps * Config.echoDelayStepTicks * maximumSamplesPerTick,
        ),
        safeEchoDelayBufferSize: number = baseEchoDelayBufferSize;
      ...
      if (this.echoDelayLineL == null || this.echoDelayLineR == null) {
        ...
        const newDelayLineL: Float32Array = new Float32Array(safeEchoDelayBufferSize),
          newDelayLineR: Float32Array = new Float32Array(safeEchoDelayBufferSize),
          oldMask: number = this.echoDelayLineL.length - 1;

        for (let i = 0; i < this.echoDelayLineL.length; i++) {
          newDelayLineL[i] = this.echoDelayLineL[(this.echoDelayPos + i) & oldMask]!;
          newDelayLineR[i] = this.echoDelayLineR[(this.echoDelayPos + i) & oldMask]!;
        }
  ...
  public compute(
    ...
    this.chipWavePitchFactor = Synth.chipWavePercentToFactor(instrument.chipWaveSettings.pitch);
    this.chipWaveTempoFactor = Synth.chipWavePercentToFactor(instrument.chipWaveSettings.tempo);
    ...
    for (let i = 0; i < Config.operatorCount; i++) {
      ...
      this.operatorChipWaveOffsets[i] = instrument.operators[i]!.chipWaveSettings.offset;
      this.operatorChipWaveLoopStarts[i] = instrument.operators[i]!.chipWaveSettings.loopStart;
      this.operatorChipWaveLoopEnds[i] = instrument.operators[i]!.chipWaveSettings.loopEnd;
      this.operatorChipWaveOneshots[i] = instrument.operators[i]!.chipWaveSettings.oneshot;
    }
    ...
    if (instrument.distortion === 0) {
      effects &= ~(1 << EffectType.distortion);
    }
    if (instrument.chorus === 0) {
      effects &= ~(1 << EffectType.chorus);
    }
    if (instrument.echoSustain === 0) {
      effects &= ~(1 << EffectType.echo);
    }
    if (instrument.reverb === 0) {
      effects &= ~(1 << EffectType.reverb);
    }
    this.effects = effects;
    ...
    const tickTimeStart: number = synth.getTicksIntoBar(),
      secondsPerTick: number = samplesPerTick / synth.samplesPerSecond,
      currentPart: number = synth.getCurrentPart();
    this.envelopeComputer.clearEnvelopes();
    this.envelopeComputer.computeEnvelopes(
      instrument,
      currentPart,
      tickTimeStart,
      secondsPerTick,
    ...
    const envelopeStarts: number[] = this.envelopeComputer.envelopeStarts,
      envelopeEnds: number[] = this.envelopeComputer.envelopeEnds,
      usesDistortion: boolean = effectsIncludeDistortion(effects),
      usesBitcrusher: boolean = effectsIncludeBitcrusher(effects),
      usesChorus: boolean = effectsIncludeChorus(effects),
      usesEcho: boolean = effectsIncludeEcho(effects),
      usesReverb: boolean = effectsIncludeReverb(effects);
    ...
    if (usesDistortion) {
      const distortionSliderStart: number = Math.max(
        ...
        distortionSliderEnd: number = Math.max(
          0.0,
          Math.min(
            1.0,
        ...
        distortionEnd: number = (1.0 - (0.895 * (20.0 ** distortionSliderEnd - 1.0)) / 19.0) ** 2.0,
        distortionDriveStart: number =
          (1.0 + 2.0 * distortionSliderStart) / Config.distortionBaseVolume,
        distortionDriveEnd: number =
    ...
    if (usesBitcrusher) {
      const freqSettingStart: number =
          ...
          Math.sqrt(envelopeStarts[InstrumentAutomationIndex.bitcrusherFrequency]!),
        freqSettingEnd: number =
          instrument.bitcrusherFreq *
          Math.sqrt(envelopeEnds[InstrumentAutomationIndex.bitcrusherFrequency]!),
        quantizationSettingStart: number =
          instrument.bitcrusherQuantization *
          Math.sqrt(envelopeStarts[InstrumentAutomationIndex.bitcrusherQuantization]!),
        quantizationSettingEnd: number =
          instrument.bitcrusherQuantization *
          Math.sqrt(envelopeEnds[InstrumentAutomationIndex.bitcrusherQuantization]!),
        basePitch: number = Config.keys[synth.song!.key]!.basePitch, // TODO: What if there's a key change mid-song?
        freqStart: number =
          Instrument.frequencyFromPitch(basePitch + 60) *
          2.0 **
            ((Config.bitcrusherFreqRange - 1 - freqSettingStart) * Config.bitcrusherOctaveStep),
        freqEnd: number =
          Instrument.frequencyFromPitch(basePitch + 60) *
          2.0 ** ((Config.bitcrusherFreqRange - 1 - freqSettingEnd) * Config.bitcrusherOctaveStep),
        phaseDeltaStart: number = Math.min(1.0, freqStart / samplesPerSecond),
        phaseDeltaEnd: number = Math.min(1.0, freqEnd / samplesPerSecond);
      ...
      const scaleStart: number =
          ...
          2.0 **
            (1.0 -
              2.0 ** ((Config.bitcrusherQuantizationRange - 1 - quantizationSettingStart) * 0.5)),
        scaleEnd: number =
          2.0 *
          Config.bitcrusherBaseVolume *
          2.0 **
            (1.0 -
              2.0 ** ((Config.bitcrusherQuantizationRange - 1 - quantizationSettingEnd) * 0.5));
    ...
    const eqFilterSettings: FilterSettings = instrument.eqFilter,
      eqFilterCount: number = effectsIncludeEqFilter(effects)
        ? eqFilterSettings.controlPointCount
        : 0;
    for (let i = 0; i < eqFilterCount; i++) {
      const eqFreqEnvelopeStart: number =
          envelopeStarts[InstrumentAutomationIndex.eqFilterFreq0 + i]!,
        eqFreqEnvelopeEnd: number = envelopeEnds[InstrumentAutomationIndex.eqFilterFreq0 + i]!,
        eqGainEnvelopeStart: number = envelopeStarts[InstrumentAutomationIndex.eqFilterGain0 + i]!,
        eqGainEnvelopeEnd: number = envelopeEnds[InstrumentAutomationIndex.eqFilterGain0 + i]!,
        point: FilterControlPoint = eqFilterSettings.controlPoints[i]!;
    ...
    let eqFilterVolumeStart: number = eqFilterVolume,
      eqFilterVolumeEnd: number = eqFilterVolume,
      delayInputMultStart = 1.0,
      delayInputMultEnd = 1.0;

    const panEnvelope: number = envelopeStarts[InstrumentAutomationIndex.pan]!,
      pan: number = ((instrument.pan - Config.panCenter) * panEnvelope) / Config.panCenter,
      panningVolumeL: number = Math.cos((1 + pan) * Math.PI * 0.25) * Math.SQRT2,
      panningVolumeR: number = Math.cos((1 - pan) * Math.PI * 0.25) * Math.SQRT2,
      panningDelay: number = pan * samplesPerSecond * Config.panDelaySecondsMax;
    this.panningVolumeL = panningVolumeL;
    this.panningVolumeR = panningVolumeR;
    this.panningOffsetL =
      this.panningDelayPos - Math.max(0.0, panningDelay) + synth.panningDelayBufferSize;
    this.panningOffsetR =
      this.panningDelayPos - Math.max(0.0, -panningDelay) + synth.panningDelayBufferSize;

    if (usesChorus) {
      const chorusEnvelopeStart: number = envelopeStarts[InstrumentAutomationIndex.chorus]!,
        chorusEnvelopeEnd: number = envelopeEnds[InstrumentAutomationIndex.chorus]!;
      let chorusStart: number = Math.max(
          0.0,
          Math.min(1.0, (chorusEnvelopeStart * instrument.chorus) / (Config.chorusRange - 1)),
        ),
        chorusEnd: number = Math.max(
          0.0,
          Math.min(1.0, (chorusEnvelopeEnd * instrument.chorus) / (Config.chorusRange - 1)),
        );
      ...
      const chorusCombinedMultStart = 1.0 / Math.sqrt(3.0 * chorusStart * chorusStart + 1.0),
        chorusCombinedMultEnd = 1.0 / Math.sqrt(3.0 * chorusEnd * chorusEnd + 1.0);
    ...
    let maxEchoMult = 0.0,
      averageEchoDelaySeconds = 0.0;
    if (usesEcho) {
      const echoSustainEnvelopeStart: number =
          envelopeStarts[InstrumentAutomationIndex.echoSustain]!,
        echoSustainEnvelopeEnd: number = envelopeEnds[InstrumentAutomationIndex.echoSustain]!,
        echoMultStart: number = Synth.echoSustainToVolumeMult(
          echoSustainEnvelopeStart * instrument.echoSustain,
        ),
        echoMultEnd: number = Synth.echoSustainToVolumeMult(
      ...
      const echoDelayEnvelopeEnd: number = envelopeEnds[InstrumentAutomationIndex.echoDelay]!,
        echoDelayOffset: number = Math.max(
          1,
          Math.round(
            (instrument.echoDelay * echoDelayEnvelopeEnd + 1) *
              Config.echoDelayStepTicks *
              samplesPerTick,
          ),
        );
    ...
    if (usesReverb) {
      const reverbEnvelopeStart: number = envelopeStarts[InstrumentAutomationIndex.reverb]!,
        reverbEnvelopeEnd: number = envelopeEnds[InstrumentAutomationIndex.reverb]!,
        reverbStart: number =
          Math.max(
            0.0,
            Math.min(1.0, (reverbEnvelopeStart * instrument.reverb) / Config.reverbRange),
          ) **
            0.667 *
          0.425,
        reverbEnd: number =
          Math.max(
            0.0,
            Math.min(1.0, (reverbEnvelopeEnd * instrument.reverb) / Config.reverbRange),
          ) **
            0.667 *
          0.425;
    ...
    if (this.tonesAddedInThisTick) {
      ...
      const attenuationThreshold: number = 1.0 / 256.0, // when the delay line signal has attenuated this much, it should be inaudible and should be flushed to zero.
        halfLifeMult: number = -Math.log2(attenuationThreshold);
      ...
      if (usesEcho) {
        if (maxEchoMult >= 1.0) {
          ...
          const attenuationPerSecond: number = maxEchoMult ** (1.0 / averageEchoDelaySeconds),
            halfLife: number = -1.0 / Math.log2(attenuationPerSecond),
            echoDuration: number = halfLife * halfLifeMult;
      ...
      if (usesReverb) {
        const averageMult: number = maxReverbMult * 2.0,
          averageReverbDelaySeconds: number = Config.reverbDelayBufferSize / 4.0 / samplesPerSecond,
          attenuationPerSecond: number = averageMult ** (1.0 / averageReverbDelaySeconds),
          halfLife: number = -1.0 / Math.log2(attenuationPerSecond),
          reverbDuration: number = halfLife * halfLifeMult;
        delayDuration += reverbDuration;
      }

      const secondsInTick: number = samplesPerTick / samplesPerSecond,
        progressInTick: number = secondsInTick / delayDuration,
        progressAtEndOfTick: number = this.attentuationProgress + progressInTick;
    ...
    if (instrument.type === InstrumentType.chip) {
      ...
      for (let i = 0; i < Config.drumCount; i++) {
        this.drumsetSpectrumWaves[i]!.getCustomWave(
          ...
          InstrumentState.#drumsetIndexToSpectrumOctave(i),
        );
      }
      this.wave = null;
    } else {
  ...
  public static drumsetIndexReferenceDelta(index: number): number {
    return Instrument.frequencyFromPitch(Config.spectrumBasePitch + index * 6) / 44_100;
  }

  static #drumsetIndexToSpectrumOctave(index: number): number {
...
export class AutomationRuntime {
  #song: Song | null = null;
  readonly #latchedValues: (number | null)[][] = [];
  readonly #activeValues: boolean[][] = [];
  readonly #muted: boolean[] = [];
  readonly #effectiveInstruments: (Instrument | null)[][] = [];
  #effectiveTempo: number = Config.tempoMin;
  #needsReconstruction = true;

  public reset(song: Song | null): void {
    this.#song = song;
    this.#latchedValues.length = 0;
    this.#activeValues.length = 0;
    this.#muted.length = 0;
    this.#effectiveInstruments.length = 0;
    this.#effectiveTempo = song?.tempo ?? Config.tempoMin;
    this.#needsReconstruction = true;
  }
  ...
  public getEffectiveInstrument(
    ...
    return (
      this.#effectiveInstruments[channelIndex]?.[instrumentIndex] ??
      song.channels[channelIndex]!.instruments[instrumentIndex]!
    );
  }

  public getLatchedOperand(automationChannelIndex: number, rowIndex: number): number | null {
    return this.#latchedValues[automationChannelIndex]?.[rowIndex] ?? null;
  }
  ...
  #findValueInPattern(
    ...
    const events: Event[] | undefined = pattern?.automationEvents[rowIndex];
    if (events === undefined) {
      return { found: false, active: false, value: 0 };
    }
    let found = false,
      value = 0;
  ...
  #reconstructRow(
    ...
    const current = this.#findValueInPattern(song.getPattern(channelIndex, bar), rowIndex, part);
    if (current.found) {
      return {
    ...
    for (let previousBar: number = bar - 1; previousBar >= 0; previousBar--) {
      const events: Event[] | undefined = song.getPattern(channelIndex, previousBar)
        ?.automationEvents[rowIndex];
      if (events !== undefined && events.length > 0) {
        const value: number = events.at(-1)!.getFinalValue();
  ...
  #getInstrumentTargetValue(
    ...
    switch (property) {
      ...
      case "pan": {
        return instrument.pan;
      }
      case "noteFilterFrequency": {
        return instrument.noteFilter.controlPoints[index]?.freq ?? null;
      }
      case "noteFilterGain": {
        return instrument.noteFilter.controlPoints[index]?.gain ?? null;
      }
      case "eqFilterFrequency": {
        return instrument.eqFilter.controlPoints[index]?.freq ?? null;
      }
      case "eqFilterGain": {
        return instrument.eqFilter.controlPoints[index]?.gain ?? null;
      }
      case "distortion": {
        return instrument.distortion;
      }
      case "chorus": {
        return instrument.chorus;
      }
      case "reverb": {
        return instrument.reverb;
      }
      case "echoSustain": {
        return instrument.echoSustain;
      }
      case "echoDelay": {
        return instrument.echoDelay;
      }
      case "bitcrusherFrequency": {
        return instrument.bitcrusherFreq;
      }
      case "bitcrusherQuantization": {
        return instrument.bitcrusherQuantization;
      }
      case "pitchShift": {
        return instrument.pitchShift;
      }
      case "detune": {
        return instrument.detune;
      }
      case "vibrato": {
        return instrument.vibrato;
      }
      case "pulseWidth": {
        return instrument.pulseWidth;
      }
      case "stringSustain": {
        return instrument.stringSustain;
      }
      case "operatorFrequency": {
        return instrument.operators[index]?.frequency ?? null;
      }
      case "operatorAmplitude": {
        return instrument.operators[index]?.amplitude ?? null;
      }
      case "feedbackAmplitude": {
        return instrument.feedbackAmplitude;
      }
      case "supersawDynamism": {
        return instrument.supersawDynamism;
      }
      case "supersawSpread": {
        return instrument.supersawSpread;
      }
      case "supersawShape": {
        return instrument.supersawShape;
      }
      case "tempo": {
        return null;
      }
  ...
  #cloneFilter(settings: FilterSettings): FilterSettings {
    const clone: FilterSettings = Object.create(settings) as FilterSettings,
      points: FilterControlPoint[] = [];
    for (let index = 0; index < settings.controlPointCount; index++) {
  ...
  #setInstrumentTargetValue(
    ...
    switch (property) {
      ...
      case "distortion": {
        effective.distortion = value;
        return;
      }
      case "chorus": {
        effective.chorus = value;
        return;
      }
      case "reverb": {
        effective.reverb = value;
        return;
      }
      case "echoSustain": {
        effective.echoSustain = value;
        return;
      }
      case "echoDelay": {
        effective.echoDelay = value;
        return;
      }
      case "bitcrusherFrequency": {
        effective.bitcrusherFreq = value;
        return;
      }
      case "bitcrusherQuantization": {
        effective.bitcrusherQuantization = value;
        return;
      }
      case "pitchShift": {
        effective.pitchShift = value;
        return;
      }
      case "detune": {
        effective.detune = value;
        return;
      }
      case "vibrato": {
        effective.vibrato = value;
        return;
      }
      case "pulseWidth": {
        effective.pulseWidth = value;
        return;
      }
      case "stringSustain": {
        effective.stringSustain = value;
        return;
      }
      case "feedbackAmplitude": {
        effective.feedbackAmplitude = value;
        return;
      }
      case "supersawDynamism": {
        effective.supersawDynamism = value;
        return;
      }
      case "supersawSpread": {
        effective.supersawSpread = value;
        return;
      }
      case "supersawShape": {
        effective.supersawShape = value;
        return;
      }
      case "noteFilterFrequency":
      case "noteFilterGain": {
        if (effective.noteFilter === base.noteFilter) {
          effective.noteFilter = this.#cloneFilter(base.noteFilter);
        }
        const point: FilterControlPoint | undefined = effective.noteFilter.controlPoints[index]!;
        if (point !== undefined) {
          if (property === "noteFilterFrequency") {
            point.freq = value;
          } else {
            point.gain = value;
          }
      ...
      case "eqFilterGain": {
        if (effective.eqFilter === base.eqFilter) {
          effective.eqFilter = this.#cloneFilter(base.eqFilter);
        }
        const point: FilterControlPoint | undefined = effective.eqFilter.controlPoints[index]!;
        if (point !== undefined) {
          if (property === "eqFilterFrequency") {
            point.freq = value;
          } else {
            point.gain = value;
          }
      ...
      case "operatorAmplitude": {
        if (effective.operators === base.operators) {
          (effective as { operators: Operator[] }).operators = base.operators.map(
  ...
  public update(song: Song, bar: number, part: number, continuous: boolean): void {
    if (this.#song !== song) {
      this.reset(song);
    }
    const reconstructAll: boolean = this.#needsReconstruction || !continuous,
      firstAutomation: number = song.pitchChannelCount + song.noiseChannelCount;
    for (let channelIndex = 0; channelIndex < song.getChannelCount(); channelIndex++) {
      const channel: Channel = song.channels[channelIndex]!;
      while (this.#effectiveInstruments.length <= channelIndex) {
        this.#effectiveInstruments.push([]);
      }
      const instruments: (Instrument | null)[] = this.#effectiveInstruments[channelIndex]!;
      instruments.length = channel.instruments.length;
      instruments.fill(null);
      if (channelIndex < firstAutomation) {
        continue;
      }
      while (this.#latchedValues.length <= channelIndex) {
        this.#latchedValues.push([]);
      }
      while (this.#activeValues.length <= channelIndex) {
        this.#activeValues.push([]);
      }
      const latches: (number | null)[] = this.#latchedValues[channelIndex]!,
        activeValues: boolean[] = this.#activeValues[channelIndex]!,
        wasMuted: boolean = this.#muted[channelIndex] === true,
        reconstructChannel: boolean = reconstructAll || (wasMuted && !channel.muted);
      ...
      for (let rowIndex = 0; rowIndex < channel.automationRows.length; rowIndex++) {
        if (reconstructChannel || latches[rowIndex] === undefined) {
          const reconstructed = this.#reconstructRow(song, channelIndex, rowIndex, bar, part);
          ...
          const current = this.#findValueInPattern(
      ...
      this.#muted[channelIndex] = channel.muted;
    }

    const resolvedValues = new Map<number, ResolvedAutomationValue>(),
      targetStride: number = Config.automationTargetIndexMax + 1;
    let tempoValue: number = song.tempo;
    const tempoTarget: ModulationTarget = Config.modulationTargets.dictionary["tempo"]!,
      tempoDomain: AutomationValueDomain = {
    ...
    for (const active of [false, true]) {
      for (
        ...
        for (let rowIndex = 0; rowIndex < channel.automationRows.length; rowIndex++) {
          if (this.#activeValues[channelIndex]?.[rowIndex] !== active) {
            continue;
          }
          const row: AutomationRow = channel.automationRows[rowIndex]!,
            operand: number | null = this.#latchedValues[channelIndex]?.[rowIndex] ?? null;
          if (operand == null || !row.isTargetValid(song)) {
            continue;
          }
          const target: ModulationTarget = row.getTarget()!;
          ...
          const base: Instrument =
            ...
            property: ModulationProperty | undefined = target.property;
          ...
          const address: number =
            ((row.targetChannel * Config.instrumentCountMax + row.targetInstrument) *
              ...
              targetStride +
            row.targetIndex;
          let resolved: ResolvedAutomationValue | undefined = resolvedValues.get(address);
          if (resolved === undefined) {
            const baseValue: number | null = this.#getInstrumentTargetValue(
              base,
              property,
              row.targetIndex,
            );
    ...
    this.#effectiveTempo = sanitizeAutomationValue(tempoValue, tempoDomain);
    for (const resolved of resolvedValues.values()) {
      ...
      let effective: Instrument | null =
        this.#effectiveInstruments[resolved.channelIndex]![resolved.instrumentIndex]!;
      if (effective == null) {
        effective = Object.create(base) as Instrument;
        this.#effectiveInstruments[resolved.channelIndex]![resolved.instrumentIndex] = effective;
      }
...
export class Synth {
  ...
  private warmUpSynthesizer(song: Song | null): void {
    if (song != null) {
      ...
      for (let j = 0; j < song.getChannelCount(); j++) {
        if (song.getChannelIsAutomation(j)) {
          continue;
        }
        for (let i = 0; i < song.channels[j]!.instruments.length; i++) {
          const instrument: Instrument = this.automationRuntime.getEffectiveInstrument(song, j, i),
            instrumentState: InstrumentState = this.channels[j]!.instruments[i]!;
  ...
  public samplesPerSecond = 44_100;
  ...
  public preferLowerLatency = false; // Enable when recording performances from keyboard or MIDI. Takes effect next time you activate audio.
  public anticipatePoorPerformance = false; // Enable on mobile devices to reduce audio stutter glitches. Takes effect next time you activate audio.
  ...
  public songEnded = false;
  public lastSynthesizeSampleCount = 0;
  public enableMetronome = false;
  public countInMetronome = false;
  public readonly automationRuntime: AutomationRuntime = new AutomationRuntime();
  ...
  private pendingSampleSeek = false;
  ...
  private static readonly effectsFunctionCache: (Function | undefined)[] = new Array<
    Function | undefined
  >(1 << 7); // Keep in sync with the number of post-process effects.
  private static readonly pickedStringFunctionCache: (Function | undefined)[] = new Array<
    Function | undefined
  >(3); // Keep in sync with the number of unison voices.

  private readonly channels: (ChannelState | null)[] = [];
  private readonly tonePool: Deque<Tone> = new Deque<Tone>();
  private readonly tempMatchedPitchTones: (Tone | null)[] = new Array<Tone | null>(
    Config.maxChordSize,
  ).fill(null);
  private readonly assetData = new Map<string, AssetData>();
  ...
  private readonly soundFontBanks = new Map<string, SoundFontBank>();
  ...
  public getSamplesPerBar(): number {
    ...
    return (
      this.getSamplesPerTick() * Config.ticksPerPart * Config.partsPerBeat * this.song.beatsPerBar
    );
  }
  ...
  public setSong(song: Song | Uint8Array): void {
    ...
    for (const sampleId of this.assetData.keys()) {
      if (!activeSampleIds.has(sampleId)) {
        this.assetData.delete(sampleId);
      }
    }
  ...
  public setAsset(sampleId: string, samples: Float32Array, sampleRate: number): void {
          ...
          instrument.type === InstrumentType.chip
            ...
            : instrument.type === InstrumentType.fm &&
              instrument.operators.some(
                (operator) =>
                  operator.wave > 0 && Config.chipWaves[operator.wave - 1]?.sampleId != null,
              ),
        ),
      )
    ) {
  ...
  public synthesize(
    ...
    const limitDecay: number = 1.0 - 0.5 ** (4.0 / this.samplesPerSecond),
      limitRise: number = 1.0 - 0.5 ** (4000.0 / this.samplesPerSecond);
    ...
    while (bufferIndex < outputBufferLength && !ended) {
      ...
      const samplesLeftInBuffer: number = outputBufferLength - bufferIndex,
        samplesLeftInTick: number = Math.ceil(this.tickSampleCountdown),
        runLength: number = Math.min(samplesLeftInTick, samplesLeftInBuffer),
        runEnd: number = bufferIndex + runLength;
      for (let channelIndex = 0; channelIndex < song.getChannelCount(); channelIndex++) {
        if (song.getChannelIsAutomation(channelIndex)) {
          continue;
        }
        const channel: Channel = song.channels[channelIndex]!,
          channelState: ChannelState = this.channels[channelIndex]!;
        ...
        for (
          ...
          const instrument: Instrument = this.automationRuntime.getEffectiveInstrument(
              song,
              channelIndex,
              instrumentIndex,
            ),
            instrumentState: InstrumentState = channelState.instruments[instrumentIndex]!;
        ...
        if (this.part === 0) {
          if (!this.startedMetronome) {
            const midBeat: boolean =
              ...
              periods: number = this.beat === 0 ? 8 : midBeat ? 6 : 4,
              hz: number = this.beat === 0 ? 1600 : midBeat ? 1200 : 800,
              amplitude: number = this.beat === 0 ? 0.06 : midBeat ? 0.05 : 0.04,
              samplesPerPeriod: number = this.samplesPerSecond / hz,
              radiansPerSample: number = (Math.PI * 2.0) / samplesPerPeriod;
      ...
      for (let i: number = bufferIndex; i < runEnd; i++) {
        ...
        const sampleL = outputDataL[i]!,
          sampleR = outputDataR[i]!,
          abs: number = Math.max(Math.abs(sampleL), Math.abs(sampleR));
        limit += (abs - limit) * (limit < abs ? limitRise : limitDecay * (1.0 + limit));
        const limitedVolume = 1.0 / (limit >= 1 ? limit * 1.05 : limit * 0.8 + 0.25);
  ...
  private newTone(): Tone {
    const tone: Tone = this.tonePool.count() > 0 ? this.tonePool.popBack() : new Tone();
  ...
  private determineLiveInputTones(song: Song, channelIndex: number, samplesPerTick: number): void {
    ...
    const channel: Channel = song.channels[channelIndex]!,
      channelState: ChannelState = this.channels[channelIndex]!,
      pitches: number[] = this.liveInputPitches;
    ...
    for (let instrumentIndex = 0; instrumentIndex < channel.instruments.length; instrumentIndex++) {
      const instrumentState: InstrumentState = channelState.instruments[instrumentIndex]!,
        toneList: Deque<Tone> = instrumentState.liveInputTones;
      let toneCount = 0;
      if (
  ...
  public static adjacentNotesHaveMatchingPitches(firstNote: Note, secondNote: Note): boolean {
    ...
    const firstNoteInterval: number = firstNote.pins.at(-1)!.interval;
  ...
  private moveTonesIntoOrderedTempMatchedList(toneList: Deque<Tone>, notePitches: number[]): void {
    ...
    for (let i = 0; i < toneList.count(); i++) {
      const tone: Tone = toneList.get(i),
        pitch: number = tone.pitches[0]! + tone.lastInterval;
  ...
  private determineCurrentActiveTones(
    ...
    const channel: Channel = song.channels[channelIndex]!,
      channelState: ChannelState = this.channels[channelIndex]!,
      pattern: Pattern | null = song.getPattern(channelIndex, this.bar),
      currentPart: number = this.getCurrentPart(),
      currentTick: number = this.tick + Config.ticksPerPart * currentPart;
    ...
    for (let instrumentIndex = 0; instrumentIndex < channel.instruments.length; instrumentIndex++) {
      const instrumentState: InstrumentState = channelState.instruments[instrumentIndex]!,
        toneList: Deque<Tone> = instrumentState.activeTones;
      let toneCount = 0;
      if (note != null) {
        const instrument: Instrument = this.automationRuntime.getEffectiveInstrument(
          song,
        ...
        let prevNoteForThisInstrument: Note | null = prevNote,
          nextNoteForThisInstrument: Note | null = nextNote;

        const partsPerBar: number = Config.partsPerBeat * song.beatsPerBar,
          transition: Transition = instrument.getTransition(),
          chord: Chord = instrument.getChord();
        ...
        if (note.start === 0) {
          ...
          if (prevPattern != null) {
            const lastNote: Note | null =
              prevPattern.notes.length <= 0 ? null : prevPattern.notes.at(-1)!;
            if (lastNote != null && lastNote.end === partsPerBar) {
              const patternForcesContinueAtStart: boolean =
                ...
                chordOfCompatibleInstrument: Chord | null =
        ...
        if (chord.singleTone) {
          ...
          if (toneList.count() <= toneCount) {
            ...
            atNoteStart &&
            ((!transition.isSeamless && !forceContinueAtStart) || prevNoteForThisInstrument == null)
          ) {
          ...
          const noteTransition: Transition = instrument.getTransition();

          if (
            ((noteTransition.isSeamless && !noteTransition.slides && chord.strumParts === 0) ||
          ...
          for (let i = 0; i < note.pitches.length; i++) {
            let prevNoteForThisTone: Note | null =
              ...
              noteForThisTone: Note = note,
              nextNoteForThisTone: Note | null =
                tonesInNextNote > i ? nextNoteForThisInstrument : null,
              noteStartPart: number = noteForThisTone.start + strumOffsetParts,
            ...
            if (noteStartPart > currentPart) {
              if (
                toneList.count() > i &&
                (noteTransition.isSeamless || forceContinueAtStart) &&
            ...
            if (
              (!noteTransition.continues && !forceContinueAtStart) ||
              prevNoteForThisTone == null
            ) {
            ...
            if (this.tempMatchedPitchTones[toneCount]! != null) {
              ...
              atNoteStart &&
              ((!noteTransition.isSeamless && !forceContinueAtStart) || prevNoteForThisTone == null)
            ) {
              const oldTone: Tone = toneList.get(toneCount);
              if (oldTone.isOnLastTick) {
                this.freeTone(oldTone);
      ...
      while (toneList.count() > toneCount) {
        const tone: Tone = toneList.popBack(),
          currentChannel: Channel = song.channels[channelIndex]!;
        if (tone.instrumentIndex < currentChannel.instruments.length && !tone.isOnLastTick) {
          const oldInstrumentState: InstrumentState =
            channelState.instruments[tone.instrumentIndex]!;
          this.releaseTone(oldInstrumentState, tone);
        } else {
          this.freeTone(tone);
        }
      }
  ...
  private playTone(channelIndex: number, bufferIndex: number, runLength: number, tone: Tone): void {
    const channelState: ChannelState = this.channels[channelIndex]!,
      instrumentState: InstrumentState = channelState.instruments[tone.instrumentIndex]!;
  ...
  private computeTone(
    ...
    const roundedSamplesPerTick: number = Math.ceil(samplesPerTick),
      channelState: ChannelState = this.channels[channelIndex]!,
      instrument: Instrument = this.automationRuntime.getEffectiveInstrument(
        song,
        channelIndex,
    ...
    const isNoiseChannel: boolean = song.getChannelIsNoise(channelIndex),
      transition: Transition = instrument.getTransition(),
      chord: Chord = instrument.getChord(),
      chordExpression: number = chord.singleTone
        ? 1.0
        : Synth.computeChordExpression(tone.chordSize),
      intervalScale: number = isNoiseChannel ? Config.noiseInterval : 1,
      secondsPerPart: number = (Config.ticksPerPart * samplesPerTick) / this.samplesPerSecond,
      sampleTime: number = 1.0 / this.samplesPerSecond,
      beatsPerPart: number = 1.0 / Config.partsPerBeat,
      ticksIntoBar: number = this.getTicksIntoBar(),
      partTimeStart: number = ticksIntoBar / Config.ticksPerPart,
      partTimeEnd: number = (ticksIntoBar + 1.0) / Config.ticksPerPart,
      currentPart: number = this.getCurrentPart();
    ...
    let toneIsOnLastTick: boolean = shouldFadeOutFast,
      ...
      chordExpressionStart: number = chordExpression,
      chordExpressionEnd: number = chordExpression,
      expressionReferencePitch = 16, // A low "E" as a MIDI pitch.
    ...
    if (released) {
      const startTicksSinceReleased: number = tone.ticksSinceReleased,
        endTicksSinceReleased: number = tone.ticksSinceReleased + 1.0;
      ...
      const note: Note = tone.note,
        nextNote: Note | null = tone.nextNote,
        noteStartPart: number = tone.noteStartPart,
        noteEndPart: number = tone.noteEndPart,
        endPinIndex: number = note.getEndPinIndex(currentPart),
        startPin: NotePin = note.pins[endPinIndex - 1]!,
        endPin: NotePin = note.pins[endPinIndex]!,
        noteStartTick: number = noteStartPart * Config.ticksPerPart,
        noteEndTick: number = noteEndPart * Config.ticksPerPart,
        pinStart: number = (note.start + startPin.time) * Config.ticksPerPart,
        pinEnd: number = (note.start + endPin.time) * Config.ticksPerPart;

      tone.ticksSinceReleased = 0;

      const tickTimeStart: number = currentPart * Config.ticksPerPart + this.tick,
        tickTimeEnd: number = tickTimeStart + 1.0,
        noteTicksPassedTickStart: number = tickTimeStart - noteStartTick,
        noteTicksPassedTickEnd: number = tickTimeEnd - noteStartTick,
        pinRatioStart: number = Math.min(1.0, (tickTimeStart - pinStart) / (pinEnd - pinStart)),
        pinRatioEnd: number = Math.min(1.0, (tickTimeEnd - pinStart) / (pinEnd - pinStart));
    ...
    const envelopeStarts: number[] = tone.envelopeComputer.envelopeStarts,
      envelopeEnds: number[] = tone.envelopeComputer.envelopeEnds;
    ...
    if (tone.note != null && transition.slides) {
      ...
      if (nextNote != null) {
        const intervalDiff: number =
          ...
          (tone.pitches[0]! + tone.note.pins.at(-1)!.interval);
    ...
    if (effectsIncludePitchShift(instrument.effects)) {
      const pitchShift: number =
          interpolateSetting(Config.justIntonationSemitones, instrument.pitchShift) / intervalScale,
        envelopeStart: number = envelopeStarts[EnvelopeComputeIndex.pitchShift]!,
        envelopeEnd: number = envelopeEnds[EnvelopeComputeIndex.pitchShift]!;
    ...
    if (effectsIncludeDetune(instrument.effects)) {
      const envelopeStart: number = envelopeStarts[EnvelopeComputeIndex.detune]!,
        envelopeEnd: number = envelopeEnds[EnvelopeComputeIndex.detune]!;
      intervalStart +=
        (Synth.detuneToCents((instrument.detune - Config.detuneCenter) * envelopeStart) *
          Config.pitchesPerOctave) /
        (12.0 * 100.0);
      intervalEnd +=
        (Synth.detuneToCents((instrument.detune - Config.detuneCenter) * envelopeEnd) *
          Config.pitchesPerOctave) /
        (12.0 * 100.0);
    }
    ...
    if (effectsIncludeVibrato(instrument.effects)) {
      const delayTicks: number = Config.vibratos[instrument.vibrato]!.delayTicks,
        vibratoAmplitude: number = Config.vibratos[instrument.vibrato]!.amplitude;
      ...
      if (tone.prevVibrato == null) {
        const lfoStart: number = Synth.getLFOAmplitude(instrument, secondsPerPart * partTimeStart),
          vibratoDepthEnvelopeStart: number = envelopeStarts[EnvelopeComputeIndex.vibratoDepth]!;
      ...
      const lfoEnd: number = Synth.getLFOAmplitude(instrument, secondsPerPart * partTimeEnd),
        vibratoDepthEnvelopeEnd: number = envelopeEnds[EnvelopeComputeIndex.vibratoDepth]!;
    ...
    if (effectsIncludeNoteFilter(instrument.effects)) {
      const noteFilterSettings: FilterSettings = instrument.noteFilter,
        noteAllFreqsEnvelopeStart: number =
          envelopeStarts[EnvelopeComputeIndex.noteFilterAllFreqs]!,
        noteAllFreqsEnvelopeEnd: number = envelopeEnds[EnvelopeComputeIndex.noteFilterAllFreqs]!;
      for (let i = 0; i < noteFilterSettings.controlPointCount; i++) {
        const noteFreqEnvelopeStart: number =
          ...
          noteFreqEnvelopeEnd: number = envelopeEnds[EnvelopeComputeIndex.noteFilterFreq0 + i]!,
          notePeakEnvelopeStart: number = envelopeStarts[EnvelopeComputeIndex.noteFilterGain0 + i]!,
          notePeakEnvelopeEnd: number = envelopeEnds[EnvelopeComputeIndex.noteFilterGain0 + i]!,
          point: FilterControlPoint = noteFilterSettings.controlPoints[i]!;
        point.toCoefficients(
          Synth.tempFilterStartCoefficients,
    ...
    if (instrument.type === InstrumentType.drumset) {
      const drumsetFilterEnvelope: Envelope = instrument.getDrumsetEnvelope(tone.drumsetPitch!),
        drumIndex: number = tone.drumsetPitch!,
        speed: number = instrument.drumsetEnvelopeSpeeds[drumIndex]!,
        a: number = instrument.drumsetEnvelopeAs[drumIndex]!,
        b: number = instrument.drumsetEnvelopeBs[drumIndex]!;
      // If the drumset lowpass cutoff decays, compensate by increasing expression.
      noteFilterExpression *= EnvelopeComputer.getLowpassCutoffDecayVolumeCompensation(
      ...
      let drumsetFilterEnvelopeStart: number = EnvelopeComputer.computeEnvelope(
        ...
        drumsetFilterEnvelopeEnd: number = EnvelopeComputer.computeEnvelope(
          drumsetFilterEnvelope,
      ...
      if (envelopeComputer.prevSlideEnd) {
        ...
        drumsetFilterEnvelopeEnd +=
          (other - drumsetFilterEnvelopeEnd) * envelopeComputer.prevSlideRatioEnd;
      }
      if (envelopeComputer.nextSlideStart) {
        const other: number = EnvelopeComputer.computeEnvelope(
          drumsetFilterEnvelope,
      ...
      if (envelopeComputer.nextSlideEnd) {
        ...
        drumsetFilterEnvelopeEnd +=
          (other - drumsetFilterEnvelopeEnd) * envelopeComputer.nextSlideRatioEnd;
      }
    ...
    if (instrument.type === InstrumentType.fm) {
      ...
      const usesUnison: boolean = effectsIncludeUnison(instrument.effects),
        unison: Unison = instrument.getUnison(),
        unisonEnvelopeStart: number = envelopeStarts[EnvelopeComputeIndex.unison]!,
        unisonEnvelopeEnd: number = envelopeEnds[EnvelopeComputeIndex.unison]!,
        unisonAStart: number =
          2.0 ** (((unison.offset + unison.spread) * unisonEnvelopeStart) / 12.0),
        unisonAEnd: number = 2.0 ** (((unison.offset + unison.spread) * unisonEnvelopeEnd) / 12.0),
        unisonBStart: number =
          2.0 ** (((unison.offset - unison.spread) * unisonEnvelopeStart) / 12.0),
        unisonBEnd: number = 2.0 ** (((unison.offset - unison.spread) * unisonEnvelopeEnd) / 12.0);
      ...
      if (tone.pitchCount > 1 && arpeggiates) {
        ...
        arpeggioInterval =
          tone.pitches[getArpeggioPitchIndex(tone.pitchCount, song.rhythm, arpeggio)]! -
          tone.pitches[0]!;
      }
      ...
      for (let i = 0; i < Config.operatorCount; i++) {
        const associatedCarrierIndex: number =
          ...
          freqMult = instrument.operators[i]!.frequency,
          interval = Config.operatorCarrierInterval[associatedCarrierIndex]! + arpeggioInterval,
          pitchStart: number = basePitch + (pitch + intervalStart) * intervalScale + interval,
          pitchEnd: number = basePitch + (pitch + intervalEnd) * intervalScale + interval,
          baseFreqStart: number = Instrument.frequencyFromPitch(pitchStart),
          baseFreqEnd: number = Instrument.frequencyFromPitch(pitchEnd),
          targetFreqStart: number = freqMult * baseFreqStart,
          targetFreqEnd: number = freqMult * baseFreqEnd,
          freqEnvelopeStart: number = envelopeStarts[EnvelopeComputeIndex.operatorFrequency0 + i]!,
          freqEnvelopeEnd: number = envelopeEnds[EnvelopeComputeIndex.operatorFrequency0 + i]!;
        let freqStart: number, freqEnd: number;
        if (freqMult === 0) {
          freqStart = 0;
          freqEnd = 0;
        } else if (freqEnvelopeStart !== 1.0 || freqEnvelopeEnd !== 1.0) {
          freqStart =
            2.0 ** (Math.log2(targetFreqStart / baseFreqStart) * freqEnvelopeStart) * baseFreqStart;
        ...
        const phaseDelta: number = freqStart * sampleTime,
          phaseDeltaScale: number =
            freqStart === 0 ? 1 : (freqEnd / freqStart) ** (1.0 / roundedSamplesPerTick);
        if (usesUnison) {
          tone.phaseDeltas[i] = phaseDelta * unisonAStart;
          tone.phaseDeltaScales[i] =
            phaseDeltaScale * (unisonAEnd / unisonAStart) ** (1.0 / roundedSamplesPerTick);
          tone.phaseDeltas[Config.operatorCount + i] = phaseDelta * unisonBStart;
          tone.phaseDeltaScales[Config.operatorCount + i] =
            phaseDeltaScale * (unisonBEnd / unisonBStart) ** (1.0 / roundedSamplesPerTick);
        } else {
        ...
        const amplitudeCurve: number = Synth.operatorAmplitudeCurve(
          ...
          amplitudeMult: number = amplitudeCurve;
        let expressionStart: number = amplitudeMult,
          expressionEnd: number = amplitudeMult;
        if (i < carrierCount) {
          // Carrier
      ...
      const unisonExpression: number = usesUnison ? (unison.expression * unison.voices) / 2.0 : 1.0,
      ...
      const feedbackAmplitude: number =
        ...
        feedbackStart: number =
          feedbackAmplitude * envelopeStarts[EnvelopeComputeIndex.feedbackAmplitude]!,
        feedbackEnd: number =
          feedbackAmplitude * envelopeEnds[EnvelopeComputeIndex.feedbackAmplitude]!;
      tone.feedbackMult = feedbackStart;
      tone.feedbackDelta = (feedbackEnd - feedbackStart) / roundedSamplesPerTick;
    } else {
      const freqEndRatio: number = 2.0 ** (((intervalEnd - intervalStart) * intervalScale) / 12.0),
        basePhaseDeltaScale: number = freqEndRatio ** (1.0 / roundedSamplesPerTick);
      ...
      if (tone.pitchCount > 1 && (chord.arpeggiates || chord.customInterval)) {
        ...
        if (chord.customInterval) {
          const intervalOffset: number =
            tone.pitches[1 + getArpeggioPitchIndex(tone.pitchCount - 1, song.rhythm, arpeggio)]! -
            tone.pitches[0]!;
      ...
      const startPitch: number = basePitch + (pitch + intervalStart) * intervalScale,
        endPitch: number = basePitch + (pitch + intervalEnd) * intervalScale;
      let pitchExpressionStart: number;
      ...
      if (instrument.type === InstrumentType.pwm) {
        const basePulseWidth: number = getPulseWidthRatio(instrument.pulseWidth),
          pulseWidthStart: number =
            basePulseWidth * envelopeStarts[EnvelopeComputeIndex.pulseWidth]!,
          pulseWidthEnd: number = basePulseWidth * envelopeEnds[EnvelopeComputeIndex.pulseWidth]!;
      ...
      if (instrument.type === InstrumentType.pickedString) {
        ...
        settingsExpressionMult *=
          2.0 ** (0.7 * (1.0 - instrument.stringSustain / (Config.stringSustainRange - 1)));
      }
      ...
      if (
        ...
        const unisonEnvelopeStart = envelopeStarts[EnvelopeComputeIndex.unison]!,
          unisonEnvelopeEnd = envelopeEnds[EnvelopeComputeIndex.unison]!,
          unisonAStart: number =
            2.0 ** (((unison.offset + unison.spread) * unisonEnvelopeStart) / 12.0),
          unisonAEnd: number =
            2.0 ** (((unison.offset + unison.spread) * unisonEnvelopeEnd) / 12.0),
          unisonBStart: number =
            2.0 ** (((unison.offset - unison.spread) * unisonEnvelopeStart) / 12.0) *
            specialIntervalMult,
          unisonBEnd: number =
        ...
        tone.phaseDeltaScales[0] =
          basePhaseDeltaScale * (unisonAEnd / unisonAStart) ** (1.0 / roundedSamplesPerTick);
      ...
      let supersawExpressionStart = 1.0,
        supersawExpressionEnd = 1.0;
      if (instrument.type === InstrumentType.supersaw) {
        const minFirstVoiceAmplitude: number = 1.0 / Math.sqrt(Config.supersawVoiceCount),
          baseDynamismSlider: number = instrument.supersawDynamism / Config.supersawDynamismMax,
          curvedDynamismStart: number =
            1.0 -
            Math.max(
              0.0,
              1.0 - baseDynamismSlider * envelopeStarts[EnvelopeComputeIndex.supersawDynamism]!,
            ) **
              0.2,
          curvedDynamismEnd: number =
            1.0 -
            Math.max(
              0.0,
              1.0 - baseDynamismSlider * envelopeEnds[EnvelopeComputeIndex.supersawDynamism]!,
            ) **
              0.2,
          firstVoiceAmplitudeStart: number =
            2.0 ** (Math.log2(minFirstVoiceAmplitude) * curvedDynamismStart),
          firstVoiceAmplitudeEnd: number =
            2.0 ** (Math.log2(minFirstVoiceAmplitude) * curvedDynamismEnd),
          dynamismStart: number = Math.sqrt(
            (1.0 / firstVoiceAmplitudeStart ** 2.0 - 1.0) / (Config.supersawVoiceCount - 1.0),
          ),
          dynamismEnd: number = Math.sqrt(
        ...
        if (initializeSupersaw) {
          ...
          for (let i = 0; i < Config.supersawVoiceCount; i++) {
            const amplitude: number = i === 0 ? 1.0 : dynamismStart,
              normalizedPhase: number = tone.phases[i]! / accumulator;
          ...
          for (let i: number = Config.supersawVoiceCount - 1; i >= 0; i--) {
            const nextDrop: number = 1.0 - tone.phases[i]!,
              phaseDelta: number = nextDrop - prevDrop;
            if (sample < 0.0) {
        ...
        const baseSpreadSlider: number = instrument.supersawSpread / Config.supersawSpreadMax,
          spreadSliderStart: number =
            baseSpreadSlider * envelopeStarts[EnvelopeComputeIndex.supersawSpread]!,
          spreadSliderEnd: number =
          ...
          averageSpreadSlider: number = (spreadSliderStart + spreadSliderEnd) * 0.5,
          curvedSpread: number =
        ...
        const baseShape: number = instrument.supersawShape / Config.supersawShapeMax,
          shapeStart: number = baseShape * envelopeStarts[EnvelopeComputeIndex.supersawShape]!,
          shapeEnd: number = baseShape * envelopeEnds[EnvelopeComputeIndex.supersawShape]!;
        ...
        if (tone.supersawDelayLine == null || tone.supersawDelayLine.length <= minBufferLength) {
          ...
          if (!initializeSupersaw && tone.supersawDelayLine != null) {
            ...
            const oldDelayBufferMask: number = (tone.supersawDelayLine.length - 1) >> 0,
              startCopyingFromIndex: number = tone.supersawDelayIndex;
      ...
      if (instrument.type === InstrumentType.pickedString) {
        ...
        if (tone.prevStringDecay == null) {
          ...
          stringDecayStart =
            ...
            Math.min(
              1.0,
              (sustainEnvelopeStart * instrument.stringSustain) / (Config.stringSustainRange - 1),
            );
        } else {
        ...
        const sustainEnvelopeEnd: number =
          ...
          stringDecayEnd: number =
            ...
            Math.min(
              1.0,
              (sustainEnvelopeEnd * instrument.stringSustain) / (Config.stringSustainRange - 1),
            );
    ...
    if (
      tone.note != null &&
      (instrument.type === InstrumentType.chip || instrument.type === InstrumentType.fm)
    ) {
  ...
  private computeTonePhaseCatchUp(
    ...
    const usesUnison: boolean =
      instrument.type === InstrumentType.fm && effectsIncludeUnison(instrument.effects);
    if (instrument.type === InstrumentType.chip) {
      if (tone.chipWaveStarted[0]! && tone.chipWaveStarted[1]!) {
        return;
      }
    } else {
    ...
    const note: Note = tone.note!,
      startPart: number = Math.max(note.start, tone.noteStartPart),
      endPart: number = Math.min(note.end, currentPart),
      continuedNotes: Note[] = this.getContinuedNoteChain(song, channelIndex, note);
    let elapsedParts: number = Math.max(0.0, endPart - startPart);
    for (let i = 1; i < continuedNotes.length; i++) {
    ...
    const intervalScale: number = song.getChannelIsNoise(channelIndex) ? Config.noiseInterval : 1.0,
      basePitch: number = Config.keys[song.key]!.basePitch,
      unison: Unison = instrument.getUnison(),
      unisonA: number = 2.0 ** ((unison.offset + unison.spread) / 12.0),
      unisonB: number = 2.0 ** ((unison.offset - unison.spread) / 12.0),
      setCatchUp = (index: number, cycles: number): void => {
        if (tone.chipWaveStarted[index]!) {
          return;
        }
    ...
    for (let i = 0; i < Config.operatorCount; i++) {
      ...
      if (tone.chipWaveStarted[i]! && (!usesUnison || tone.chipWaveStarted[secondVoiceIndex]!)) {
        continue;
      }
      const associatedCarrierIndex: number =
          Config.algorithms[instrument.algorithm]!.associatedCarrier[i]! - 1,
        pitchIndex: number = chord.arpeggiates
          ? 0
          : i < tone.pitchCount
            ? i
            : associatedCarrierIndex < tone.pitchCount
      ...
      if (usesUnison) {
        setCatchUp(secondVoiceIndex, cycles * unisonB);
      }
    }
  }

  private getContinuedNoteChain(song: Song, channelIndex: number, note: Note): Note[] {
    const notes: Note[] = [note],
      partsPerBar: number = Config.partsPerBeat * song.beatsPerBar;
    let currentNote: Note = note,
      bar: number = this.bar;
    while (bar > 0 && currentNote.start === 0 && currentNote.continuesLastPattern) {
      const previousPattern: Pattern | null = song.getPattern(channelIndex, bar - 1);
      if (previousPattern == null || previousPattern.notes.length === 0) {
        break;
      }
      const previousNote: Note = previousPattern.notes.at(-1)!;
      if (
  ...
  private static integrateContinuedPitchBendCycles(
    ...
    for (let i = 0; i < notes.length; i++) {
      ...
      if (i + 1 < notes.length) {
        ...
        pitch -= previousNote.pins.at(-1)!.interval;
      }
    }
    return cycles;
  }
  ...
  private static integratePitchBendCycles(
    ...
    for (let i = 1; i < note.pins.length; i++) {
      const startPin: NotePin = note.pins[i - 1]!,
        endPin: NotePin = note.pins[i]!,
        pinStartPart: number = note.start + startPin.time,
        pinEndPart: number = note.start + endPin.time,
        segmentStart: number = Math.max(startPart, pinStartPart),
        segmentEnd: number = Math.min(endPart, pinEndPart);
      if (segmentEnd <= segmentStart || pinEndPart <= pinStartPart) {
        continue;
      }

      const startRatio: number = (segmentStart - pinStartPart) / (pinEndPart - pinStartPart),
        endRatio: number = (segmentEnd - pinStartPart) / (pinEndPart - pinStartPart),
        intervalStart: number =
          startPin.interval + (endPin.interval - startPin.interval) * startRatio,
        intervalEnd: number = startPin.interval + (endPin.interval - startPin.interval) * endRatio,
        frequencyStart: number = Instrument.frequencyFromPitch(
          basePitch + intervalStart * intervalScale,
        ),
        frequencyEnd: number = Instrument.frequencyFromPitch(
          basePitch + intervalEnd * intervalScale,
        ),
        durationSeconds: number = (segmentEnd - segmentStart) * secondsPerPart,
        frequencyRatio: number = frequencyEnd / frequencyStart;
      cycles +=
        Math.abs(frequencyRatio - 1.0) < 1.0e-12
          ? frequencyStart * durationSeconds
          : ((frequencyEnd - frequencyStart) / Math.log(frequencyRatio)) * durationSeconds;
    }
    return cycles;
  }
  ...
  public static prepareChipWaveStretch(
    ...
    const stretch: TimeStretch = (tone.chipWaveStretchers[index] ??= new TimeStretch()),
      advance: number =
        baseDelta * (scale === 1 ? runLength : (scale ** runLength - 1) / (scale - 1));
    stretch.prepare(wave, phase, pitch, tempo, loopStart, loopEnd, oneshot, advance, modulation);
    return stretch;
  }
  ...
  public static getInstrumentSynthFunction(instrument: Instrument): Function {
    if (instrument.type === InstrumentType.fm) {
      const usesUnison: boolean = effectsIncludeUnison(instrument.effects),
        fingerprint = `${instrument.algorithm}_${
          instrument.feedbackType
        }_waves_${instrument.operators
          .map((operator) => {
            const wave = operator.wave === 0 ? undefined : Config.chipWaves[operator.wave - 1]!;
            return operator.wave + (wave?.sampleId === undefined ? "" : ":" + wave.sampleId);
          })
          .join("_")}${usesUnison ? "_unison" : ""}`;
      if (Synth.fmSynthFunctionCache[fingerprint] === undefined) {
        ...
        for (const line of Synth.fmSourceTemplate) {
          if (line.indexOf("// CARRIER OUTPUTS") !== -1) {
                ...
                for (const operatorLine of asset
                  ...
                  if (operatorLine.indexOf("/* + operator@Scaled*/") !== -1) {
                    ...
                    for (const modulatorNumber of Config.algorithms[instrument.algorithm]!
                      .modulatedBy[j]!) {
                      modulators += ` + operator${voice}${modulatorNumber - 1}Scaled`;
                    }

                    const feedbackIndices: readonly number[] =
                      Config.feedbacks[instrument.feedbackType]!.indices[j]!;
                    if (feedbackIndices.length > 0) {
                      modulators += " + feedbackMult * (";
                      const feedbacks: string[] = [];
                      for (const modulatorNumber of feedbackIndices) {
                        feedbacks.push(`operator${voice}${modulatorNumber - 1}Output`);
                      }
                      modulators += `${feedbacks.join(" + ")})`;
                    }
                    if (asset && modulators !== "") {
                      modulators = ` + (${modulators.substring(3)}) * operator${voice}${
                        j
                      }PhaseModScale`;
                    }
                    renderedLine = renderedLine.replace("/* + operator@Scaled*/", modulators);
                  }
                  synthSource.push(
                    renderedLine
                      .replaceAll("/*operatorIndex*/", String(j))
                      .replaceAll("operator#", `operator${voice}${j}`),
                  );
                }
              }
            }
            ...
            for (const voice of usesUnison ? ["A", "B"] : [""]) {
              for (let j = 0; j < Config.operatorCount; j++) {
                const stateIndex: number = j + (voice === "B" ? Config.operatorCount : 0);
                let voiceLine: string = line.replaceAll(
                  /tone\.(phases|phaseDeltas|phaseDeltaScales|phaseCatchUpCycles|chipWaveStarted|feedbackOutputs)\[#\]/g,
                  (_match, arrayName) => `tone.${arrayName}[${stateIndex}]`,
                );
                voiceLine = voiceLine.replaceAll(
                ...
                const operatorWaveIndex = instrument.operators[j]!.wave,
                  chipWave =
                    operatorWaveIndex === 0 ? undefined : Config.chipWaves[operatorWaveIndex - 1]!;
                if (chipWave?.sampleId !== undefined) {
                  if (line.indexOf("Config.getFmWave") !== -1) {
                    voiceLine =
                      `\t\tconst operator#SampleData = synth.assetData.get(${JSON.stringify(
                        chipWave.sampleId,
                      )});\n` +
                      ...
                      } : operator#SampleData.sampleRate / ${Instrument.frequencyFromPitch(
                      ...
                      `\t\tconst operator#Oneshot = instrument.operatorChipWaveOneshots[${j}];`;
                  } else if (line.indexOf("let operator#Phase =") !== -1) {
                    voiceLine = `\t\tlet operator#Phase = tone.chipWaveStarted[${
                      stateIndex
                  ...
                  } else if (line.indexOf("tone.chipWaveStarted[#] = true") !== -1) {
                    voiceLine = `\t\ttone.chipWaveStarted[${
                      stateIndex
                    }] = operator#SampleData != undefined;`;
                  } else if (line.indexOf("let operator#BasePhaseDelta =") !== -1) {
                    voiceLine = `\t\tlet operator#BasePhaseDelta = +tone.phaseDeltas[${
                      stateIndex
                    }] * operator#PhaseScale;`;
                  } else if (line.indexOf("let operator#PhaseDelta =") !== -1) {
                    voiceLine = `\t\tlet operator#PhaseDelta = operator#BasePhaseDelta * instrument.operatorChipWaveTempoFactors[${
                      j
                    }];`;
                  } else if (line.indexOf("let operator#PhaseDeltaScale =") !== -1) {
                    ...
                    const peak = (index: number): string => {
                        const wave = Config.chipWaves[instrument.operators[index]!.wave - 1]!;
                        if (wave?.sampleId == null) {
                          return "2";
                        }
                        return `(synth.assetData.get(${JSON.stringify(
                          wave.sampleId,
                        )})?.peak ?? 0) * ${2 * Math.sqrt(TimeStretch.frameSize)}`;
                      },
                      terms: string[] = Config.algorithms[instrument.algorithm]!.modulatedBy[
                        j
                      ]!.map((number) => {
                        const index: number = number - 1;
                        return `Math.max(Math.abs(tone.operatorExpressions[${
                          index
                        }]), Math.abs(tone.operatorExpressions[${
                          index
                        }] + tone.operatorExpressionDeltas[${
                          index
                        }] * Math.ceil(synth.tickSampleCountdown))) * ${peak(index)}`;
                      });
                    for (const number of Config.feedbacks[instrument.feedbackType]!.indices[j]!) {
                      terms.push(
                        `Math.max(Math.abs(tone.feedbackMult), Math.abs(tone.feedbackMult + tone.feedbackDelta * Math.ceil(synth.tickSampleCountdown))) * ${peak(
                          number - 1,
                        )}`,
                      );
                    }
                  ...
                  } else if (line.indexOf("operator#Phase += operator#PhaseDelta") !== -1) {
                    voiceLine =
                ...
                voiceLine = voiceLine
                  ...
                  .replaceAll("/*operatorIndex*/", String(j));
                voiceLine = voiceLine.replaceAll("operator#", `operator${voice}${j}`);
                synthSource.push(voiceLine);
              }
            }
          } else if (usesUnison && line.indexOf("const inputSample = fmOutput;") !== -1) {
            synthSource.push(line.replace("fmOutput", "fmOutputA + fmOutputB * unisonSign"));
          } else {
        ...
        Synth.fmSynthFunctionCache[fingerprint] = new Function("Config", "Synth", wrappedFmSynth)(
          Config,
          Synth,
        );
      }
  ...
  private static soundFontSynth(
    ...
    const bankId: string | null = instrumentState.soundFontBankId,
      bank: SoundFontBank | undefined =
        bankId == null ? undefined : synth.soundFontBanks.get(bankId),
      preset = bank?.presets[instrumentState.soundFontPresetIndex];
    if (bankId == null || bank === undefined || preset === undefined) {
      return;
    }
    ...
    if (!(frequencyA > 0) || !Number.isFinite(frequencyA)) {
      return;
    }
    if (!tone.soundFontInitialized) {
      const velocity: number = Math.max(
        1,
        Math.min(127, Math.round((127 * tone.envelopeComputer.noteSizeStart) / Config.noteSizeMax)),
      );
      for (const zone of getSoundFontZones(preset, tone.soundFontKey, velocity)) {
    ...
    if (tone.soundFontVoices.length === 0) {
      return;
    }

    const data: Float32Array = synth.tempMonoInstrumentSampleBuffer!,
      unisonSign: number = tone.specialIntervalExpressionMult * instrumentState.unison!.sign,
      copyFirstVoice: boolean =
        instrumentState.unison!.voices === 1 && !instrumentState.chord!.customInterval,
      frequencyB: number = tone.phaseDeltas[1]! * synth.samplesPerSecond,
      midiPitchA: number = 69 + 12 * Math.log2(frequencyA / 440),
      midiPitchB: number =
        frequencyB > 0 && Number.isFinite(frequencyB)
          ? 69 + 12 * Math.log2(frequencyB / 440)
          : midiPitchA;
    for (let i = 0; i < tone.soundFontVoices.length; i++) {
      const voice: SoundFontToneVoice = tone.soundFontVoices[i]!,
        zone: SoundFontZone = voice.zone;
      if (copyFirstVoice) {
        voice.phaseB = voice.phaseA;
        voice.endedB = voice.endedA;
      }
      const scale: number = zone.scaleTuning / 100,
        effectiveKey: number = zone.fixedKey ?? tone.soundFontKey,
        trackedPitch: number = (effectiveKey - zone.rootKey) * scale + zone.tuneCents / 100;
      voice.phaseDeltaA =
        (zone.sample.sampleRate / synth.samplesPerSecond) *
        2 ** ((trackedPitch + midiPitchA - tone.soundFontKey) / 12);
      voice.phaseDeltaB =
        (zone.sample.sampleRate / synth.samplesPerSecond) *
        2 ** ((trackedPitch + midiPitchB - tone.soundFontKey) / 12);
    ...
    const expressionDelta = Number(tone.expressionDelta),
      filters: DynamicBiquadFilter[] = tone.noteFilters,
      filterCount: number = tone.noteFilterCount | 0;
    let initialFilterInput1 = Number(tone.initialNoteFilterInput1),
      initialFilterInput2 = Number(tone.initialNoteFilterInput2);
    const stopIndex: number = bufferIndex + runLength;
    for (let sampleIndex: number = bufferIndex; sampleIndex < stopIndex; sampleIndex++) {
      let inputSample = 0.0;
      for (let voiceIndex = 0; voiceIndex < tone.soundFontVoices.length; voiceIndex++) {
        const voice: SoundFontToneVoice = tone.soundFontVoices[voiceIndex]!,
          zone: SoundFontZone = voice.zone,
          loop: boolean = zone.loopMode === 1 || (zone.loopMode === 3 && !tone.released);
        let waveA = 0.0,
          waveB = 0.0;
        if (!voice.endedA) {
          voice.phaseA = Synth.normalizeSoundFontPhase(voice.phaseA, zone, loop);
          ...
          if (!loop && voice.phaseA >= zone.end) {
            voice.endedA = true;
          }
        }
        if (!voice.endedB) {
          voice.phaseB = Synth.normalizeSoundFontPhase(voice.phaseB, zone, loop);
    ...
    tone.phaseDeltas[0]! *= tone.phaseDeltaScales[0]! ** runLength;
    tone.phaseDeltas[1]! *= tone.phaseDeltaScales[1]! ** runLength;
  ...
  private static normalizeSoundFontPhase(
    ...
    if (loop && zone.loopEnd - zone.loopStart >= 2 && phase >= zone.loopEnd) {
      return zone.loopStart + ((phase - zone.loopEnd) % (zone.loopEnd - zone.loopStart));
    }
    return phase;
  }
  ...
  private static interpolateSoundFontSample(
    ...
    if (phase < zone.start || phase >= zone.end) {
      return 0.0;
    }
    ...
    if (nextIndex >= zone.end) {
      nextIndex = index;
    }
    const wave: Int16Array = zone.sample.data,
      sample: number = wave[index]! + (wave[nextIndex]! - wave[index]!) * (phase - index);
  ...
  private static chipSynth(
    ...
    const data: Float32Array = synth.tempMonoInstrumentSampleBuffer!,
      wave: Float32Array = instrumentState.wave!,
      waveLength: number = wave.length - 1, // The first sample is duplicated at the end, don't double-count it.
      unisonSign: number = instrumentState.usesUnison
        ? tone.specialIntervalExpressionMult * instrumentState.unison!.sign
        : 0.0,
      singleVoice: boolean =
        instrumentState.unison!.voices === 1 && !instrumentState.chord!.customInterval;
    if (singleVoice && tone.chipWaveStarted[0]!) {
      tone.phases[1] = tone.phases[0]!;
    }
    let basePhaseDeltaA: number = tone.phaseDeltas[0]! * waveLength,
      basePhaseDeltaB: number = tone.phaseDeltas[1]! * waveLength,
      phaseDeltaA: number = basePhaseDeltaA * instrumentState.chipWavePitchFactor,
      phaseDeltaB: number = basePhaseDeltaB * instrumentState.chipWavePitchFactor;
    const phaseDeltaScaleA = Number(tone.phaseDeltaScales[0]!),
      phaseDeltaScaleB = Number(tone.phaseDeltaScales[1]!);
    let expression = Number(tone.expression);
    const expressionDelta = Number(tone.expressionDelta);
    let phaseA: number = tone.chipWaveStarted[0]!
        ? (tone.phases[0]! % 1) * waveLength
        : tone.phaseCatchUpCycles[0]! * instrumentState.chipWavePitchFactor * waveLength,
      phaseB: number = singleVoice
        ? phaseA
        : tone.chipWaveStarted[1]!
          ? (tone.phases[1]! % 1) * waveLength
          : tone.phaseCatchUpCycles[1]! * instrumentState.chipWavePitchFactor * waveLength;
    ...
    const applyFilters: Function = Synth.applyFilters,
      interpolateIntegral = (phase: number): number => {
        const phaseFloor: number = Math.floor(phase),
          index: number = ((phaseFloor % waveLength) + waveLength) % waveLength,
          ratio: number = phase - phaseFloor,
          value: number = wave[index]!;
        return value + (wave[index + 1]! - value) * ratio;
      };
    let prevWaveIntegralA: number = interpolateIntegral(phaseA),
      prevWaveIntegralB: number = interpolateIntegral(phaseB);
    ...
    for (let sampleIndex: number = bufferIndex; sampleIndex < stopIndex; sampleIndex++) {
      ...
      const nextWaveIntegralA: number = interpolateIntegral(phaseA),
        nextWaveIntegralB: number = interpolateIntegral(phaseB),
        waveA: number =
          phaseDeltaA === 0.0 ? 0.0 : (nextWaveIntegralA - prevWaveIntegralA) / phaseDeltaA,
        waveB: number =
          phaseDeltaB === 0.0 ? 0.0 : (nextWaveIntegralB - prevWaveIntegralB) / phaseDeltaB;
      prevWaveIntegralA = nextWaveIntegralA;
      prevWaveIntegralB = nextWaveIntegralB;

      const inputSample: number = waveA + waveB * unisonSign,
        sample: number = applyFilters(
          inputSample,
          initialFilterInput1,
          initialFilterInput2,
          filterCount,
  ...
  private static assetSynth(
    ...
    if (sampleData === undefined || sampleData.samples.length === 0) {
      return;
    }

    const data: Float32Array = synth.tempMonoInstrumentSampleBuffer!,
      wave: Float32Array = sampleData.samples,
      waveLength: number = wave.length,
      rootFrequency: number = Instrument.frequencyFromPitch(instrumentState.assetRootKey),
      samplePhaseScale: number = sampleData.sampleRate / rootFrequency,
      pitchFactor: number = instrumentState.chipWavePitchFactor,
      tempoFactor: number = instrumentState.chipWaveTempoFactor,
      loopStart: number = Math.max(
        0,
        Math.min(waveLength - 1, instrumentState.chipWaveLoopStart * waveLength),
      ),
      loopEnd: number = Math.max(
        loopStart + 1,
        Math.min(waveLength, instrumentState.chipWaveLoopEnd * waveLength),
      ),
      offset: number = Math.max(0, Math.min(loopEnd, instrumentState.chipWaveOffset * waveLength)),
      oneshot: boolean = instrumentState.chipWaveOneshot,
      unisonSign: number = instrumentState.usesUnison
        ? tone.specialIntervalExpressionMult * instrumentState.unison!.sign
        : 0.0,
      singleVoice: boolean =
    ...
    let basePhaseDeltaA: number = tone.phaseDeltas[0]! * samplePhaseScale,
      basePhaseDeltaB: number = tone.phaseDeltas[1]! * samplePhaseScale;
    const phaseDeltaScaleA = Number(tone.phaseDeltaScales[0]!),
      phaseDeltaScaleB = Number(tone.phaseDeltaScales[1]!);
    let expression = Number(tone.expression);
    const expressionDelta = Number(tone.expressionDelta);
    ...
    const stretchA: TimeStretch | null = Synth.prepareChipWaveStretch(
      ...
      stretchB: TimeStretch | null = singleVoice
        ? stretchA
    ...
    let initialFilterInput1 = Number(tone.initialNoteFilterInput1),
      initialFilterInput2 = Number(tone.initialNoteFilterInput2);
    const applyFilters: Function = Synth.applyFilters,
      stopIndex: number = bufferIndex + runLength;
    for (let sampleIndex: number = bufferIndex; sampleIndex < stopIndex; sampleIndex++) {
      const sourcePhaseDeltaA: number = basePhaseDeltaA * tempoFactor,
        sourcePhaseDeltaB: number = basePhaseDeltaB * tempoFactor,
        waveA: number =
          oneshot && phaseA >= loopEnd
            ? 0
            : stretchA == null
              ? interpolateChipWaveSample(wave, phaseA, loopStart, loopEnd, oneshot)
              : stretchA.read(),
        waveB: number = singleVoice
          ? waveA
          : oneshot && phaseB >= loopEnd
            ? 0
            : stretchB == null
              ? interpolateChipWaveSample(wave, phaseB, loopStart, loopEnd, oneshot)
              : stretchB.read();
      stretchA?.advance(basePhaseDeltaA);
      if (!singleVoice) {
        stretchB?.advance(basePhaseDeltaB);
      }

      const inputSample: number = waveA + waveB * unisonSign,
        filteredSample: number = applyFilters(
          inputSample,
          initialFilterInput1,
          initialFilterInput2,
          filterCount,
      ...
      phaseA = Synth.advanceChipWavePhase(phaseA, sourcePhaseDeltaA, loopStart, loopEnd, oneshot);
      phaseB = Synth.advanceChipWavePhase(phaseB, sourcePhaseDeltaB, loopStart, loopEnd, oneshot);
      basePhaseDeltaA *= phaseDeltaScaleA;
      basePhaseDeltaB *= phaseDeltaScaleB;
    }
  ...
  private static harmonicsSynth(
    ...
    let phaseDeltaA: number = tone.phaseDeltas[0]! * waveLength,
      phaseDeltaB: number = tone.phaseDeltas[1]! * waveLength;
    const phaseDeltaScaleA = Number(tone.phaseDeltaScales[0]!),
      phaseDeltaScaleB = Number(tone.phaseDeltaScales[1]!);
    let expression = Number(tone.expression);
    const expressionDelta = Number(tone.expressionDelta);
    let phaseA: number = (tone.phases[0]! % 1) * waveLength,
      phaseB: number = (tone.phases[1]! % 1) * waveLength;
    ...
    let initialFilterInput1 = Number(tone.initialNoteFilterInput1),
      initialFilterInput2 = Number(tone.initialNoteFilterInput2);
    const applyFilters: Function = Synth.applyFilters,
      phaseAInt: number = phaseA | 0,
      phaseBInt: number = phaseB | 0,
      indexA: number = phaseAInt % waveLength,
      indexB: number = phaseBInt % waveLength,
      phaseRatioA: number = phaseA - phaseAInt,
      phaseRatioB: number = phaseB - phaseBInt;
    let prevWaveIntegralA = Number(wave[indexA]!),
      prevWaveIntegralB = Number(wave[indexB]!);
    ...
    for (let sampleIndex: number = bufferIndex; sampleIndex < stopIndex; sampleIndex++) {
      ...
      const nextPhaseAInt: number = phaseA | 0,
        nextPhaseBInt: number = phaseB | 0,
        nextIndexA: number = nextPhaseAInt % waveLength,
        nextIndexB: number = nextPhaseBInt % waveLength;
      let nextWaveIntegralA: number = wave[nextIndexA]!,
        nextWaveIntegralB: number = wave[nextIndexB]!;
      const nextPhaseRatioA: number = phaseA - nextPhaseAInt,
        nextPhaseRatioB: number = phaseB - nextPhaseBInt;
      nextWaveIntegralA += (wave[nextIndexA + 1]! - nextWaveIntegralA) * nextPhaseRatioA;
      nextWaveIntegralB += (wave[nextIndexB + 1]! - nextWaveIntegralB) * nextPhaseRatioB;
      const waveA: number = (nextWaveIntegralA - prevWaveIntegralA) / phaseDeltaA,
        waveB: number = (nextWaveIntegralB - prevWaveIntegralB) / phaseDeltaB;
      prevWaveIntegralA = nextWaveIntegralA;
      prevWaveIntegralB = nextWaveIntegralB;
  ...
  private static pickedStringSynth(
    ...
    if (pickedStringFunction === undefined) {
      ...
      pickedStringSource = pickedStringSource.replaceAll(/^.*#.*$/gm, (line) => {
        const lines = [];
        for (let voice = 0; voice < voiceCount; voice++) {
          lines.push(line.replaceAll("#", String(voice)));
        }
        return lines.join("\n");
      });
  ...
  private static effectsSynth(
    ...
    const usesDistortion: boolean = effectsIncludeDistortion(instrumentState.effects),
      usesBitcrusher: boolean = effectsIncludeBitcrusher(instrumentState.effects),
      usesEqFilter: boolean = instrumentState.eqFilterCount > 0,
      usesChorus: boolean = effectsIncludeChorus(instrumentState.effects),
      usesEcho: boolean = effectsIncludeEcho(instrumentState.effects),
      usesReverb: boolean = effectsIncludeReverb(instrumentState.effects);
    let signature = 0;
    if (usesDistortion) {
      signature |= 1;
    }
  ...
  private static pulseWidthSynth(
    ...
    let phaseDeltaA: number = tone.phaseDeltas[0]!,
      phaseDeltaB: number = tone.phaseDeltas[1]!;
    const phaseDeltaScaleA = Number(tone.phaseDeltaScales[0]!),
      phaseDeltaScaleB = Number(tone.phaseDeltaScales[1]!);
    let expression = Number(tone.expression);
    const expressionDelta = Number(tone.expressionDelta);
    let phaseA: number = tone.phases[0]! % 1,
      phaseB: number = tone.phases[1]! % 1,
      pulseWidth: number = tone.pulseWidth;
    ...
    for (let sampleIndex: number = bufferIndex; sampleIndex < stopIndex; sampleIndex++) {
      const sawPhaseA: number = phaseA % 1,
        sawPhaseB: number = (phaseA + pulseWidth) % 1,
        sawPhaseC: number = phaseB % 1,
        sawPhaseD: number = (phaseB + pulseWidth) % 1;

      let pulseWaveA: number = sawPhaseB - sawPhaseA,
        pulseWaveB: number = sawPhaseD - sawPhaseC;

      // This is a PolyBLEP, which smooths out discontinuities at any frequency to reduce aliasing.
      if (sawPhaseA < phaseDeltaA) {
        const t = sawPhaseA / phaseDeltaA;
        pulseWaveA += (t + t - t * t - 1) * 0.5;
      } else if (sawPhaseA > 1.0 - phaseDeltaA) {
        const t = (sawPhaseA - 1.0) / phaseDeltaA;
        pulseWaveA += (t + t + t * t + 1) * 0.5;
      }
      if (sawPhaseB < phaseDeltaA) {
        const t = sawPhaseB / phaseDeltaA;
        pulseWaveA -= (t + t - t * t - 1) * 0.5;
      } else if (sawPhaseB > 1.0 - phaseDeltaA) {
        const t = (sawPhaseB - 1.0) / phaseDeltaA;
        pulseWaveA -= (t + t + t * t + 1) * 0.5;
      }
      if (sawPhaseC < phaseDeltaB) {
        const t = sawPhaseC / phaseDeltaB;
        pulseWaveB += (t + t - t * t - 1) * 0.5;
      } else if (sawPhaseC > 1.0 - phaseDeltaB) {
        const t = (sawPhaseC - 1.0) / phaseDeltaB;
        pulseWaveB += (t + t + t * t + 1) * 0.5;
      }
      if (sawPhaseD < phaseDeltaB) {
        const t = sawPhaseD / phaseDeltaB;
        pulseWaveB -= (t + t - t * t - 1) * 0.5;
      } else if (sawPhaseD > 1.0 - phaseDeltaB) {
        const t = (sawPhaseD - 1.0) / phaseDeltaB;
        pulseWaveB -= (t + t + t * t + 1) * 0.5;
      }
  ...
  private static supersawSynth(
    ...
    const data: Float32Array = synth.tempMonoInstrumentSampleBuffer!,
      voiceCount: number = Config.supersawVoiceCount | 0;

    let phaseDeltaA: number = tone.phaseDeltas[0]!,
      phaseDeltaB: number = tone.phaseDeltas[1]!;
    const phaseDeltaScaleA = Number(tone.phaseDeltaScales[0]!),
      phaseDeltaScaleB = Number(tone.phaseDeltaScales[1]!),
      groupCount: number = instrumentState.usesUnison ? 2 : 1,
      unisonSign: number = tone.specialIntervalExpressionMult * instrumentState.unison!.sign;
    let expression = Number(tone.expression);
    const expressionDelta = Number(tone.expressionDelta),
      phases: number[] = tone.phases;

    let dynamism = Number(tone.supersawDynamism);
    const dynamismDelta = Number(tone.supersawDynamismDelta),
      unisonDetunes: number[] = tone.supersawUnisonDetunes;
    ...
    const delayLengthDelta = Number(tone.supersawDelayLengthDelta),
      delayLine: Float32Array = tone.supersawDelayLine!,
      delayBufferMask: number = (delayLine.length - 1) >> 0;
    ...
    const applyFilters: Function = Synth.applyFilters,
      stopIndex: number = bufferIndex + runLength;
    for (let sampleIndex: number = bufferIndex; sampleIndex < stopIndex; sampleIndex++) {
      let supersawSample = 0.0;
      for (let group = 0; group < groupCount; group++) {
        const groupOffset: number = group * voiceCount,
          phaseDelta: number = group === 0 ? phaseDeltaA : phaseDeltaB;
        let phase: number = (phases[groupOffset]! + phaseDelta) % 1.0,
          groupSample: number = phase - 0.5 * (1.0 + (voiceCount - 1.0) * dynamism);
        if (phase < phaseDelta) {
      ...
      const delaySampleTime: number = delayIndex - delayLength,
        lowerIndex: number = delaySampleTime | 0,
        upperIndex: number = lowerIndex + 1,
        delayRatio: number = delaySampleTime - lowerIndex,
        prevDelaySample: number = delayLine[lowerIndex & delayBufferMask]!,
        nextDelaySample: number = delayLine[upperIndex & delayBufferMask]!,
        delaySample: number = prevDelaySample + (nextDelaySample - prevDelaySample) * delayRatio;
      delayIndex++;
  ...
  private static fmSourceTemplate: string[] = `
      ...
      Config.sineWaveLength
		...
		let operator#BasePhaseDelta = +tone.phaseDeltas[#] * ${Config.sineWaveLength};
		...
		tone.phaseDeltas[#] = operator#BasePhaseDelta / ${Config.sineWaveLength};
  ...
  private static noiseSynth(
    ...
    let phaseA: number = (tone.phases[0]! % 1) * Config.chipNoiseLength,
      phaseB: number = (tone.phases[1]! % 1) * Config.chipNoiseLength;
    ...
    let noiseSampleA = Number(tone.noiseSample),
      noiseSampleB = Number(tone.noiseSampleB);
    ...
    const applyFilters: Function = Synth.applyFilters,
      ...
      pitchRelativefilterA: number = Math.min(
        1.0,
        phaseDeltaA * instrumentState.noisePitchFilterMult,
      ),
      pitchRelativefilterB: number = Math.min(
        1.0,
    ...
    for (let sampleIndex: number = bufferIndex; sampleIndex < stopIndex; sampleIndex++) {
      const waveSampleA: number = wave[phaseA & phaseMask]!,
        waveSampleB: number = wave[phaseB & phaseMask]!;
  ...
  private static spectrumSynth(
    ...
    let phaseDeltaA: number = tone.phaseDeltas[0]! * samplesInPeriod,
      phaseDeltaB: number = tone.phaseDeltas[1]! * samplesInPeriod;
    const phaseDeltaScaleA = Number(tone.phaseDeltaScales[0]!),
      phaseDeltaScaleB = Number(tone.phaseDeltaScales[1]!);
    let expression = Number(tone.expression);
    const expressionDelta = Number(tone.expressionDelta);
    ...
    let phaseA: number = (tone.phases[0]! % 1) * Config.spectrumNoiseLength,
      phaseB: number = (tone.phases[1]! % 1) * Config.spectrumNoiseLength;
    // Zero phase means the tone was reset, just give noise a random start phase instead.
    if (tone.phases[0] === 0.0) {
    ...
    const phaseMask: number = Config.spectrumNoiseLength - 1,
      // This is for a "legacy" style simplified 1st order lowpass filter with
      // A cutoff frequency that is relative to the tone's fundamental frequency.
      pitchRelativefilterA: number = Math.min(1.0, phaseDeltaA),
      pitchRelativefilterB: number = Math.min(1.0, phaseDeltaB),
      stopIndex: number = bufferIndex + runLength;
    for (let sampleIndex: number = bufferIndex; sampleIndex < stopIndex; sampleIndex++) {
      const phaseAInt: number = phaseA | 0,
        phaseBInt: number = phaseB | 0,
        indexA: number = phaseAInt & phaseMask,
        indexB: number = phaseBInt & phaseMask;
      let waveSampleA: number = wave[indexA]!,
        waveSampleB: number = wave[indexB]!;
  ...
  private static drumsetSynth(
    ...
    const data: Float32Array = synth.tempMonoInstrumentSampleBuffer!,
      wave: Float32Array = instrumentState.getDrumsetWave(tone.drumsetPitch!),
      referenceDelta: number = InstrumentState.drumsetIndexReferenceDelta(tone.drumsetPitch!),
      unisonSign: number = instrumentState.usesUnison
        ? tone.specialIntervalExpressionMult * instrumentState.unison!.sign
        : 0.0,
      singleVoice: boolean =
    ...
    let phaseDeltaA: number = tone.phaseDeltas[0]! / referenceDelta,
      phaseDeltaB: number = tone.phaseDeltas[1]! / referenceDelta;
    const phaseDeltaScaleA = Number(tone.phaseDeltaScales[0]!),
      phaseDeltaScaleB = Number(tone.phaseDeltaScales[1]!);
    let expression = Number(tone.expression);
    const expressionDelta = Number(tone.expressionDelta),
    ...
    for (let sampleIndex: number = bufferIndex; sampleIndex < stopIndex; sampleIndex++) {
      ...
      let noiseSampleA: number = wave[indexA]!,
        noiseSampleB: number = wave[indexB]!;
  ...
  public static instrumentVolumeToVolumeMult(instrumentVolume: number): number {
    ...
    const maximumSetting: number = Config.volumeRange - 1,
      clampedSetting: number = Math.min(maximumSetting, instrumentVolume),
      ratio: number = (clampedSetting - 1) / (maximumSetting - 1);
    return Config.volumeMinGain * (Config.volumeMaxGain / Config.volumeMinGain) ** ratio;
  }
  public static echoSustainToVolumeMult(echoSustain: number): number {
    const oldMaximumSetting: number = Config.echoSustainRange - 1,
      oldMaximumVolume: number = (oldMaximumSetting / Config.echoSustainRange) ** 1.1 * 0.9;
    if (echoSustain <= oldMaximumSetting) {
      return (Math.max(0.0, echoSustain) / Config.echoSustainRange) ** 1.1 * 0.9;
    }
    const ratio: number = Math.min(1.0, (echoSustain - oldMaximumSetting) / oldMaximumSetting),
      easedRatio: number = ratio * ratio * (3.0 - 2.0 * ratio);
  ...
  public static volumeMultToInstrumentVolume(volumeMult: number): number {
    ...
    let lower = 1,
      upper: number = Config.volumeRange - 1;
    while (lower < upper) {
    ...
    const lowerDistance: number = volumeMult - Synth.instrumentVolumeToVolumeMult(lower - 1),
      upperDistance: number = Synth.instrumentVolumeToVolumeMult(lower) - volumeMult;
  ...
  private getSamplesPerTick(): number {
    ...
    const beatsPerMinute: number =
      ...
      beatsPerSecond: number = beatsPerMinute / 60.0,
      partsPerSecond: number = Config.partsPerBeat * beatsPerSecond,
      ticksPerSecond: number = Config.ticksPerPart * partsPerSecond;
  ...
  private sanitizeFilters(filters: DynamicBiquadFilter[]): void {
    ...
    for (const filter of filters) {
      const output1: number = Math.abs(filter.output1),
        output2: number = Math.abs(filter.output2);
  ...
  public static sanitizeDelayLine(delayLine: Float32Array, lastIndex: number, mask: number): void {
    while (true) {
      ...
      const index: number = lastIndex & mask,
        sample: number = Math.abs(delayLine[index]!);
  ...
  public static applyFilters(
    ...
    for (let i = 0; i < filterCount; i++) {
      const filter: DynamicBiquadFilter = filters[i]!,
        output1: number = filter.output1,
        output2: number = filter.output2,
        a1: number = filter.a1,
        a2: number = filter.a2,
        b0: number = filter.b0,
        b1: number = filter.b1,
