...
import { type AssetDefinition, Config } from "./synth-config.js";
import type {
  LiveInputState,
  SynthCommand,
  SynthEvent,
...
import { type Note, Song, SynthEngine } from "./synth.js";
import {
  type SoundFontInstrumentSettings,
  getSoundFontInstrumentSettings,
  parseSoundFont,
} from "./sound-font.js";
import { cacheAsset, evictCachedAsset } from "./asset-cache.js";
...
export class SynthController {
  ...
  public preferLowerLatency = false;
  public anticipatePoorPerformance = false;
  public song: Song;
  ...
  private errorWasShown = false;
  private readonly assetLoadStates = new Map<string, AssetLoadStatus>();
  private readonly assetLoadProgress = new Map<string, number | null>();
  private readonly assetLoadErrors = new Map<string, string>();
  private readonly assetLoads = new Map<string, Promise<void>>();
  private readonly sampleAssets = new Map<string, SampleAssetData>();
  private readonly soundFontAssets = new Map<string, ArrayBuffer>();
  private readonly soundFontPresets = new Map<string, readonly SoundFontPresetInfo[]>();

  private desiredPlaying = false;
  private desiredRecording = false;
  private reportedPlayhead = 0;
  private reportedPreviousBar: number | null = null;
  private reportedTempo = 120;
  private reportContextTime = 0;
  private reportedCountIn = false;
  private loopRepeatCountInternal = -1;
  private metronomeEnabledInternal = false;
  private countInEnabledInternal = false;
  ...
  private masterVolumeInternal = 1.0;
  ...
  public async loadAssetsInto(synth: SynthEngine): Promise<void> {
    ...
    for (const asset of this.song.assets) {
      if (asset.type === "soundFont") {
        const data: ArrayBuffer | undefined = this.soundFontAssets.get(asset.id);
        if (data !== undefined) {
          synth.setSoundFont(asset.id, data);
          continue;
        }
      } else {
        const data: SampleAssetData | undefined = this.sampleAssets.get(asset.id);
        if (data !== undefined) {
  ...
  public playSamplePreview(
    ...
    const sample: SampleAssetData | undefined = this.sampleAssets.get(sampleId),
      context: AudioContext | null = this.audioContext,
      output: GainNode | null = this.outputGainNode;
    if (sample === undefined || sample.samples.length === 0 || context == null || output == null) {
      return false;
    }

    this.stopSamplePreviewInternal(false);
    const sampleLength: number = sample.samples.length,
      loopStartFrame: number = Math.max(0, Math.min(sampleLength - 1, settings.loopStartFrame)),
      loopEndFrame: number = Math.max(
        loopStartFrame + 1,
        Math.min(sampleLength, settings.loopEndFrame),
      );
    let offsetFrame: number = Math.max(0, Math.min(loopEndFrame, settings.offsetFrame));
    if (!settings.oneshot && offsetFrame >= loopEndFrame) {
      offsetFrame = loopStartFrame;
    }
    const buffer: AudioBuffer = context.createBuffer(1, sampleLength, sample.sampleRate);
    ...
    source.onended = (): void => {
      if (this.samplePreviewSource !== source) {
        return;
      }
      source.disconnect();
    ...
    if (settings.oneshot) {
      source.start(0, offsetSeconds, (loopEndFrame - offsetFrame) / sample.sampleRate);
    } else {
  ...
  public getSoundFontPresets(soundFontId: string): readonly SoundFontPresetInfo[] | null {
  ...
  public set masterVolume(value: number) {
    this.masterVolumeInternal = Number.isFinite(value) ? Math.max(0, Math.min(1, value)) : 1.0;
  ...
  public get playhead(): number {
    if (
      ...
      const elapsed: number = Math.max(0, this.audioContext.currentTime - this.reportContextTime),
        estimated: number =
          this.reportedPlayhead + (elapsed * this.reportedTempo) / (60 * this.song.beatsPerBar);
  ...
  public getPreviousBar(bar: number): number | null {
    ...
    return bar > Math.floor(this.reportedPlayhead) ? bar - 1 : this.reportedPreviousBar;
  }
  ...
  private ensureAudio(): Promise<void> {
    ...
    this.audioInitialization = (async (): Promise<void> => {
      const context: AudioContext =
        this.audioContext ?? new AudioContext({ latencyHint: this.latencyHint() });
      ...
      await context.audioWorklet.addModule(new URL("synth_worklet.js", document.baseURI));
      const node: AudioWorkletNode = new AudioWorkletNode(context, "synth-processor", {
        numberOfInputs: 0,
        numberOfOutputs: 1,
      ...
      node.port.onmessage = (event: MessageEvent<SynthEvent>): void => this.handleEvent(event.data);
  ...
  private setAssetLoadStatus(sampleId: string, status: AssetLoadStatus): void {
    if (this.assetLoadStates.get(sampleId) === status) {
      return;
    }
  ...
  private setAssetLoadProgress(sampleId: string, progress: number | null): void {
  ...
  private refreshAssetLoads(): void {
    ...
    for (const sample of this.song.assets) {
      if (this.assetLoadStates.has(sample.id)) {
        continue;
      }
  ...
  private async readAssetResponse(response: Response, sampleId: string): Promise<ArrayBuffer> {
    if (response.body == null) {
      return response.arrayBuffer();
    }
    const reader: ReadableStreamDefaultReader<Uint8Array> = response.body.getReader(),
      contentLength = Number(response.headers.get("content-length")),
      hasContentLength: boolean = Number.isFinite(contentLength) && contentLength > 0,
      chunks: Uint8Array[] = [];
    let received = 0;
    for (;;) {
    ...
    const result: ArrayBuffer = new ArrayBuffer(received),
      resultView: Uint8Array = new Uint8Array(result);
    let offset = 0;
  ...
  private async fetchAsset(
    ...
    return {
      data: await this.readAssetResponse(response, sample.id),
      response,
    };
  }
  ...
  private async loadAsset(sample: AssetDefinition): Promise<void> {
    try {
      let { data: encodedAudio, response } = await this.fetchAsset(sample);
      if (sample.type === "soundFont") {
        let soundBank: ReturnType<typeof parseSoundFont>;
        try {
          soundBank = parseSoundFont(encodedAudio);
        } catch {
          await evictCachedAsset(sample);
          ({ data: encodedAudio, response } = await this.fetchAsset(sample, true));
          soundBank = parseSoundFont(encodedAudio);
        }
        ...
        if (
          !this.song.assets.some(
            (currentAsset: AssetDefinition): boolean => currentAsset.id === sample.id,
          )
        ) {
          return;
        }
        ...
        this.post({ type: "setSoundFont", soundFontId: sample.id, data: workletData }, [
          workletData,
        ]);
        this.setAssetLoadStatus(sample.id, "loaded");
        cacheAsset(sample, encodedAudio, response);
        return;
      }
      await this.ensureAudio();
      ...
      const cacheData: ArrayBuffer = encodedAudio.slice(0),
        audioBuffer: AudioBuffer = await context.decodeAudioData(encodedAudio);
      if (
        !this.song.assets.some(
          (currentSample: AssetDefinition): boolean => currentSample.id === sample.id,
        )
      ) {
        return;
      }
      ...
      const pcmBuffer: ArrayBuffer = new ArrayBuffer(
        ...
        pcm: Float32Array = new Float32Array(pcmBuffer),
        channelCount: number = audioBuffer.numberOfChannels;
      if (channelCount === 1) {
        pcm.set(audioBuffer.getChannelData(0));
      } else {
        for (let channelIndex = 0; channelIndex < channelCount; channelIndex++) {
          const channelData: Float32Array = audioBuffer.getChannelData(channelIndex);
      ...
      this.setAssetLoadStatus(sample.id, "loaded");
      cacheAsset(sample, cacheData, response);
    } catch (error: unknown) {
      if (
        !this.song.assets.some(
          (currentSample: AssetDefinition): boolean => currentSample.id === sample.id,
  ...
  private handleEvent(event: SynthEvent): void {
    switch (event.type) {
      ...
      case "initializationError": {
        this.reportAudioError(`Unable to initialize synthesizer: ${event.message}`);
        break;
      }
      case "idle": {
        this.suspendAudioIfIdle();
  ...
  private serializedSongsMatch(first: Uint8Array, second: Uint8Array | null): boolean {
  ...
  public setLiveInputState(
    ...
    const acceptsNotes: boolean =
      ...
      safePitches: readonly number[] = acceptsNotes ? pitches : [],
      safeInstruments: readonly number[] = acceptsNotes ? instruments : [];
    this.livePitches.length = Math.min(safePitches.length, Config.maxChordSize);
    for (let i = 0; i < this.livePitches.length; i++) {
      this.livePitches[i] = safePitches[i]!;
    }
    this.liveInstruments.length = safeInstruments.length;
    for (let i = 0; i < safeInstruments.length; i++) {
    ...
    if (this.livePitches.length > 0) {
      this.activateAudio();
    }
    this.liveStarted = false;
  }

  public setLiveInputPitches(pitches: readonly number[], duration: number, started: boolean): void {
  ...
  public setLiveInputInstruments(instruments: readonly number[]): void {
    if (this.song.getChannelIsAutomation(this.liveChannel)) {
      instruments = [];
    }
  ...
  public goToNextBar(): void {
    ...
    this.reportedPlayhead = (this.reportedPreviousBar + 1) % this.song.barCount;
    this.markReportTime();
  ...
  public dispose(): void {
    if (this.songSyncFrame != null) {
      cancelAnimationFrame(this.songSyncFrame);
    }
    this.songSyncFrame = null;
    this.stopSamplePreviewInternal(false);
