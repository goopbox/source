...
export class SongPerformance {
  ...
  private _lastBarHasPatterns(): boolean {
    for (
      ...
      if (
        this._doc.song.channels[channelIndex].bars[
          this._doc.song.barCount - 1
        ] != 0
      )
        return true;
      ...
      if (
        ...
        while (noteStartPart < endPart) {
          ...
          if (
            ...
            this._lastNote.continuesLastPattern =
              noteStartPart == 0 && !this._pitchesChanged;
  ...
  public addPerformedPitch(pitch: number): void {
    ...
    if (this._doc.synth.liveInputPitches.indexOf(pitch) == -1) {
      ...
      this._pitchesChanged = true;
      while (pitches.length > Config.maxChordSize) pitches.shift();
      this._doc.synth.setLiveInputPitches(
        pitches,
  ...
  public removePerformedPitch(pitch: number): void {
    ...
    if (pitches.length == this._doc.synth.liveInputPitches.length) return;
    this._doc.synth.setLiveInputPitches(
      pitches,
      Number.MAX_SAFE_INTEGER,
      false,
  ...
  private _documentChanged = (): void => {
    ...
    this._doc.synth.setLiveInputInstruments(
      this._doc.song.channels[this._doc.channel].instruments.map(
        (_instrument, index) => index,
      ),
    );
  };
}
