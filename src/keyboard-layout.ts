...
export class KeyboardLayout {
  #doc: SongDocument;
  static #pianoAtC: readonly (readonly (number | null)[])[] = [
  ...
  static #pianoAtA: readonly (readonly (number | null)[])[] = [
  ...
  public static keyPosToPitch(
    ...
    switch (keyboardLayout) {
      ...
      case "songScale": {
        const scaleFlags: readonly boolean[] = Config.scales[doc.song.scale]!.flags,
          scaleIndices: number[] = scaleFlags
      ...
      case "pianoAtC": {
        pitchOffset = KeyboardLayout.#pianoAtC[y]![x]!;
        forcedKey = Config.keys.dictionary["C"]!.basePitch;
        break;
      }
      case "pianoAtA": {
        pitchOffset = KeyboardLayout.#pianoAtA[y]![x]!;
    ...
    if (forcedKey != null) {
      const keyBasePitch: number = Config.keys[doc.song.composingKey]!.basePitch;
      keyOffset = (forcedKey - keyBasePitch + 144) % 12;
    }

    const compositionOffset: number = doc.song.getChannelIsNoise(doc.channel)
        ? 0
        : doc.song.composingKey - doc.song.key,
      pitch = octaveOffset + keyOffset + pitchOffset + compositionOffset;
  ...
  #possiblyPlayingPitchesFromKeyboard = false;
  ...
  #onWindowBlur = (_event: Event) => {
    // Browsers don't explicitly release keys when the page isn't in focus so let's just assume they're all released.
    if (this.#possiblyPlayingPitchesFromKeyboard) {
      this.#doc.performance.clearAllPitches();
      this.#possiblyPlayingPitchesFromKeyboard = false;
    }
  };
  ...
  public handleKeyEvent(event: KeyboardEvent, pressed: boolean): void {
    ...
    switch (event.code) {
      ...
      case "IntlYen": {
        ...
        break;
      } // Present on Russian and Japanese keyboards.
      ...
      case "IntlHash": {
        this.handleKey(11, 1, pressed);
        break;
      } // Present on non-US keyboards... but in practice it is actually represented as "Backslash" so this is obsolete. Oh well. :/

      case "IntlBackslash": {
        this.handleKey(-1, 0, pressed);
        break;
      } // Present on Brazillian and many European keyboards.
      case "KeyZ": {
      ...
      case "IntlRo": {
        ...
        break;
      } // Present on Brazillian and Japanese keyboards.

      default: {
        return;
      } //Unhandled, don't prevent default.
    }
  ...
  public handleKey(x: number, y: number, pressed: boolean): void {
    ...
    if (isDrum) {
      if (x >= 0 && x < Config.drumCount) {
        if (pressed) {
          this.#doc.performance.preferLowLatency();
          this.#doc.performance.addPerformedPitch(x);
          this.#possiblyPlayingPitchesFromKeyboard = true;
        } else {
          this.#doc.performance.removePerformedPitch(x);
        }
      }
      return;
    }
    ...
    const pitch: number | null = KeyboardLayout.keyPosToPitch(
      ...
      x,
      y,
      this.#doc.prefs.keyboardLayout,
    );

    if (pitch != null) {
      if (pressed) {
        this.#doc.performance.preferLowLatency();
        this.#doc.performance.addPerformedPitch(pitch);
        this.#possiblyPlayingPitchesFromKeyboard = true;
      } else {
        this.#doc.performance.removePerformedPitch(pitch);
      }
    }
  }
}
