...
export class HarmonicsEditor {
  ...
  private _xToFreq(x: number): number {
    return (
      ((Config.harmonicsControlPoints - 1) * x) / (this._editorWidth - 8) - 0.5
    );
  }
  ...
  private _whenCursorMoved(event: PointerEvent): void {
    if (event.pointer!.isDown) {
      ...
      harmonicsWave.harmonics[
        Math.max(
          0,
          Math.min(Config.harmonicsControlPoints - 1, Math.round(freq)),
        )
  ...
  public render(): void {
    ...
    for (let i: number = 0; i < Config.harmonicsControlPoints - 1; i++) {
      ...
      path +=
        ...
        xPos +
        " " +
        prettyNumber(controlPointToHeight(harmonicsWave.harmonics[i])) +
        " ";
    }
    ...
    for (let i: number = 0; i < 4; i++) {
      ...
      rect.setAttribute(
        "height",
        prettyNumber(this._editorHeight - lastHeight),
      );
    }
