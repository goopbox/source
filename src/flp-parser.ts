...
const FLHD_MAGIC: readonly number[] = [0x46, 0x4c, 0x68, 0x64],
  FLDT_MAGIC: readonly number[] = [0x46, 0x4c, 0x64, 0x74],
  FLHD_LENGTH = 6,
  NOTE_RECORD_SIZE = 24,
  NOTE_FLAG_SLIDE = 0x08,
  PLAYLIST_PATTERN_BASE = 20_480,
  PLAYLIST_UNSET_OFFSET = 0xff_ff_ff_ff,
  LEGACY_PLAYLIST_UNSET_OFFSET = 0xbf_80_00_00,
  MODERN_PLAYLIST_MAX_TRACK = 499,
  LEGACY_PLAYLIST_MAX_TRACK = 198;
...
function matchesMagic(view: DataView, offset: number, expected: readonly number[]): boolean {
  if (offset < 0 || offset > view.byteLength - expected.length) {
    return false;
  }
  for (let i = 0; i < expected.length; i++) {
    if (view.getUint8(offset + i) !== expected[i]!) {
      return false;
    }
  }
  return true;
...
function readVarInt(view: DataView, state: { offset: number }, end: number): number {
  let value = 0,
    multiplier = 1;
  for (let byteIndex = 0; byteIndex < 5; byteIndex++) {
    ...
    if (!Number.isSafeInteger(value) || value > 0xff_ff_ff_ff) {
...
function readEvents(buffer: ArrayBuffer, view: DataView, start: number, end: number): FlpEvent[] {
  ...
  if (state.offset !== end) {
    corrupt("FLP event stream exceeds its data block.");
  }
  return events;
}
...
function decodeAscii(payload: Uint8Array): string {
  ...
  return new TextDecoder("utf-8", { fatal: false }).decode(payload.subarray(0, end));
}
...
function decodeUtf16Le(payload: Uint8Array): string {
  ...
  return new TextDecoder("utf-16le", { fatal: false }).decode(payload.subarray(0, end));
}

function parseVersion(events: readonly FlpEvent[]): FlpVersion | undefined {
  for (const event of events) {
    if (event.opcode !== 0xc7 || event.kind !== "blob") {
      continue;
    }
    const version = decodeAscii(event.payload).trim(),
      match = /^(\d+)\.(\d+)(?:\.(\d+))?(?:\.\d+)?$/.exec(version);
    if (match === null) {
      continue;
    }
...
function usesLegacyText(version: FlpVersion | undefined): boolean {
  return (
    version !== undefined && (version.major < 11 || (version.major === 11 && version.minor < 5))
  );
}
...
function decodeChannelKind(value: number): Exclude<FlpChannel["kind"], undefined> {
  switch (value) {
    case 0: {
      return "sampler";
    }
...
function printableAsciiRuns(payload: Uint8Array, minimumLength = 4): PrintableAsciiRun[] {
  const runs: PrintableAsciiRun[] = [];
  for (let start = 0; start < payload.length;) {
    while (start < payload.length && (payload[start]! < 0x20 || payload[start]! > 0x7e)) {
      start++;
    }
    let end = start;
...
function cleanStatePath(candidate: string): string {
  ...
  if (drivePath?.index != null) {
    candidate = candidate.slice(drivePath.index);
  }
  return candidate.replace(/^[^A-Za-z0-9%._~\\/-]+/, "").trim();
}

function findStatePath(payload: Uint8Array, extensions: readonly string[]): string | undefined {
  for (const run of printableAsciiRuns(payload)) {
    ...
    for (const extension of extensions) {
      ...
      return cleanStatePath(run.text.slice(0, extensionIndex + extension.length));
    }
  }
  return undefined;
}
...
function decodeSoundFontPlayerState(payload: Uint8Array): {
  ...
  const info: {
    ...
    statePath = findStatePath(payload, [".sf2", ".sf3"]);
  if (statePath != null) {
    info.statePath = statePath;
  }
  if (payload.byteLength >= 12) {
    const view = new DataView(payload.buffer, payload.byteOffset, payload.byteLength),
      oneBasedProgram = view.getUint32(4, true),
      bank = view.getUint32(8, true);
    if (oneBasedProgram > 0 && oneBasedProgram <= 0x1_00_00) {
      info.soundFontProgram = oneBasedProgram - 1;
    }
    if (bank <= 0xff_ff) {
      info.soundFontBank = bank;
    }
  }
  return info;
}
...
function decodeVstWrapper(payload: Uint8Array): {
  ...
  while (offset + 12 <= payload.byteLength) {
    const id = view.getUint32(offset, true),
      length = view.getBigUint64(offset + 4, true),
      remaining = BigInt(payload.byteLength - offset - 12);
    if (length > remaining) {
      break;
    }
    const dataLength = Number(length),
      data = payload.subarray(offset + 12, offset + 12 + dataLength);
    if (id === 54) {
...
function buildChannels(events: readonly FlpEvent[], legacyText: boolean): FlpChannel[] {
  const channels: FlpChannel[] = [],
    channelsById = new Map<number, FlpChannel>(),
    preferredNames = new Set<number>(),
    pluginEvents = new Set<number>();
  ...
  for (const event of events) {
    ...
    if (event.opcode === 0x62 || event.opcode === 0x93 || event.opcode === 0xec) {
      inChannelScope = false;
      continue;
    }
    if (!inChannelScope || current === undefined || event.kind !== "blob") {
      if (inChannelScope && current !== undefined && event.opcode === 0x15 && event.kind === "u8") {
    ...
    if (event.opcode === 0xc4) {
      ...
      if (current.plugin.internalName === "Fruity Wrapper") {
        ...
        if (wrapper.name !== undefined) {
          current.plugin.name = wrapper.name;
        }
        if (wrapper.vendor !== undefined) {
          current.plugin.vendor = wrapper.vendor;
        }
      } else if (current.plugin.internalName.toLowerCase() === "fruity soundfont player") {
...
function decodeNotes(payload: Uint8Array): FlpNote[] {
  ...
  for (let offset = 0; offset < payload.byteLength; offset += NOTE_RECORD_SIZE) {
...
function buildPatterns(
  ...
  const builders: PatternBuilder[] = [],
    byId = new Map<number, PatternBuilder>();
  ...
  for (const event of events) {
    ...
    if (event.opcode === 0xc1 && event.kind === "blob" && current.name === undefined) {
      const name = decodeName(event.payload, legacyText);
      if (name !== undefined) {
        current.name = name;
      }
    ...
    if (event.opcode === 0xa4 && event.kind === "u32" && current.length === undefined) {
  ...
  for (const builder of builders) {
    ...
    for (const payload of payloads) {
      builder.notes.push(...decodeNotes(payload));
    }
  }
  ...
  return builders.map((builder: PatternBuilder): FlpPattern => {
    ...
    if (builder.length !== undefined) {
      pattern.length = builder.length;
    }
    return pattern;
  });
}
...
function playlistRecordSize(
  ...
  if (candidates.length !== 1) {
    corrupt("FLP playlist record size is missing or ambiguous without a version.");
  }
  return candidates[0]!;
}
...
function playlistMaxTrack(version: FlpVersion | undefined): number {
  if (
    version !== undefined &&
    (version.major < 12 ||
      (version.major === 12 && (version.minor < 9 || (version.minor === 9 && version.patch < 1))))
  ) {
...
function decodePlaylistOffset(value: number): number | null {
  return value === PLAYLIST_UNSET_OFFSET || value === LEGACY_PLAYLIST_UNSET_OFFSET ? null : value;
}
...
function decodePlaylistClips(
  ...
  for (let offset = 0; offset < payload.byteLength; offset += recordSize) {
    ...
    const startOffset = view.getUint32(offset + 24, true),
      endOffset = view.getUint32(offset + 28, true);
    clips.push({
      position: view.getUint32(offset, true),
      length: view.getUint32(offset + 8, true),
      patternId,
...
function buildArrangements(
  ...
  const arrangements: FlpArrangement[] = [],
    byId = new Map<number, FlpArrangement>(),
    patternIds = new Set(patterns.map((pattern: FlpPattern): number => pattern.id)),
    hasArrangementMarkers = events.some(
      (event: FlpEvent): boolean => event.opcode === 0x63 && event.kind === "u16",
    ),
    trackDataOpcode = version !== undefined && version.major < 25 ? 0xde : 0xee,
    trackNameOpcode = version !== undefined && version.major < 25 ? 0xdf : 0xef;
  ...
  for (const event of events) {
    ...
    if (event.opcode === trackDataOpcode && event.kind === "blob" && current !== undefined) {
      current.tracks.push({ index: current.tracks.length });
      continue;
    }
    if (event.opcode === trackNameOpcode && event.kind === "blob" && current !== undefined) {
      const track = current.tracks.at(-1)!,
        name = decodeName(event.payload, legacyText);
      if (track !== undefined && name !== undefined) {
        track.name = name;
      }
      continue;
    }
    if (event.opcode === 0xe9 && event.kind === "blob" && current !== undefined) {
      current.clips.push(...decodePlaylistClips(event.payload, version, patternIds));
    }
  }
  return arrangements;
}
...
function findTempo(events: readonly FlpEvent[]): number {
  ...
  let coarse: number | undefined, fine: number | undefined;
  for (const event of events) {
    if (event.opcode === 0x42 && event.kind === "u16" && coarse === undefined) {
      coarse = event.value;
    } else if (event.opcode === 0x5d && event.kind === "u16" && fine === undefined) {
...
function findBeatsPerBar(events: readonly FlpEvent[]): number {
  let numerator: number | undefined, denominator: number | undefined;
  for (const event of events) {
    if (event.opcode === 0x11 && event.kind === "u8" && numerator === undefined) {
      numerator = event.value;
    } else if (event.opcode === 0x12 && event.kind === "u8" && denominator === undefined) {
...
function findCurrentArrangementId(events: readonly FlpEvent[]): number | undefined {
  for (const event of events) {
    if (event.opcode === 0x64 && event.kind === "u16") {
      return event.value;
    }
  }
  return undefined;
...
function parseFlpProjectInternal(buffer: ArrayBuffer): FlpProject {
  ...
  offset += 2;
  if (ppq === 0) {
    corrupt("FLP project PPQ must be greater than zero.");
  }
  ...
  const events = readEvents(buffer, view, offset, dataEnd),
    version = parseVersion(events),
    legacyText = usesLegacyText(version),
    channels = buildChannels(events, legacyText),
    patterns = buildPatterns(events, version, legacyText),
    arrangements = buildArrangements(events, version, patterns, legacyText),
    project: FlpProject = {
      ppq,
...
export function parseFlpProject(buffer: ArrayBuffer): FlpProject {
  try {
    ...
    if (error instanceof FlpImportError) {
      throw error;
    }
    const detail = error instanceof Error ? ` ${error.message}` : "";
    throw new FlpImportError("corrupt", `Unsupported or corrupt FLP structure.${detail}`);
  }
}
