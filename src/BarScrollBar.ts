...
export class BarScrollBar {
  ...
  private readonly _notches: SVGSVGElement = SVG.svg({
    "pointer-events": "none",
  });
  ...
  public readonly container: HTMLElement = HTML.div(
    {
      class: "barScrollBar",
      style:
        "width: 512px; height: 20px; overflow: hidden; position: relative;",
    },
    this._svg,
  );
  ...
  constructor(private _doc: SongDocument) {
    ...
    this._rightHighlight.setAttribute(
      "d",
      `M ${this._editorWidth - tip} ${center} L ${this._editorWidth - base} ${center + arrowHeight} L ${this._editorWidth - base} ${center - arrowHeight} z`,
    );
  ...
  public render(): void {
    this._notchSpace =
      (this._editorWidth - 1) /
      Math.max(this._doc.trackVisibleBars, this._doc.song.barCount);
