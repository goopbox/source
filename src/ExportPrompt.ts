...
import { type Preset, EditorConfig } from "./EditorConfig.js";
...
export class ExportPrompt implements Prompt {
  ...
  private readonly _progressBar: HTMLProgressElement = progress({
    style: "display: none;",
    max: "1",
  });
  private readonly _statusMessage: HTMLDivElement = div({
  ...
  private readonly _whenFormatChanged = (): void =>
  ...
  constructor(
    ...
    forcedFormat: ExportFormat | null = null,
  ) {
    ...
    if (
      this._doc.song.loopStart + this._doc.song.loopLength ==
    ...
    if (forcedFormat != null) {
      ...
      try {
        const lastExportFormat: string | null =
          window.localStorage.getItem("exportFormat");
        if (
  ...
  public cleanUp = (): void => {
    ...
    this._loopDropDown.removeEventListener(
      "blur",
      ExportPrompt._validateNumber,
    );
  ...
  private _updateSongRangeControls(): void {
  ...
  private async _synthesize(sampleRate: number): Promise<{
  ...
  private async _exportToWav(): Promise<void> {
    ...
    const { recordedSamplesL, recordedSamplesR } =
      await this._synthesize(sampleRate);
    ...
    if (bytesPerSample > 1) {
      ...
      for (let i: number = 0; i < sampleFrames; i++) {
        let valL: number = Math.floor(
          Math.max(-1, Math.min(1, recordedSamplesL[i])) * range,
        );
        let valR: number = Math.floor(
          Math.max(-1, Math.min(1, recordedSamplesR[i])) * range,
        );
        if (bytesPerSample == 2) {
      ...
      for (let i: number = 0; i < sampleFrames; i++) {
        ...
        let valR: number = Math.floor(
          Math.max(-1, Math.min(1, recordedSamplesR[i])) * 127 + 128,
        );
  ...
  private _exportToMp3(): void {
    const whenEncoderIsAvailable = async (): Promise<void> => {
      ...
      const mp3encoder: any = new lamejs.Mp3Encoder(
        channelCount,
        sampleRate,
        kbps,
      );
  ...
  private _exportToMidi(): void {
    ...
    const midiTicksPerPart: number =
      midiTicksPerEditorTick * Config.ticksPerPart;
    ...
    const microsecondsPerBeat: number = Math.round(
      microsecondsPerMinute / beatsPerMinute,
    );
    ...
    for (
      ...
      loopIndex < Number(this._loopDropDown.value);
      loopIndex++
    ) {
      for (
        let bar: number = song.loopStart;
        bar < song.loopStart + song.loopLength;
        bar++
      ) {
        unrolledBars.push(bar);
      }
    ...
    if (this._enableOutro.checked) {
      for (
        let bar: number = song.loopStart + song.loopLength;
        bar < song.barCount;
        bar++
      ) {
        unrolledBars.push(bar);
    ...
    for (const track of tracks) {
      ...
      const writeControlEvent = function (
      ...
      if (isMeta) {
        ...
        const isMinor: boolean =
          Config.scales[song.scale].flags[3] &&
          !Config.scales[song.scale].flags[4];
        ...
        for (
          let loopIndex: number = 0;
          loopIndex < parseInt(this._loopDropDown.value);
          loopIndex++
        ) {
          ...
          writer.writeMidiAscii(
            loopIndex < Number(this._loopDropDown.value) - 1
              ? "Loop Repeat"
              : "Loop End",
          );
        }
        ...
        writeEventTime(0);
        writeControlEvent(
          MidiControlEventMessage.setParameterMSB,
          pitchBendRange,
        ); // pitch bend semitone range
        writeEventTime(0);
        ...
        function writeInstrumentSettings(instrumentIndex: number): void {
          const instrument: Instrument =
            song.channels[channel].instruments[instrumentIndex];
          const preset: Preset | null = EditorConfig.valueToPreset(
            instrument.preset,
          );
          ...
          if (prevInstrumentIndex != instrumentIndex) {
            ...
            if (!isDrumset) {
              ...
              if (preset != null && preset.midiProgram != undefined) {
                ...
                if (
                  ...
                  if (
                    ExportPrompt.midiChipInstruments.length >
                    instrument.chipWave
                  ) {
                    instrumentProgram =
                      ExportPrompt.midiChipInstruments[instrument.chipWave];
                  }
                } else if (
            ...
            // Instrument volume:
            writeEventTime(barStartTime);
            let instrumentVolume: number = volumeMultToMidiVolume(
            ...
            // Instrument pan:
            writeEventTime(barStartTime);
            let instrumentPan: number =
              (instrument.pan / Config.panCenter - 1) * 0x3f + 0x40;
            writeControlEvent(
          ...
          if (pattern != null) {
            ...
            if (instrument.getChord().customInterval) {
              if (
                ...
                console.error(
                  "Unrecognized instrument type for harmonizing arpeggio: " +
                    instrument.type,
                );
              }
            }
            ...
            for (
              ...
              noteIndex < pattern.notes.length;
              noteIndex++
            ) {
              const note: Note = pattern.notes[noteIndex];

              const noteStartTime: number =
                barStartTime + note.start * midiTicksPerPart;
              ...
              const toneCount: number = Math.min(
                polyphony,
                note.pitches.length,
              );
              const velocity: number = isDrumset
                ? Math.max(
                    1,
                    Math.round(
                      (defaultNoteVelocity * note.pins[0].size) /
                        Config.noteSizeMax,
                    ),
                  )
                : defaultNoteVelocity;
              ...
              if (!isDrumset) {
                ...
                for (
                  ...
                  maxPitchOffset = Math.min(
                    maxPitchOffset,
                    interval + pitchBendRange,
                  );
                  minPitchOffset = Math.max(
                    minPitchOffset,
                    interval - pitchBendRange,
                  );
                }
                ...
                pitchOffset = Math.min(
                  maxPitchOffset,
                  Math.max(minPitchOffset, pitchOffset),
                );
              }
              ...
              for (
                ...
                for (let midiTick: number = 0; midiTick < length; midiTick++) {
                  ...
                  const linearSize: number = lerp(
                    pinSize,
                    nextPinSize,
                    midiTick / length,
                  );
                  const linearInterval: number = lerp(
                    pinInterval,
                    nextPinInterval,
                    midiTick / length,
                  );
                  ...
                  const interval: number =
                    linearInterval * intervalScale - pitchOffset;

                  const pitchBend: number = Math.max(
                    0,
                    Math.min(
                      0x3fff,
                      Math.round(0x2000 * (1.0 + interval / pitchBendRange)),
                    ),
                  );
                  ...
                  const expression: number = Math.min(
                    ...
                    Math.round(
                      volumeMultToMidiExpression(
                        Synth.noteSizeToVolumeMult(linearSize),
                      ),
                    ),
                  );
                  ...
                  if (expression != prevExpression && !isDrumset) {
                    writeEventTime(midiTickTime);
                    writeControlEvent(
                      MidiControlEventMessage.expressionMSB,
                      expression,
                    );
                  ...
                  for (
                    ...
                    if (isDrumset) {
                      ...
                      if (nextPitch < 0 || nextPitch >= drumsetMap.length)
                        throw new Error(
                          "Could not find corresponding drumset pitch. " +
                            nextPitch,
                        );
                      ...
                      if (
                        ...
                        const midiTicksSinceBeat =
                          (midiTickTime - barStartTime) % midiTicksPerBeat;
                        const midiTicksPerArpeggio =
                          (Config.rhythms[song.rhythm].ticksPerArpeggio *
                            midiTicksPerPart) /
                          Config.ticksPerPart;
                        const arpeggio: number = Math.floor(
                          midiTicksSinceBeat / midiTicksPerArpeggio,
                        );
                        nextPitch =
                          note.pitches[
                            toneIndex +
                      ...
                      nextPitch =
                        channelRoot + nextPitch * intervalScale + pitchOffset;
                      if (
                        preset != null &&
                        preset.midiSubharmonicOctaves != undefined
                      ) {
              ...
              const noteEndTime: number =
                barStartTime + note.end * midiTicksPerPart;
      ...
      writer.rewriteUint32(
        trackStartIndex,
        writer.getWriteIndex() - trackStartIndex - 4,
      );
    }
  ...
  private _exportToGoop(): void {
    const songData: Uint8Array = this._doc.song.toBinary();
    const blob: Blob = new Blob([new Uint8Array(songData)], {
