...
export class OctaveScrollBar {
  ...
  public readonly container: HTMLDivElement = HTML.div(
    {
      ...
      style:
        "width: 20px; height: 100%; overflow: hidden; position: relative; flex-shrink: 0;",
    },
    this._svg,
  );
  ...
  constructor(private _doc: SongDocument) {
    ...
    this._downHighlight.setAttribute(
      "d",
      `M ${center} ${this._editorHeight - tip} L ${center + arrowWidth} ${this._editorHeight - base} L ${center - arrowWidth} ${this._editorHeight - base} z`,
    );
  ...
  private _onPointerDown = (_event: PointerEvent): void => {
    this._mouseY =
      this._pointers.latest.getPointInNormalized(this.container).y *
      this._editorHeight;
    if (!this._doc.song.getChannelIsPitch(this._doc.channel)) return;
    this._updatePreview();

    if (
      this._mouseY >= this._barBottom - this._barHeight &&
      this._mouseY <= this._barBottom
    ) {
      this._dragging = true;
      this._change = null;
  ...
  private _onPointerMove = (_event: PointerEvent): void => {
    ...
    if (this._dragging) {
      ...
      const scrollableOctaves: number =
      ...
      const currentOctave: number = this._doc.getBaseVisibleOctave(
        this._doc.channel,
      );
  ...
  private _onPointerUp = (_event: PointerEvent): void => {
    if (this._doc.song.getChannelIsPitch(this._doc.channel)) {
      if (this._dragging) {
  ...
  private _documentChanged = (): void => {
    ...
    if (
      ...
      this._renderedVisibleOctaveCount = visibleOctaveCount;
      this._barHeight =
        this._octaveHeight * visibleOctaveCount + this._notchHeight;
      ...
      this._handleHighlight.setAttribute(
        "y",
        String(this._barBottom - this._barHeight),
      );
    }
    this._updatePreview();
  };
