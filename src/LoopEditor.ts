...
export class LoopEditor {
  ...
  private readonly _loop: SVGPathElement = SVG.path({
    fill: "none",
    stroke: ColorConfig.accent,
    "stroke-width": 4,
  });
  private readonly _highlight: SVGPathElement = SVG.path({
  ...
  private _updateCursorStatus(): void {
    ...
    if (
      bar > this._doc.song.loopStart - 0.25 &&
      bar < this._doc.song.loopStart + this._doc.song.loopLength + 0.25
    ) {
  ...
  private _whenCursorMoved(): void {
    if (this._mouseDown) {
      ...
      if (this._doc.synth.loopRepeatCount != 0) {
  ...
  private _updatePreview(): void {
    ...
    if (showHighlight) {
      ...
      let highlightStop: number =
        (this._doc.song.loopStart + this._doc.song.loopLength) * this._barWidth;
      if (this._cursor.mode == this._startMode) {
  ...
  private _render(): void {
    ...
    if (
      ...
      const editorWidth =
        ...
        this._barWidth * this._doc.song.barCount;
      this.container.style.width = editorWidth + "px";
      this._svg.setAttribute(
        "width",
        editorWidth - TrackEditor.channelNumberWidth + "",
      );
    }
