...
import { Note, type Pattern } from "../synth/synth.js";
...
export class SongPerformance {
  ...
  public _channelIsDrum = false;
  public _channelOctave = -1;
  public _songKey = -1;
  public _pitchesAreTemporary = false;
  public readonly _recentlyAddedPitches: number[] = []; // Pitches that are rapidly added then removed within a minimum rhythm duration wouldn't get recorded until I explicitly track recently added notes and check if any are no longer held.

  public _songLengthWhenRecordingStarted = -1;
  public _playheadPart = -1;
  public _playheadPattern: Pattern | null = null;
  public _pitchesChanged = false;
  public _lastNote: Note | null = null;
  public _recordingChange: ChangeGroup | null = null;
  public _lowLatencyPreferredUntilTime = 0;
  ...
  public play(): void {
    ...
    if (
      this._doc.synth.preferLowerLatency &&
      performance.now() >= this._lowLatencyPreferredUntilTime
    ) {
  ...
  public pause(): void {
    ...
    if (this._recordingChange != null) {
      if (
        this._doc.song.barCount > this._songLengthWhenRecordingStarted &&
        !this._lastBarHasPatterns()
      ) {
  ...
  public _getCurrentPlayheadPart(): number {
    const currentPart: number =
        this._doc.synth.playhead * this._doc.song.beatsPerBar * Config.partsPerBeat,
      minDivision: number = this._getMinDivision();
    return Math.round(currentPart / minDivision) * minDivision;
  }

  public _lastBarHasPatterns(): boolean {
  ...
  public _updateRecordedNotes(): boolean {
    if (this._recordingChange == null) {
      return false;
    }
    if (
      ...
      this._doc.song.getChannelIsAutomation(this._doc.synth.liveInputChannel)
    ) {
      this.abortRecording();
      return false;
    }
    ...
    const partsPerBar: number = this._doc.song.beatsPerBar * Config.partsPerBeat,
      oldPart: number = this._playheadPart % partsPerBar,
      oldBar: number = Math.floor(this._playheadPart / partsPerBar),
      oldPlayheadPart: number = this._playheadPart;
    this._playheadPart = this._getCurrentPlayheadPart();
    const newPart: number = this._playheadPart % partsPerBar,
      newBar: number = Math.floor(this._playheadPart / partsPerBar);
    ...
    for (let bar: number = oldBar; bar <= newBar; bar++) {
      ...
      const startPart: number = bar === oldBar ? oldPart : 0,
        endPart: number = bar === newBar ? newPart : partsPerBar;
      if (startPart === endPart) {
        break;
      }
      if (
        this._lastNote != null &&
        !this._pitchesChanged &&
        startPart > 0 &&
        this._doc.synth.liveInputPitches.length > 0
      ) {
        this._recordingChange.append(
          new ChangePinTime(
            this._doc,
            this._lastNote,
            1,
            endPart,
            this._lastNote.continuesLastPattern,
          ),
        );
        ...
        while (noteStartPart < endPart) {
          ...
          if (
            ...
            if (this._playheadPattern == null) {
              this._doc.selection.erasePatternInBar(
                this._recordingChange,
                this._doc.synth.liveInputChannel,
                bar,
              );
              this._recordingChange.append(
                new ChangeEnsurePatternExists(this._doc, this._doc.synth.liveInputChannel, bar),
              );
              this._playheadPattern = this._doc.song.getPattern(
                this._doc.synth.liveInputChannel,
                bar,
              );
            }
            if (this._playheadPattern == null) {
              throw new Error("Recording pattern is unavailable.");
            }
            this._lastNote = new Note(
              -1,
              noteStartPart,
              noteEndPart,
              Config.noteSizeMax,
              this._doc.song.getChannelIsNoise(this._doc.synth.liveInputChannel),
            );
            ...
            this._recordingChange.append(
              new ChangeNoteAdded(
                this._doc,
                this._playheadPattern,
                this._lastNote,
                this._playheadPattern.notes.length,
              ),
            );
  ...
  public setTemporaryPitches(pitches: number[], duration: number): void {
    ...
    this._doc.synth.setLiveInputPitches(pitches.slice(0, Config.maxChordSize), duration, true);
  ...
  public preferLowLatency(): void {
    ...
    this._lowLatencyPreferredUntilTime = performance.now() + 10_000.0;
  }
  ...
  public addPerformedPitch(pitch: number): void {
    ...
    const compositionOffset: number = this._doc.song.composingKey - this._doc.song.key,
      scaleIndex: number =
    ...
    if (
      ...
      !Config.scales[this._doc.song.scale]!.flags[scaleIndex]!
    ) {
      return;
    }
    if (this._doc.synth.liveInputPitches.indexOf(pitch) === -1) {
      const pitches: number[] = [...this._doc.synth.liveInputPitches, pitch];
      this._pitchesChanged = true;
      while (pitches.length > Config.maxChordSize) {
        pitches.shift();
      }
      this._doc.synth.setLiveInputPitches(pitches, Number.MAX_SAFE_INTEGER, true);
  ...
  public removePerformedPitch(pitch: number): void {
    this._updateRecordedNotes();
    const pitches: number[] = this._doc.synth.liveInputPitches.filter(
      (heldPitch: number): boolean => heldPitch !== pitch,
    );
    if (pitches.length === this._doc.synth.liveInputPitches.length) {
      return;
    }
    this._doc.synth.setLiveInputPitches(pitches, Number.MAX_SAFE_INTEGER, false);
    this._pitchesChanged = true;
  }

  public clearAllPitches(): void {
    this._updateRecordedNotes();
    this._doc.synth.setLiveInputPitches([], 0, false);
    this._pitchesChanged = true;
  }

  public _documentChanged = (): void => {
    const isDrum: boolean = this._doc.song.getChannelIsNoise(this._doc.channel),
      octave: number = this._doc.song.channels[this._doc.channel]!.octave;
    if (
      ...
      this._doc.synth.setLiveInputChannel(this._doc.channel);
      this._channelIsDrum = isDrum;
      this._channelOctave = octave;
      this._songKey = this._doc.song.key;
      this.clearAllPitches();
    }
    this._doc.synth.setLiveInputInstruments(
      this._doc.song.channels[this._doc.channel]!.instruments.map((_instrument, index) => index),
    );
  };
}
