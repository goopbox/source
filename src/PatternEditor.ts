...
export class PatternEditor {
  private readonly _svgNoteBackground: SVGPatternElement = SVG.pattern({
    x: "0",
    y: "0",
    patternUnits: "userSpaceOnUse",
  });
  private readonly _svgDrumBackground: SVGPatternElement = SVG.pattern({
    x: "0",
    y: "0",
    patternUnits: "userSpaceOnUse",
  });
  ...
  private readonly _pointers: EasyPointers = new EasyPointers(this._svg, {
  ...
  constructor(
    ...
    if (!this._interactive) {
      this.container.classList.add(
        "pattern-preview",
        this._barOffset < 0
          ? "pattern-preview-previous"
          : "pattern-preview-next",
      );
    ...
    } else this._blurSvg = null;
    this._svgNoteBackground.setAttribute(
      "id",
      "patternEditorNoteBackground" + this._barOffset,
    );
    this._svgDrumBackground.setAttribute(
      "id",
      "patternEditorDrumBackground" + this._barOffset,
    );
  ...
  private _getMaxDivision(): number {
    const rhythmStepsPerBeat: number =
  ...
  private _updateCursorStatus(): void {
    ...
    this._cursor.part =
      Math.floor(
        Math.max(
          0,
          Math.min(partsPerPattern - minDivision, this._cursor.exactPart),
        ) / minDivision,
      ) * minDivision;
    ...
    if (this._pattern != null) {
      ...
      for (const note of this._pattern.notes) {
        if (note.end <= cursorPartForMatching) {
          ...
          note.start <= cursorPartForMatching &&
          note.end > cursorPartForMatching
        ) {
    ...
    if (this._cursor.curNote != null) {
      ...
      for (let j: number = 1; j < this._cursor.curNote.pins.length; j++) {
        ...
        const leftSide: number =
          this._partWidth * (this._cursor.curNote.start + prevPin.time);
        const rightSide: number =
          this._partWidth * (this._cursor.curNote.start + nextPin.time);
        if (this._mouseX > rightSide) continue;
        if (this._mouseX < leftSide) throw new Error();
        const intervalRatio: number =
          (this._mouseX - leftSide) / (rightSide - leftSide);
        const arc: number =
          Math.sqrt(1.0 / Math.sqrt(4.0) - Math.pow(intervalRatio - 0.5, 2.0)) -
          0.5;
        const bendHeight: number = Math.abs(
          nextPin.interval - prevPin.interval,
        );
        interval =
          prevPin.interval * (1.0 - intervalRatio) +
          nextPin.interval * intervalRatio;
      ...
      if (!this._doc.song.getChannelIsNoise(this._doc.channel)) {
        ...
        for (let i: number = 0; i < this._cursor.curNote.pitches.length; i++) {
          const distance: number = Math.abs(
            this._cursor.curNote.pitches[i] - mousePitch + 0.5,
          );
      ...
      if (this._cursor.pitchIndex != -1) {
        ...
        for (let i: number = 0; i < this._cursor.curNote.pitches.length; i++) {
          ...
          if (
            -15.5 < distance &&
            (distance < 0 || this._mouseY < this._editorHeight * 0.25)
          ) {
            this._cursor.overrideScrolling = true;
      ...
      const defaultLength: number =
        this._copiedPins[this._copiedPins.length - 1].time;
      const fullBeats: number = Math.floor(
        this._cursor.part / Config.partsPerBeat,
      );
      ...
      if (defaultLength == 1) {
        ...
        if (maxDivision < Config.partsPerBeat && modMouse > maxDivision) {
          this._cursor.start +=
            Math.floor(modMouse / maxDivision) * maxDivision;
        }
      } else {
        this._cursor.start = fullBeats * Config.partsPerBeat;
        let division =
  ...
  private _cursorAtEndOfSelection(): boolean {
    return (
      ...
      this._doc.selection.patternSelectionActive &&
      this._cursor.pitchIndex == -1 &&
      this._doc.selection.patternSelectionEnd - 1.25 <=
  ...
  private _findMousePitch(pixelY: number): number {
    return (
      Math.max(
        ...
        Math.min(
          this._pitchCount - 1,
          this._pitchCount - pixelY / this._pitchHeight,
        ),
      ) + this._octaveOffset
    );
  }
  ...
  private _snapToPitch(guess: number, min: number, max: number): number {
    ...
    const scaleIndex = (pitch: number): number =>
  ...
  public movePlayheadToMouse(): boolean {
    if (this._pointers.latest.isPresent) {
      this._doc.synth.playhead =
        this._doc.bar + this._barOffset + this._mouseX / this._editorWidth;
      return true;
    }
    return false;
  }
  ...
  private _animatePlayhead = (_timestamp: number): void => {
    ...
    if (
      ...
      ((this._pattern != null &&
        this._doc.song.getPattern(
          this._doc.channel,
          Math.floor(this._doc.synth.playhead),
        ) == this._pattern) ||
  ...
  private _onPointerDown = (event: PointerEvent): void => {
    ...
    this._lastChangeWasPatternSelection = this._doc.lastChangeWas(
      this._changePatternSelection,
    );
  ...
  private _addNewNoteAtPointer(sequence: ChangeSequence): void {
    ...
    sequence.append(
      new ChangeNoteAdded(this._doc, pattern, note, this._cursor.curIndex),
    );
  ...
  private _onPointerMove = (event: PointerEvent): void => {
    ...
    if (
      ...
      if (
        ...
        if (this._mouseDragging && pattern != null) {
          ...
          const notesInScale: number = Config.scales[
            this._doc.song.scale
          ].flags.filter((x) => x).length;
          const pitchRatio: number = this._doc.song.getChannelIsNoise(
            this._doc.channel,
          )
            ? 1
            : 12 / notesInScale;
          const draggedParts: number =
            Math.round(
              (this._mouseX - this._mouseXStart) /
                (this._partWidth * minDivision),
            ) * minDivision;
          const draggedTranspose: number = Math.round(
            (this._mouseYStart - this._mouseY) /
              (this._pitchHeight * pitchRatio),
          );
          sequence.append(
        ...
        if (this._mouseDragging) {
          let start: number = Math.max(
            ...
            Math.min(
              ...
              Math.floor(this._cursor.exactPart / Config.partsPerBeat) *
                Config.partsPerBeat,
            ),
          );
          ...
          if (currentPart < start) {
            ...
            for (
              let beat: number = 0;
              beat <= this._doc.song.beatsPerBar;
              beat++
            ) {
        ...
        if (this._cursor.curNote == null) {
          ...
          if (start < end) {
            ...
            if (pattern == null) throw new Error();
            sequence.append(
              new ChangeNoteTruncate(this._doc, pattern, start, end),
            );
            let i: number;
            ...
            sequence.append(
              new ChangeNoteAdded(this._doc, pattern, theNote, i),
            );
          ...
          const shift: number =
            (this._mouseX - this._mouseXStart) / this._partWidth;

          const shiftedPin: NotePin =
            this._cursor.curNote.pins[this._cursor.nearPinIndex];
          let shiftedTime: number =
            Math.round(
              (this._cursor.curNote.start + shiftedPin.time + shift) /
                minDivision,
            ) * minDivision;
          ...
          if (
            (shiftedTime <= this._cursor.curNote.start &&
              this._cursor.nearPinIndex ==
                this._cursor.curNote.pins.length - 1) ||
            (shiftedTime >= this._cursor.curNote.end &&
              this._cursor.nearPinIndex == 0)
          ) {
            sequence.append(
              new ChangeNoteAdded(
                this._doc,
            ...
            this._dragSize =
              this._cursor.curNote.pins[this._cursor.nearPinIndex].size;
            this._dragVisible = true;
          ...
          const bendPart: number =
            Math.max(
              ...
              Math.min(
                ...
                Math.round(this._mouseX / (this._partWidth * minDivision)) *
                  minDivision,
              ),
            ) - this._cursor.curNote.start;
          ...
          for (let i: number = 1; i < this._cursor.curNote.pins.length; i++) {
            ...
            const sizeRatio: number =
              (bendPart - prevPin.time) / (nextPin.time - prevPin.time);
            bendSize = Math.round(
              prevPin.size * (1.0 - sizeRatio) +
                nextPin.size * sizeRatio +
                (this._mouseYStart - this._mouseY) /
                  (75.0 / Config.noteSizeMax),
            );
          ...
          this._dragPitch =
            this._cursor.curNote.pitches[this._cursor.abovePitchIndex] +
            bendInterval;
    ...
    if (!(event.pointer!.isDown && this._cursor.valid && continuousState)) {
      ...
      this._pointers.preventTouchGestureScrolling =
        (this._cursor.valid && this._cursor.overrideScrolling) ||
  ...
  private _onPointerUp = (_event: PointerEvent | null): void => {
      ...
      if (this._draggingSelectionContents) {
        ...
        if (this._cursor.pitchIndex == -1) {
          ...
          if (!this._doc.synth.playing) {
            ...
            this._doc.performance.setTemporaryPitches(
              this._cursor.curNote.pitches,
              duration,
            );
          }
  ...
  private _onPointerCancel = (_event: PointerEvent | null): void => {
    const continuousState: boolean = this._doc.lastChangeWas(this._dragChange);
    if (continuousState && this._dragChange != null) {
      this._dragChange.undo();
    }
  ...
  private _setPatternSelection(change: UndoableChange): void {
    ...
    if (!this._doc.hasRedoHistory()) {
      this._doc.record(
        this._changePatternSelection,
        this._lastChangeWasPatternSelection,
      );
    } else {
  ...
  private _updatePreview(): void {
    if (this._pointers.latest.isTouch) {
      if (
        ...
        const y: number = this._pitchToPixelHeight(
          this._dragPitch - this._octaveOffset,
        );
        ...
        pathString +=
          ...
          prettyNumber(x) +
          " " +
          prettyNumber(y - radius * (this._dragSize / sizeMax)) +
          " ";
        pathString +=
          "L " +
          prettyNumber(x) +
          " " +
          prettyNumber(y - radius * (this._dragSize / sizeMax) - height) +
          " ";
        pathString +=
          "M " +
          prettyNumber(x) +
      ...
      if (
        ...
        if (this._cursorAtStartOfSelection()) {
          const center: number =
            this._partWidth * this._doc.selection.patternSelectionStart;
          ...
          const center: number =
            this._partWidth * this._doc.selection.patternSelectionEnd;
          const left: string = prettyNumber(center - 4);
          const right: string = prettyNumber(center + 4);
          const bottom: number = this._pitchToPixelHeight(-0.5);
          this._svgPreview.setAttribute(
          ...
          const right: string = prettyNumber(
  ...
  public render(): void {
    ...
    this._partWidth =
      this._editorWidth / (this._doc.song.beatsPerBar * Config.partsPerBeat);
    ...
    if (
      ...
      this._backgroundDrumRow.setAttribute(
        "height",
        "" + (this._pitchHeight - 2),
      );
      for (let j: number = 0; j < Config.pitchesPerOctave; j++) {
        ...
        const y: number =
          (Config.pitchesPerOctave - j) % Config.pitchesPerOctave;
    ...
    for (let j: number = 0; j < Config.pitchesPerOctave; j++) {
      this._backgroundPitchRows[j].style.visibility = Config.scales[
        this._doc.song.scale
      ].flags[j]
        ? "visible"
        : "hidden";
  ...
  private _redrawNotePatterns(): void {
    ...
    for (
      let channel: number = this._doc.song.getChannelCount() - 1;
      channel >= 0;
      channel--
    ) {
        ...
        for (let i: number = 0; i < note.pitches.length; i++) {
          ...
          if (note.continuesLastPattern) {
            ...
            arrowPath =
              ...
              " " +
              prettyNumber(
                this._pitchToPixelHeight(pitch - this._octaveOffset) -
                  0.1 * arrowHeight,
              );
            arrowPath +=
              "L " +
