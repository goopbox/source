...
import type { Song } from "../synth/synth.js";
...
const mix = (first: string, second: string, firstWeight: number): string => {
    const parse = (color: string): [number, number, number] => {
      ...
      toLinear = (component: number): number => {
        component /= 255;
        return component <= 0.04045 ? component / 12.92 : ((component + 0.055) / 1.055) ** 2.4;
      },
      toOklab = (color: [number, number, number]): [number, number, number] => {
        const [red, green, blue] = color.map(toLinear) as [number, number, number],
          lightness = Math.cbrt(0.4122214708 * red + 0.5363325363 * green + 0.0514459929 * blue),
      ...
      toSrgb = (component: number): number =>
        255 * (component <= 0.0031308 ? 12.92 * component : 1.055 * component ** (1 / 2.4) - 0.055),
      [red, green, blue] = parse(first);
    ...
    const firstOklab = toOklab([red, green, blue]),
      secondOklab = toOklab(parse(second)),
      [firstLightness, firstGreenRed, firstBlueYellow] = firstOklab,
      [secondLightness, secondGreenRed, secondBlueYellow] = secondOklab,
      lightness: number = firstLightness * firstWeight + secondLightness * (1 - firstWeight),
      greenRed: number = firstGreenRed * firstWeight + secondGreenRed * (1 - firstWeight),
      blueYellow: number = firstBlueYellow * firstWeight + secondBlueYellow * (1 - firstWeight),
      long = (lightness + 0.3963377774 * greenRed + 0.2158037573 * blueYellow) ** 3,
      medium = (lightness - 0.1055613458 * greenRed - 0.0638541728 * blueYellow) ** 3,
      short = (lightness - 0.0894841775 * greenRed - 1.291485548 * blueYellow) ** 3,
  ...
  channelCss = (type: string, colors: readonly ColorGradient[], channelCount: number): string =>
    Array.from({ length: channelCount }, (_, index) => {
      const [start, end] = colors[index % colors.length]!,
        noteStart = mix(start, "#fff", 0.85),
        noteEnd = mix(end, "#fff", 0.85),
        name = `--${type}${index + 1}`;
      return [
  ...
  themeCss = (theme: Theme): { css: string; widgetBackground: string } => {
...
export class ColorConfig {
  static #nextGradientId = 0;
  static readonly #selectGradientText = new WeakMap<HTMLSelectElement, HTMLSpanElement>();

  public static readonly themes: Readonly<Record<string, Theme>> = {
    "GoopBox Dark": {
      background: "#03040f",
      button: "#22334f",
      pitchRow: "#1c2433",
    ...
    return toNameMap(
      Array.from({ length: channelCount }, (_, index) => {
        const name = `${type}${index + 1}`,
          gradient = (purpose: string): ColorGradient => [
  ...
  public static svgGradient(
    ...
    const id = `channelGradient${this.#nextGradientId++}`;
    return {
  ...
  public static applyChannelColors(container: HTMLElement, colors: ChannelColors): void {
    for (const [name, gradient] of [
      ...
      container.style.setProperty(`--channel-${name}`, this.cssGradient(gradient));
    }
    ...
    for (const select of container.querySelectorAll("select")) {
      ...
      let text: HTMLSpanElement | undefined = this.#selectGradientText.get(select);
      if (text === undefined) {
        text = HTML.span({
          class: "channel-gradient-select-text",
          "aria-hidden": "true",
        });
        this.#selectGradientText.set(select, text);
      ...
      const hostBounds: DOMRect = select.parentElement!.getBoundingClientRect(),
        selectBounds: DOMRect = select.getBoundingClientRect();
  ...
  public static readonly pitchChannels: DictionaryArray<ChannelColors> = this.makeChannelColors(
    "pitch",
    Config.pitchChannelCountMax,
  );
  public static readonly noiseChannels: DictionaryArray<ChannelColors> = this.makeChannelColors(
    "noise",
  ...
  static readonly #styleElement: HTMLStyleElement = document.head.appendChild(
  ...
  public static setTheme(name: string): void {
    ...
    if (theme === undefined) {
      theme = this.themes["GoopBox Dark"]!;
    }
