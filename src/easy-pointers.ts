...
declare global {
  ...
  interface PointerEvent<T = unknown> {
...
const _buttonSet = new Set<`${PointerButton}`>([
...
const _indexFromButton = new Map<`${PointerButton}`, _ButtonIndex>([
  ...
  _buttonFromIndex = new Map<_ButtonIndex, PointerButton>(
    Array.from(_indexFromButton, (entry) => entry.reverse()) as [_ButtonIndex, PointerButton][],
  ),
  _bitmaskFromButton = new Map<`${PointerButton}`, _ButtonBitmask>([
  ...
  _buttonFromBitmask = new Map<_ButtonBitmask, PointerButton>(
    Array.from(_bitmaskFromButton, (entry) => entry.reverse()) as [_ButtonBitmask, PointerButton][],
  );
function _buttonSetFromBitmask(buttons: number): Set<PointerButton> {
  const result = new Set<PointerButton>();
  ...
  public distanceFrom(other: _Point | number, y?: number): number {
    if (typeof other === "number") {
      const dx: number = this.x - other,
        dy: number = this.y - (y ?? 0);
      return Math.sqrt(dx * dx + dy * dy);
    }
    const dx: number = this.x - other.x,
      dy: number = this.y - other.y;
    return Math.sqrt(dx * dx + dy * dy);
  }
...
class _Gesture {
  ...
  public deferringEvents = true;
  public dispatchingDeferredEvents = false;
  public pressingButtons: number = _ButtonBitmask.none;
  public hasMovedFromStart = false;
  public wasHeldStill = false;
...
export const activeModifierKeys = {
  alt: false,
  ctrl: false,
  meta: false,
  shift: false,
...
function updateActiveModifierKeys(event: KeyboardEvent | PointerEvent): void {
  activeModifierKeys.alt = event.altKey;
  activeModifierKeys.ctrl = event.ctrlKey;
  activeModifierKeys.meta = event.metaKey;
  activeModifierKeys.shift = event.shiftKey;
}

window.addEventListener("keydown", updateActiveModifierKeys, { capture: true });
window.addEventListener("keyup", updateActiveModifierKeys, { capture: true });
window.addEventListener("blur", () => {
  // The window may not receive keyup events after losing focus, so any
  // Active modifier state can no longer be trusted.
  activeModifierKeys.alt = false;
  activeModifierKeys.ctrl = false;
  activeModifierKeys.meta = false;
  activeModifierKeys.shift = false;
});

function verifyBooleanOption(
  options: EasyPointersOptions,
  name: keyof EasyPointersOptions,
...
// oxlint-disable no-use-before-define -- EasyPointers and _InternalPointer require each other.
export class EasyPointers<UserData = unknown> {
  static readonly #managedTargets = new WeakSet<HTMLElement | SVGElement>();
  ...
  public get target(): HTMLElement | SVGElement {
    return this.#target;
  }

  readonly #byId = new Map<number, _InternalPointer<UserData>>();
  public get byId(): ReadonlyMap<number, Pointer<UserData>> {
    return this.#byId;
  }

  public constructor(target: HTMLElement | SVGElement, options: EasyPointersOptions = {}) {
    if (!(target instanceof HTMLElement) && !(target instanceof SVGElement)) {
      throw new Error("EasyPointers: The target should be an HTMLElement or SVGElement.");
    }
    if (EasyPointers.#managedTargets.has(target)) {
      throw new Error(
        "EasyPointers: Cannot register EasyPointers twice for the same target element simultaneously. Try calling dispose() on the first one before creating another.",
      );
    }
    EasyPointers.#managedTargets.add(target);
    this.#target = target;
    if (
      options.listenForTheseButtons !== undefined &&
      typeof options.listenForTheseButtons[Symbol.iterator] !== "function"
    ) {
      throw new Error(
        "EasyPointers: The listenForTheseButtons option, if provided, should be an iterable object, such as an array, that contains pointer button names.",
      );
    }
    const listenForTheseButtons = new Set<`${PointerButton}`>(
      options.listenForTheseButtons ?? [PointerButton.primary],
    );
    for (const button of listenForTheseButtons) {
      if (!_buttonSet.has(button)) {
        throw new Error(
          `EasyPointers: Unrecognized pointer button name provided: ${button}. Valid values are: ${[
            ..._buttonSet,
          ]
            .map((s) => `"${s}"`)
            .join(", ")}`,
    ...
    const listenForTheseButtonIndices = new Set<_ButtonIndex>(
        [...listenForTheseButtons].map((button) => _indexFromButton.get(button)!),
      ),
      listenForTheseButtonsBitmask: number = [...listenForTheseButtons]
        .map((button) => _bitmaskFromButton.get(button)!)
        .reduce((acc, bit) => acc | bit, 0),
      listenForMultipleButtonsDown: boolean = verifyBooleanOption(
        options,
        "listenForMultipleButtonsDown",
        false,
      ),
      listenForMultiplePointers: boolean = verifyBooleanOption(
        options,
        "listenForMultiplePointers",
        false,
      ),
      listenForPossiblyAccidentalPointers: boolean = verifyBooleanOption(
        options,
        "listenForPossiblyAccidentalPointers",
        true,
      ),
      moveMinDistance: number = options.moveMinDistance ?? 15; // In my experience, 15 is sufficient for scrolling to trigger (and cancel the pointer) before this threshold is crossed.
    if (
      typeof moveMinDistance !== "number" ||
      moveMinDistance < 0 ||
      !Number.isFinite(moveMinDistance)
    ) {
      throw new Error(
        `EasyPointers: The moveMinDistance option, if provided, should be a nonnegative number value.`,
      );
    }
    const holdStillMinMillis: number = options.holdStillMinMillis ?? Infinity;
    if (
      typeof holdStillMinMillis !== "number" ||
      holdStillMinMillis <= 0 ||
      Number.isNaN(holdStillMinMillis)
    ) {
      throw new Error(
        `EasyPointers: The holdStillMinMillis option, if provided, should be a positive number value.`,
      );
    }
    this.deferInitialEvents = verifyBooleanOption(options, "deferInitialEvents", false);
    this.preventTouchGestureScrolling = verifyBooleanOption(
      options,
      "preventTouchGestureScrolling",
      false,
    );
    this.preventTextSelection = verifyBooleanOption(
      options,
      "preventTextSelection",
      listenForTheseButtons.has(PointerButton.primary),
    );
    this.preventContextMenu = verifyBooleanOption(
      options,
      "preventContextMenu",
      listenForTheseButtons.has(PointerButton.secondary),
    );
    this.preventMiddleButtonScrolling = verifyBooleanOption(
      options,
      "preventMiddleButtonScrolling",
      listenForTheseButtons.has(PointerButton.middle),
    );
    const stopEventsAfterTargetIsRemovedFromLayout: boolean = verifyBooleanOption(
      options,
      "stopEventsAfterTargetIsRemovedFromLayout",
      true,
    );

    this.#latest = new _InternalPointer(this, 0, "mouse");
    this.#latest._isInTarget = false;
    this.#latest._isPresent = false;
    this.#first = this.#latest;

    const stop = (event: Event): void => event.stopImmediatePropagation(),
      syncEventAndPointer = (event: PointerEvent, pointer: _InternalPointer<UserData>): void => {
        ...
        updateActiveModifierKeys(event);
      ...
      resetPresses = (pointer: _InternalPointer): void => {
      ...
      internalEnter = (event: PointerEvent): void => {
        ...
        if (stopEventsAfterTargetIsRemovedFromLayout && !elementIsInLayout(this.#target)) {
          if (this.#byId.has(event.pointerId)) {
            this.#dispatchPointerLeaveEvent(event.pointerId);
          }
          return stop(event);
        }
        if (!listenForMultiplePointers) {
          ...
          for (const pointer of this.#byId.values()) {
            if (
              pointer.id !== event.pointerId &&
              pointer.type !== event.pointerType &&
              !pointer._isDown
            ) {
              this.#dispatchPointerLeaveEvent(pointer.id);
            }
          }
        }
        if (!listenForMultiplePointers && this.#byId.size > 0) {
          return stop(event);
        }
        if (!listenForPossiblyAccidentalPointers && this.#byId.size === 0 && !event.isPrimary) {
          return stop(event);
        }

        // CHECK POINTER ASSUMPTIONS
        let pointer: _InternalPointer<UserData> | undefined = this.#byId.get(event.pointerId);
        ...
        this.#byId.set(event.pointerId, pointer);
        if (this.#byId.size === 1) {
          this.#first = pointer;
        }
      ...
      internalLeave = (event: PointerEvent): void => {
        // The enter and leave events aren't bubbling, so ordinarily listeners
        // Can assume that (event.target == event.currentTarget), but this is
        // Listening in the capture phase so we have to check that manually.
        if (event.target !== event.currentTarget) {
        ...
        // CHECK POINTER ASSUMPTIONS
        const pointer: _InternalPointer<UserData> | undefined = this.#byId.get(event.pointerId);
        if (!pointer || !pointer._isPresent) {
          return stop(event);
        }
        if (pointer._isDown) {
        ...
        pointer._verifyValidity();
        this.#byId.delete(event.pointerId);
      },
      internalMove = (event: PointerEvent): void => {
        // PREVENT EVENT DEFAULTS
        if (
          this.preventMiddleButtonScrolling &&
        ...
        // CHECK POINTER ASSUMPTIONS
        let pointer: _InternalPointer<UserData> | undefined = this.#byId.get(event.pointerId);
        const pointerIsDown = (): boolean =>
            (pointer?._isDown ?? false) || (pointer?._gesture?.deferringEvents ?? false),
          pointerPressingButtons = (): number =>
            (pointer?._gesture?.deferringEvents ?? false)
        ...
        let pressingButtons: number = pointerPressingButtons(),
          addedButton: _ButtonIndex = _ButtonIndex.none,
          removedButton: _ButtonIndex = _ButtonIndex.none;
        if (changedButtonIndex !== _ButtonIndex.none) {
          const changedButtonBitmask: number | undefined = _bitmaskFromButton.get(
            _buttonFromIndex.get(changedButtonIndex)!,
          );
          if (changedButtonBitmask === undefined) {
            throw new Error(
              `EasyPointers: Missing bitmask for button index: ${changedButtonIndex}`,
            );
          }
          ...
          if (pressingChangedButton) {
            if ((pressingButtons & changedButtonBitmask) !== 0) {
              this.dispatchPointerCancelEvent(event.pointerId);
              pointer = this.#byId.get(event.pointerId);
              pressingButtons = pointerPressingButtons();
            }
            addedButton = changedButtonIndex;
            pressingButtons |= changedButtonBitmask;
          }
        ...
        if (addedButton !== _ButtonIndex.none && !pointerIsDown()) {
          if (this.#dispatchPointerDownEvent(event, addedButton).defaultPrevented) {
        ...
        if (
          ...
          if (this.#dispatchPointerUpEvent(event, removedButton).defaultPrevented) {
            // Make sure the browser knows the app wanted to prevent the default behavior.
            event.preventDefault();
          }
          return stop(event);
        ...
        if (!pointer || !pointer._isPresent) {
          ...
          this.#dispatchPointerEnterEvent(event);
          pointer = this.#byId.get(event.pointerId);
          if (!pointer || !pointer._isPresent) {
            return stop(event);
          }
        ...
        if (pointerIsDown() && gesture && !gesture.hasMovedFromStart) {
          ...
          const zoom: number = window.outerWidth / (visualViewport?.width ?? window.innerWidth),
            dx: number = (event.clientX - gesture.startClientX) * zoom,
            dy: number = (event.clientY - gesture.startClientY) * zoom,
            distanceMoved: number = Math.sqrt(dx * dx + dy * dy);
        ...
        pointer._isInTarget = target.contains(
          document.elementFromPoint(event.clientX, event.clientY),
        );
        if (addedButton) {
          pointer._pressedButtons = pressingButtons;
        }
      ...
      internalDown = (event: PointerEvent): void => {
        // PREVENT EVENT DEFAULTS
        if (this.preventMiddleButtonScrolling && event.button === _ButtonIndex.middle) {
          event.preventDefault();
        }
        ...
        if (!listenForTheseButtonIndices.has(event.button)) {
          return stop(event);
        }
        ...
        if (!pointer || !pointer._isPresent) {
          if (!listenForMultiplePointers) {
            ...
            for (const otherPointer of this.#byId.values()) {
              if (otherPointer.type !== event.pointerType) {
                this.#dispatchPointerLeaveEvent(otherPointer.id);
              }
            }
          }
          this.#dispatchPointerEnterEvent(event);
        ...
        if (!(gesture?.dispatchingDeferredEvents ?? false)) {
          ...
          gesture.deferringEvents =
            this.deferInitialEvents ||
            (event.pointerType === "touch" && !this.preventTouchGestureScrolling);
          if (gesture.deferringEvents) {
            gesture.deferredEvents.push(event);
        ...
        // Listeners.
        if (!pointer._isDown && !pointer._isInTarget && pointer._isPresent) {
          throw new Error(
            "EasyPointers: A pointer that isn't down or in the target shouldn't be present.",
          );
        }
      },
      internalUp = (event: PointerEvent): void => {
        // PREVENT EVENT DEFAULTS: N/A
      ...
      internalCancel = (event: PointerEvent): void => {
        // PREVENT EVENT DEFAULTS: N/A
      ...
      touchPreventScrolling = (event: TouchEvent): void => {
      ...
      selectDragListener = (event: Event): void => {
      ...
      contextmenu = (event: PointerEvent): void => {
        if (this.preventContextMenu) {
          ...
          const pointer: _InternalPointer | undefined = this.#byId.get(event.pointerId);
          if (pointer && pointer._isDown) {
    ...
    target.addEventListener("touchstart", touchPreventScrolling as EventListener, {
      capture: true,
      passive: false,
    });
    target.addEventListener("touchmove", touchPreventScrolling as EventListener, {
      capture: true,
      passive: false,
    });
    target.addEventListener("selectstart", selectDragListener as EventListener, { capture: true });
    target.addEventListener("selectionchange", selectDragListener as EventListener, {
      capture: true,
    });
    target.addEventListener("dragstart", selectDragListener as EventListener, {
      capture: true,
    });
    target.addEventListener("contextmenu", contextmenu as EventListener, {
      capture: true,
    });
    target.addEventListener("pointerenter", internalEnter as EventListener, {
      capture: true,
    });
    target.addEventListener("pointerleave", internalLeave as EventListener, {
      capture: true,
    });
    target.addEventListener("pointermove", internalMove as EventListener, {
      capture: true,
    });
    target.addEventListener("pointerdown", internalDown as EventListener, {
      capture: true,
    });
    target.addEventListener("pointerup", internalUp as EventListener, {
      capture: true,
    });
    target.addEventListener("pointercancel", internalCancel as EventListener, {
      capture: true,
    });
    ...
    this.dispose = () => {
      ...
      target.removeEventListener("touchstart", touchPreventScrolling as EventListener, {
        capture: true,
      });
      target.removeEventListener("touchmove", touchPreventScrolling as EventListener, {
        capture: true,
      });
      target.removeEventListener("selectstart", selectDragListener as EventListener, {
        capture: true,
      });
      target.removeEventListener("selectionchange", selectDragListener as EventListener, {
        capture: true,
      });
      target.removeEventListener("dragstart", selectDragListener as EventListener, {
        capture: true,
      });
      ...
      target.removeEventListener("pointermove", internalMove as EventListener, {
        capture: true,
      });
      target.removeEventListener("pointerdown", internalDown as EventListener, {
        capture: true,
      });
      target.removeEventListener("pointerup", internalUp as EventListener, {
        capture: true,
      });
      target.removeEventListener("pointercancel", internalCancel as EventListener, {
        capture: true,
      });
      EasyPointers.#managedTargets.delete(target);
    };
  }

  #dispatchSyntheticEvent(
    type: string,
    modelEvent: PointerEvent,
    overrides: Partial<PointerEvent>,
  ): PointerEvent {
    const proxy = new Proxy(modelEvent, {
        get(_, key) {
          return Object.hasOwn(overrides, key)
            ? Reflect.get(overrides, key)
            : Reflect.get(modelEvent, key);
        },
      }),
      newEvent: PointerEvent = new PointerEvent(type, proxy);
    // TODO: Are there situations when I need to be able to override target? Bubbling enter/leave events?
    if (newEvent.bubbles && this.#target.contains(modelEvent.target as Node)) {
  ...
  #dispatchPointerEnterEvent(modelEvent: PointerEvent): PointerEvent {
    return this.#dispatchSyntheticEvent("pointerenter", modelEvent, {
      bubbles: false,
  ...
  #dispatchPointerLeaveEvent(pointerId: number): PointerEvent | null {
    const pointer: _InternalPointer | undefined = this.#byId.get(pointerId);
    if (!pointer) {
      return null;
    }
    return this.#dispatchSyntheticEvent("pointerleave", pointer._lastEvent!, {
      bubbles: false,
      button: _ButtonIndex.none,
      buttons: _ButtonBitmask.none,
      relatedTarget: null,
  ...
  #dispatchPointerDownEvent(modelEvent: PointerEvent, button: _ButtonIndex): PointerEvent {
    return this.#dispatchSyntheticEvent("pointerdown", modelEvent, {
      bubbles: true,
      button,
  ...
  #dispatchPointerUpEvent(modelEvent: PointerEvent, button: _ButtonIndex): PointerEvent {
    return this.#dispatchSyntheticEvent("pointerup", modelEvent, {
      bubbles: true,
      button,
      buttons: _ButtonBitmask.none,
      relatedTarget: null,
  ...
  public dispatchPointerCancelEvent(pointerId: number): PointerEvent | null {
    ...
    return this.#dispatchSyntheticEvent("pointercancel", pointer._lastEvent!, {
      bubbles: true,
      button: _ButtonIndex.none,
      buttons: _ButtonBitmask.none,
      relatedTarget: null,
  ...
  #latest: _InternalPointer<UserData>;
  public get latest(): Pointer<UserData> {
    return this.#latest;
  }

  #first: _InternalPointer<UserData>;
  public get first(): Pointer<UserData> {
    return this.#first;
  }

  public [Symbol.iterator](): Iterator<Pointer<UserData>> {
  ...
  public get firstHasJustEntered(): boolean {
    return this.#byId.size === 1 && this.#latest.hasJustEntered;
  }
  public get firstHasJustBecomeDown(): boolean {
    return this.downCount === 1 && this.#latest.hasJustBecomeDown;
  }
  public get lastHasJustLeft(): boolean {
    return this.#byId.size === 0 && this.#latest.hasJustLeft;
  }
  public get lastHasJustBecomeUp(): boolean {
    return this.downCount === 0 && this.#latest.hasJustBecomeUp;
  }
  ...
  public get isUsingTouch(): boolean {
    ...
    if (this.#latest.isTouch) {
      return true;
    }
    return false;
  }

  public get altKeyIsDown(): boolean {
    return activeModifierKeys.alt;
  }
  public get ctrlKeyIsDown(): boolean {
    return activeModifierKeys.ctrl;
  }
  public get metaKeyIsDown(): boolean {
    return activeModifierKeys.meta;
  }
  public get shiftKeyIsDown(): boolean {
    return activeModifierKeys.shift;
  }
  public get ctrlOrMetaKeyIsDown(): boolean {
    return activeModifierKeys.ctrl || activeModifierKeys.meta;
  }
}
...
function _derive2dMatrixOfTransformedElementByAppendingChild(element: HTMLElement): DOMMatrix {
  ...
  const dx1: number = x1 - x2,
    ...
    dx2: number = x3 - x2,
    dy2: number = y3 - y2,
    sx: number = x0 - x1 + x2 - x3,
    sy: number = y0 - y1 + y2 - y3;
  ...
  if (sx === 0 && sy === 0) {
    const a: number = x1 - x0,
      ...
      e: number = y3 - y0,
      f: number = y0;
    matrix = new DOMMatrix([a, d, 0, 0, b, e, 0, 0, 0, 0, 1, 0, c, f, 0, 1]);
  } else {
    const denom: number = dx1 * dy2 - dy1 * dx2,
      g: number = (sx * dy2 - sy * dx2) / denom,
      h: number = (dx1 * sy - dy1 * sx) / denom,
      a: number = x1 - x0 + g * x1,
      b: number = x3 - x0 + h * x3,
      c: number = x0,
      d: number = y1 - y0 + g * y1,
      e: number = y3 - y0 + h * y3,
      f: number = y0;
    matrix = new DOMMatrix([a, d, 0, g, b, e, 0, h, 0, 0, 1, 0, c, f, 0, 1]);
  }
...
export function projectPointToPlane(px: number, py: number, matrix: DOMMatrix): Point2d {
  ...
  let y: number =
    (py * (matrix.m14 * x + matrix.m44) - matrix.m12 * x - matrix.m42) /
    (matrix.m22 - matrix.m24 * py);
  if (isNaN(y)) {
    y =
      (px * (matrix.m14 * x + matrix.m44) - matrix.m11 * x - matrix.m41) /
      (matrix.m21 - matrix.m24 * px);
  }
  if (!(x * matrix.m14 + y * matrix.m24 + matrix.m44 >= 0)) {
    // The camera ray is above the horizon and doesn't move toward the plane.
    return new Point2d(Number.NaN, Number.NaN);
  }
...
const _containStylesForOffsetParent = new Set<string>(["paint", "content", "strict", "layout"]),
  // Possible values of the will-change style that force an element to be an offset parent.
  _willChangeStylesForOffsetParent = new Set<string>([
    "transform",
    "rotate",
    "scale",
    "translate",
...
function _couldBeOffsetParent(
  childPosition: string,
  ancestor: HTMLElement | SVGElement,
      ...
      if (ancestor === document.body) {
        ...
        if (display !== "inline") {
          if (
            ...
            throw new Error(
              "EasyPointers: Sorry, EasyPointers does not support body elements with css transforms unless the body element is also a positioned element, e.g. position: relative.",
            );
          }
        }
        return false;
  ...
  if (display !== "inline") {
    if (ancestor !== document.documentElement && (ancestorStyle.zoom ?? "1") !== "1") {
      throw new Error(
        "EasyPointers: Sorry, EasyPointers does not support elements with css zoom unless the element is also a positioned element, e.g. position: relative.",
      );
    }
    if (
  ...
  return (
    ...
    (ancestorStyle.contain ?? "none")
      .split(" ")
      .some((s) => _containStylesForOffsetParent.has(s)) ||
...
function _parseObjectPositionComponent(positionComponent: string, unusedSize: number): number {
  ...
  if (positionComponent.charCodeAt(0) === 99) {
    ...
    return endOffset == null ? 0 : unusedSize - Number.parseFloat(endOffset);
  } else if (positionComponent.charCodeAt(positionComponent.length - 1) === 37) {
    // "%"
    // Ends with a percent, based on unused size.
    return (unusedSize * Number.parseFloat(positionComponent)) / 100;
  }
...
function _imageElementContainsSvg(element: HTMLImageElement): boolean {
  ...
  const fileExtension: string | undefined =
    element instanceof HTMLImageElement
      ? element.src.match(/\.([^.?#/]+)(\?.*)?(#.*)?$/)?.[1]?.toLowerCase()
      : undefined;
...
export function getElementMatrix(
  ...
  const styles: CSSStyleDeclaration = getComputedStyle(element),
    // oxlint-disable-next-line no-use-before-define -- The matrix helper recursively uses this API.
    matrix: DOMMatrix = _getBorderBoxMatrix(element, styles);
  ...
  if (element instanceof HTMLElement) {
    switch (elementBox) {
      ...
      case ElementBox.imagePixels: {
        {
          ...
          if (element instanceof HTMLImageElement) {
            ...
            throw new Error(
              `EasyPointers: Unsupported element type for image box: ${element.tagName}`,
            );
          }
          ...
          switch (styles.objectFit) {
            ...
            default: {
              throw new Error(
                `EasyPointers: Unrecognized object-fit style value:${styles.objectFit}`,
              );
            }
          }

          const stylePosMatch: RegExpMatchArray | null = styles.objectPosition.match(
              /(calc\([^)]*\)|[^(]+%|[^(]*px)\s+(calc\([^)]*\)|[^(]+%|[^(]*px)/,
            ),
            posLeft: number = _parseObjectPositionComponent(
              stylePosMatch?.[1] ?? "50%",
              availableWidth - renderedWidth,
            ),
            posTop: number = _parseObjectPositionComponent(
              stylePosMatch?.[2] ?? "50%",
              availableHeight - renderedHeight,
            ),
            contentLeft: number = parseFloat(styles.paddingLeft) + element.clientLeft,
            contentTop: number = parseFloat(styles.paddingTop) + element.clientTop,
            imageLeft: number = contentLeft + posLeft,
            imageTop: number = contentTop + posTop;
      ...
      default: {
        throw new Error(`EasyPointers: Unrecognized element box: ${elementBox}`);
      }
    }
  }
  ...
  switch (displayBox ?? DisplayBox.viewport) {
    ...
    case DisplayBox.page: {
      {
        matrix.preMultiplySelf(new DOMMatrix().translateSelf(window.scrollX, window.scrollY));
      }
      break;
    }
    default: {
      throw new Error(`EasyPointers: Unrecognized display box: ${displayBox}`);
    }
  }
...
export function getElementDimensions(
  element: HTMLElement,
  elementBox: `${ElementBox}` = ElementBox.paddingBox,
  ...
  switch (elementBox) {
    ...
    case ElementBox.insideBorder: {
      return {
        width:
          element.offsetWidth -
          Number.parseFloat(styles.borderLeft) -
          Number.parseFloat(styles.borderRight),
        height:
    ...
    case ElementBox.contentBox: {
      ...
      return {
        width:
          element.scrollWidth - parseFloat(styles.paddingLeft) - parseFloat(styles.paddingRight),
        height:
          element.scrollHeight - parseFloat(styles.paddingTop) - parseFloat(styles.paddingBottom),
      };
    }
    case ElementBox.imagePixels: {
      if (element instanceof HTMLImageElement) {
...
const _elementsThatDoNotRenderChildren = new Set<string>([
  "audio",
  "br",
  "canvas",
  "embed",
...
function _getBorderBoxMatrix(
  element: HTMLElement | SVGElement,
  styles: CSSStyleDeclaration,
): DOMMatrix {
  if (element instanceof SVGElement) {
    const svgMatrix: SVGMatrix | DOMMatrix | null = (
      element as SVGGraphicsElement
    ).getScreenCTM?.();
    if (!svgMatrix) {
      throw new Error("EasyPointers: Could not get transform matrix of SVG Element.");
    }
    let domMatrix: DOMMatrix;
    if (svgMatrix instanceof DOMMatrix) {
      domMatrix = svgMatrix;
    } else {
      const legacyMatrix: SVGMatrix = svgMatrix as SVGMatrix;
  ...
  const position: string = styles.position,
    ...
    offsetParent: HTMLElement | SVGElement | null = _getOffsetParent(position, element),
    parentStyle: CSSStyleDeclaration | null = offsetParent ? getComputedStyle(offsetParent) : null;
  //*/

  let perspective: number | null = null,
    perspectiveOriginX: number | null = null,
    perspectiveOriginY: number | null = null;
  const parentPerspective: string = parentStyle?.perspective ?? "none";
  if (parentPerspective !== "none") {
    perspective = Number.parseFloat(parentPerspective);
    const perspectiveOrigin: string[] = (parentStyle?.perspectiveOrigin ?? "0px 0px").split(" ");
    perspectiveOriginX = Number.parseFloat(perspectiveOrigin[0]!);
    perspectiveOriginY = Number.parseFloat(perspectiveOrigin[1]!);
  }
  ...
  const zoom: string = element === document.documentElement ? "1" : (styles.zoom ?? "1"),
    translate: string = styles.translate ?? "none",
    rotate: string = styles.rotate ?? "none",
    scale: string = styles.scale ?? "none",
    transform: string = styles.transform,
    hasOffsetPath: boolean = (styles.offsetPath ?? "none") !== "none",
    displayIsInline: boolean = styles.display === "inline",
  ...
  const hasReportedOffsets: boolean =
  ...
  let mustDeriveTransformFromChildren: boolean =
    (!hasReportedOffsets && usesTransforms) || hasOffsetPath;
  ...
  if (element === document.body && !usesTransforms) {
    ...
    if (
      ...
      if (position === "static") {
        throw new Error(
          "EasyPointers: Can't determine how the root html element's transform affects the body element if the body isn't a positioned element.",
        );
      } else {
  ...
  if (mustDeriveTransformFromChildren) {
    ...
    const canDeriveTransformFromChildren: boolean =
      (usesTransforms || position !== "static") &&
      !_elementsThatDoNotRenderChildren.has(element.tagName.toLowerCase()) &&
      !preserve3d;
    if (!canDeriveTransformFromChildren) {
      throw new Error(
        `EasyPointers: Can't compute the transform of a the element: ${element.tagName}#${
          element.id
        }.${element.className}`,
      );
    }
  ...
  if (hasReportedOffsets) {
    ...
    while (offsetAncestor && !domScopes.has(offsetAncestor.getRootNode())) {
      if (offsetAncestor instanceof SVGElement) {
        throw new Error(
          "EasyPointers: Can't determine matrix of an HTMLElement that is in the slot of a shadow dom that contains SVGElements.",
        );
      }
  ...
  if (usesTransforms) {
    ...
    const transformOrigin: string[] = styles.transformOrigin.split(" "),
      negatedOrigin: string[] = transformOrigin.map((c) =>
        c.charCodeAt(0) === 45 /* "-" */ ? c.slice(1) : `-${c}`,
      );
    let originTranslate = `translate(${transformOrigin.slice(0, 2).join(", ")}) `,
      reverseTranslate = `translate(${negatedOrigin.slice(0, 2).join(", ")}) `;
...
class _InternalPointer<UserData = unknown> implements Pointer<UserData> {
  ...
  public _isPresent = true;
  public get isPresent(): boolean {
    return this._isPresent;
  }

  public _isInTarget = true;
  public get isInTarget(): boolean {
    return this._isInTarget && this._isPresent;
  }

  public _isDown = false;
  ...
  public _hasJustEntered = false;
  public get hasJustEntered(): boolean {
    return this._hasJustEntered;
  }
  public _hasJustLeft = false;
  public get hasJustLeft(): boolean {
    return this._hasJustLeft;
  }
  public _hasJustMoved = false;
  public get hasJustMoved(): boolean {
    return this._hasJustMoved;
  }
  public _hasJustBecomeDown = false;
  public get hasJustBecomeDown(): boolean {
    return this._hasJustBecomeDown;
  }
  public _hasJustBecomeUp = false;
  public get hasJustBecomeUp(): boolean {
    return this._hasJustBecomeUp;
  }
  public _hasJustBeenCanceled = false;
  public get hasJustBeenCanceled(): boolean {
    return this._hasJustBeenCanceled;
  }

  public get hasMovedFromStart(): boolean {
    return this._gesture?.hasMovedFromStart ?? false;
  }
  public get wasHeldStill(): boolean {
    return this._gesture?.wasHeldStill ?? false;
  }

  public _pressedButtons: number = _ButtonBitmask.none;
  public get pressedButtons(): Set<PointerButton> {
    return _buttonSetFromBitmask(this._pressedButtons);
  }
  public get hadPressedPrimaryButton(): boolean {
    return (this._pressedButtons & _ButtonBitmask.primary) !== 0;
  }
  public get hadPressedSecondaryButton(): boolean {
    return (this._pressedButtons & _ButtonBitmask.secondary) !== 0;
  }
  public get hadPressedMiddleButton(): boolean {
    return (this._pressedButtons & _ButtonBitmask.middle) !== 0;
  }
  public get hadPressedOnlyPrimaryButton(): boolean {
    return this._pressedButtons === _ButtonBitmask.primary;
  }
  public get hadPressedOnlySecondaryButton(): boolean {
    return this._pressedButtons === _ButtonBitmask.secondary;
  }
  public get hadPressedOnlyMiddleButton(): boolean {
    return this._pressedButtons === _ButtonBitmask.middle;
  }
  ...
  public get pressingButtons(): Set<PointerButton> {
    return _buttonSetFromBitmask(this._pressingButtons);
  }
  public get isPressingPrimaryButton(): boolean {
    return (this._pressingButtons & _ButtonBitmask.primary) !== 0;
  }
  public get isPressingSecondaryButton(): boolean {
    return (this._pressingButtons & _ButtonBitmask.secondary) !== 0;
  }
  public get isPressingMiddleButton(): boolean {
    return (this._pressingButtons & _ButtonBitmask.middle) !== 0;
  }
  public get isPressingOnlyPrimaryButton(): boolean {
    return this._pressingButtons === _ButtonBitmask.primary;
  }
  public get isPressingOnlySecondaryButton(): boolean {
    return this._pressingButtons === _ButtonBitmask.secondary;
  }
  public get isPressingOnlyMiddleButton(): boolean {
    return this._pressingButtons === _ButtonBitmask.middle;
  }
  ...
  public get startedWithButton(): PointerButton {
    return _buttonFromIndex.get(this._startedWithButton)!;
  }
  public get startedWithPrimaryButton(): boolean {
    return this._startedWithButton === _ButtonIndex.primary;
  }
  public get startedWithSecondaryButton(): boolean {
    return this._startedWithButton === _ButtonIndex.secondary;
  }
  public get startedWithMiddleButton(): boolean {
    return this._startedWithButton === _ButtonIndex.middle;
  }

  public _justPressedButton: _ButtonIndex = _ButtonIndex.none;
  public get justPressedButton(): PointerButton {
    return _buttonFromIndex.get(this._justPressedButton)!;
  }
  public get hasJustPressedPrimaryButton(): boolean {
    return this._justPressedButton === _ButtonIndex.primary;
  }
  public get hasJustPressedSecondaryButton(): boolean {
    return this._justPressedButton === _ButtonIndex.secondary;
  }
  public get hasJustPressedMiddleButton(): boolean {
    return this._justPressedButton === _ButtonIndex.middle;
  }

  public _justReleasedButton: _ButtonIndex = _ButtonIndex.none;
  public get justReleasedButton(): PointerButton {
    return _buttonFromIndex.get(this._justReleasedButton)!;
  }
  public get hasJustReleasedPrimaryButton(): boolean {
    return this._justReleasedButton === _ButtonIndex.primary;
  }
  public get hasJustReleasedSecondaryButton(): boolean {
    return this._justReleasedButton === _ButtonIndex.secondary;
  }
  public get hasJustReleasedMiddleButton(): boolean {
    return this._justReleasedButton === _ButtonIndex.middle;
  }

  public get pageX(): number {
    return this._lastEvent!.clientX + window.scrollX;
  }
  public get pageY(): number {
    return this._lastEvent!.clientY + window.scrollY;
  }
  public get pointInPage(): Point2d {
    return new Point2d(
      this._lastEvent!.clientX + window.scrollX,
      this._lastEvent!.clientY + window.scrollY,
    );
  }
  public get viewportX(): number {
    return this._lastEvent!.clientX;
  }
  public get viewportY(): number {
    return this._lastEvent!.clientY;
  }
  public get pointInViewport(): Point2d {
    return new Point2d(this._lastEvent!.clientX, this._lastEvent!.clientY);
  }
  public getPointIn(element: HTMLElement | SVGElement, box?: `${ElementBox}`): Point2d {
    if (!elementIsInLayout(element)) {
      return new Point2d(Number.NaN, Number.NaN);
    }
  ...
  public getPointInNormalized(element: HTMLElement, box?: `${ElementBox}`): Point2d {
    ...
    const point: Point2d = projectPointToPlane(
      ...
      dimensions = getElementDimensions(element, box);
    point.x = point.x / dimensions.width || 0;
    point.y = point.y / dimensions.height || 0;
    return point;
  }

  public _startedWithAltKey = false;
  public get startedWithAltKey(): boolean {
    return this._startedWithAltKey;
  }
  public _startedWithCtrlKey = false;
  public get startedWithCtrlKey(): boolean {
    return this._startedWithCtrlKey;
  }
  public _startedWithMetaKey = false;
  public get startedWithMetaKey(): boolean {
    return this._startedWithMetaKey;
  }
  public _startedWithShiftKey = false;
  public get startedWithShiftKey(): boolean {
    return this._startedWithShiftKey;
  }

  public cancel(): void {
    this.#pointers.dispatchPointerCancelEvent(this.id);
  }
  ...
  public _verifyValidity(): void {
    ...
    if (this._isDown !== (this._pressingButtons !== _ButtonBitmask.none)) {
      throw new Error(
        "EasyPointers: Whether pointer is down should correspond to whether it is pressing at least one button.",
      );
    }
    if ((this._pressingButtons & this._pressedButtons) !== this._pressingButtons) {
      throw new Error(
        "EasyPointers: A pointer is pressing buttons that were not recorded has having been pressed.",
      );
    }
    if (
      (this._startedWithButton !== _ButtonIndex.none) !==
      (this._pressedButtons !== _ButtonBitmask.none)
    ) {
      throw new Error(
        "EasyPointers: Whether pointer is has a started button should correspond to whether it has pressed any buttons.",
      );
    }
  }
}
