...
import { type Song, SynthEngine } from "../synth/synth.js";
...
export class SongRenderer {
  readonly #assetSource: SongRendererAssetSource;
  public outputSamplesL!: Float32Array;
  public outputSamplesR!: Float32Array;
  public canceled = false;

  public constructor(_assetSource: SongRendererAssetSource) {
  ...
  public async *generate(
    ...
    await this.#assetSource.loadAssetsInto(synth);
    if (this.canceled) {
      return;
    }
    ...
    const totalBars: number = synth.getTotalBars(enableIntro, enableOutro),
      totalTicks: number = totalBars * song.beatsPerBar * Config.partsPerBeat * Config.ticksPerPart;
    synth.renderTicksRemaining = totalTicks;
    const estimatedSampleLength: number = Math.ceil(synth.getSamplesPerBar() * totalBars),
      chunksL: Float32Array[] = [],
      chunksR: Float32Array[] = [];
    let renderedSampleCount = 0,
      samplesPerNextRender = 1000;
    while (!synth.songEnded) {
      const samplesToRender: number = samplesPerNextRender,
        chunkL: Float32Array = new Float32Array(samplesToRender),
        chunkR: Float32Array = new Float32Array(samplesToRender),
        startMillis: number = performance.now();
      synth.synthesize(chunkL, chunkR, samplesToRender);
      const stopMillis: number = performance.now(),
        rendered: number = synth.lastSynthesizeSampleCount;
      if (rendered > 0) {
      ...
      const elapsedMillis: number = stopMillis - startMillis,
        targetMillis: number = 1000 / 30;
      samplesPerNextRender = Math.ceil(
        Math.max(1000, Math.min(500_000, (samplesToRender * targetMillis) / elapsedMillis)),
      );
    ...
    let offset = 0;
    for (let index = 0; index < chunksL.length; index++) {
