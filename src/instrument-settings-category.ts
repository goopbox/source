...
const effectProperties: ReadonlySet<string> = new Set([
  ...
  generalProperties: ReadonlySet<string> = new Set([
    "volume",
    "pan",
    "fadeInSeconds",
    "fadeOutTicks",
...
function propertyIsInCategory(property: string, category: InstrumentSettingsCategory): boolean {
  if (category === "effects") {
    return effectProperties.has(property);
  }
  if (category === "envelopes") {
    return property === "envelopes";
  }
  return (
    !effectProperties.has(property) && property !== "envelopes" && !generalProperties.has(property)
  );
}
