...
export class FilterEditor {
  ...
  private readonly _highlight: SVGCircleElement = SVG.circle({
    fill: "white",
    stroke: "none",
    "pointer-events": "none",
    r: 4,
  ...
  private _yToGain(y: number): number {
    return (
      (Config.filterGainRange - 1) * (1 - (y - 0.5) / (this._editorHeight - 1))
    );
  }
  private _gainToY(gain: number): number {
    return (
      (this._editorHeight - 1) * (1 - gain / (Config.filterGainRange - 1)) + 0.5
    );
  }
  ...
  private _updateCursor(): void {
    ...
    for (let i: number = 0; i < this._filterSettings.controlPointCount; i++) {
      ...
      const distance: number = Math.sqrt(
        Math.pow(this._freqToX(point.freq) - this._mouseX, 2) +
          Math.pow(this._gainToY(point.gain) - this._mouseY, 2),
      );
      if (
        (distance <= 13 ||
          this._filterSettings.controlPointCount >= Config.filterMaxPoints) &&
  ...
  private _whenCursorMoved(_event: PointerEvent): void {
      ...
      if (this._addingPoint) {
        const gain: number = Math.max(
          ...
          Math.min(
            ...
            Math.round(this._yToGain(this._mouseY)),
          ),
        );
        const freq: number = this._findNearestFreqSlot(
          this._filterSettings,
          this._xToFreq(this._mouseX),
          -1,
        );
        ...
        this._selectedIndex >= this._filterSettings.controlPointCount ||
        this._selectedIndex == -1
      ) {
        this._dragChange = null;
        this._mouseDown = false;
        ...
        const point: FilterControlPoint =
          this._filterSettings.controlPoints[this._selectedIndex];
        const gain: number = Math.max(
          0,
          Math.min(
            Config.filterGainRange - 1,
            Math.round(point.gain + gainDelta),
          ),
        );
        const freq: number = this._findNearestFreqSlot(
          this._filterSettings,
  ...
  private _updatePath(): void {
    ...
    for (let i: number = 0; i < this._filterSettings.controlPointCount; i++) {
      ...
      controlPointPath += FilterEditor._circlePath(
        pointX,
        pointY,
        this._pointRadius,
      );
      ...
      if (
        (this._selectedIndex == i ||
          (this._addingPoint &&
            this._mouseDown &&
            i == this._filterSettings.controlPointCount - 1)) &&
        this._pointers.latest.isPresent &&
        !this._deletingPoint
      ) {
        this._label.textContent =
          i + 1 + ": " + Config.filterTypeNames[point.type]; // + " " + prettyNumber(point.getHz()) + "Hz";
      }
    }
    ...
    for (let i: number = -1; i <= Config.filterFreqRange; i++) {
      ...
      const cornerRadiansPerSample: number =
        (2.0 * Math.PI * hz) / standardSampleRate;
  ...
  public render(): void {
    ...
    const filterSettings: FilterSettings = this._useNoteFilter
