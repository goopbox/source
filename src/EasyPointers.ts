...
function verifyBooleanOption(
  ...
  if (typeof value != "boolean" && value !== undefined) {
    throw new Error(
      `EasyPointers: The ${name} option, if provided, should be a boolean value.`,
    );
  }
...
export class EasyPointers<UserData = unknown> {
  private static readonly _managedTargets: WeakSet<HTMLElement | SVGElement> =
    new WeakSet();
  ...
  constructor(
    ...
    if (!(target instanceof HTMLElement) && !(target instanceof SVGElement)) {
      throw new Error(
        "EasyPointers: The target should be an HTMLElement or SVGElement.",
      );
    }
    ...
    const listenForTheseButtons: Set<`${PointerButton}`> = new Set(
      options.listenForTheseButtons ?? [PointerButton.primary],
    );
    for (const button of listenForTheseButtons) {
      if (!_buttonSet.has(button)) {
        throw new Error(
          "EasyPointers: Unrecognized pointer button name provided: " +
            button +
            ". Valid values are: " +
            [..._buttonSet].map((s) => `"${s}"`).join(", "),
        );
      }
    }

    const listenForTheseButtonIndices: Set<_ButtonIndex> = new Set(
      [...listenForTheseButtons].map((button) => _indexFromButton.get(button)!),
    );
    const listenForTheseButtonsBitmask: number = [...listenForTheseButtons]
      .map((button) => _bitmaskFromButton.get(button)!)
      .reduce((acc, bit) => acc | bit, 0);
    const listenForMultipleButtonsDown: boolean = verifyBooleanOption(
      options,
      "listenForMultipleButtonsDown",
      false,
    );
    const listenForMultiplePointers: boolean = verifyBooleanOption(
      options,
      "listenForMultiplePointers",
      false,
    );
    const listenForPossiblyAccidentalPointers: boolean = verifyBooleanOption(
      options,
    ...
    this.deferInitialEvents = verifyBooleanOption(
      options,
      "deferInitialEvents",
      false,
    );
    ...
    const stopEventsAfterTargetIsRemovedFromLayout: boolean =
      verifyBooleanOption(
        options,
    ...
    this._latest = new _InternalPointer(this, 0, "mouse");
    ...
    const syncEventAndPointer = (
      event: PointerEvent,
      pointer: _InternalPointer<UserData>,
    ): void => {
    ...
    const internalEnter = (event: PointerEvent): void => {
      ...
      if (
        stopEventsAfterTargetIsRemovedFromLayout &&
        !elementIsInLayout(this._target)
      ) {
      ...
      let pointer: _InternalPointer<UserData> | undefined = this._byId.get(
        event.pointerId,
      );
    ...
    const internalMove = (event: PointerEvent): void => {
      ...
      const pointerPressingButtons = (): number =>
        (pointer?._gesture?.deferringEvents ?? false)
          ? (pointer?._gesture?.pressingButtons ?? _ButtonBitmask.none)
          : (pointer?._pressingButtons ?? _ButtonBitmask.none);
      let changedButtonIndex: _ButtonIndex = listenForTheseButtonIndices.has(
        event.button,
      )
        ? event.button
        : _ButtonIndex.none;
      ...
      if (changedButtonIndex != _ButtonIndex.none) {
        ...
        if (changedButtonBitmask == undefined)
          throw new Error(
            "EasyPointers: Missing bitmask for button index: " +
              changedButtonIndex,
          );
        const pressingChangedButton: boolean =
          (event.buttons & changedButtonBitmask) != 0;
        ...
        if (
          !pressingChangedButton &&
          (pressingButtons & changedButtonBitmask) != 0
        ) {
          removedButton = changedButtonIndex;
          pressingButtons = pressingButtons & ~changedButtonBitmask;
        }
      }
      const pressingAnyMissingButtons: boolean =
        (pressingButtons & ~event.buttons) != 0;
      ...
      if (addedButton != _ButtonIndex.none && !pointerIsDown()) {
        if (
          this._dispatchPointerDownEvent(event, addedButton).defaultPrevented
        ) {
      ...
      if (
        ...
        if (
          this._dispatchPointerUpEvent(event, removedButton).defaultPrevented
        ) {
          // Make sure the browser knows the app wanted to prevent the default behavior.
          event.preventDefault();
        }
      ...
      if (!pointer || !pointer._isPresent) {
        if (
          ...
          throw new Error(
            "EasyPointers: Cannot add or remove buttons on a missing pointer.",
          );
        }
      ...
      if (pointerIsDown() && gesture && !gesture.hasMovedFromStart) {
        ...
        const zoom: number =
          window.outerWidth / (visualViewport?.width ?? window.innerWidth);
    ...
    const internalDown = (event: PointerEvent): void => {
      ...
      if (!(gesture?.dispatchingDeferredEvents ?? false)) {
        ...
        if (gesture.deferringEvents) {
          gesture.deferredEvents.push(event);
          gesture.pressingButtons =
            event.buttons & listenForTheseButtonsBitmask;
          return stop(event);
        }
      }
    ...
    const contextmenu = (event: PointerEvent): void => {
      if (this.preventContextMenu) {
        ...
        const pointer: _InternalPointer | undefined = this._byId.get(
          event.pointerId,
        );
        if (pointer && pointer._isDown) {
    ...
    target.addEventListener(
      "touchstart",
      touchPreventScrolling as EventListener,
    ...
    target.addEventListener(
      "selectstart",
      selectDragListener as EventListener,
      { capture: true },
    );
    target.addEventListener(
      "selectionchange",
    ...
    this.dispose = () => {
      ...
      target.removeEventListener(
        "pointerenter",
        internalEnter as EventListener,
        { capture: true },
      );
      target.removeEventListener(
        "pointerleave",
        internalLeave as EventListener,
        { capture: true },
      );
      ...
      target.removeEventListener(
        "pointercancel",
        internalCancel as EventListener,
        { capture: true },
      );
  ...
  private _dispatchPointerDownEvent(
    modelEvent: PointerEvent,
    button: _ButtonIndex,
  ): PointerEvent {
  ...
  private _dispatchPointerUpEvent(
    modelEvent: PointerEvent,
    button: _ButtonIndex,
  ): PointerEvent {
...
const _buttonFromIndex: Map<_ButtonIndex, PointerButton> = new Map(
  Array.from(_indexFromButton, (entry) => entry.reverse()) as Array<
    [_ButtonIndex, PointerButton]
  >,
);
...
const _buttonFromBitmask: Map<_ButtonBitmask, PointerButton> = new Map(
  Array.from(_bitmaskFromButton, (entry) => entry.reverse()) as Array<
    [_ButtonBitmask, PointerButton]
  >,
);
...
class _InternalPointer<UserData = unknown> implements Pointer<UserData> {
  ...
  public _verifyValidity(): void {
    ...
    if ((this._isDown || this._isInTarget) && !this._isPresent)
      throw new Error(
        "EasyPointers: A pointer that is down or in the target shouldn't have left.",
      );
...
function _derive2dMatrixOfTransformedElementByAppendingChild(
  element: HTMLElement,
): DOMMatrix {
...
const _containStylesForOffsetParent: Set<string> = new Set([
  "paint",
  "content",
  "strict",
  "layout",
...
const _willChangeStylesForOffsetParent: Set<string> = new Set([
  "transform",
  "rotate",
  "scale",
  "translate",
...
function _couldBeOffsetParent(
  ...
  return (
    ...
    ancestorStyle.willChange
      .split(", ")
      .some((s) => _willChangeStylesForOffsetParent.has(s))
  );
}
...
function _parseObjectPositionComponent(
  ...
  if (positionComponent.charCodeAt(0) == 99) {
    ...
    const endOffset: string | undefined = positionComponent.match(
      /calc\(100% - (.+)px\)/,
    )?.[1];
...
export function getElementMatrix(
  ...
  if (elementBox == undefined) {
    elementBox =
      element instanceof SVGElement
        ? ElementBox.svgTransform
        : ElementBox.paddingBox;
  }
  ...
  if (element instanceof SVGElement && elementBox != ElementBox.svgTransform) {
    ...
    throw new Error(
      "EasyPointers: The element box must be svgTransform for SVG elements.",
    );
  }
  ...
  if (element instanceof HTMLElement) {
    switch (elementBox) {
      ...
      case ElementBox.imagePixels:
        {
          ...
          if (element instanceof HTMLImageElement) {
            ...
            throw new Error(
              "EasyPointers: Unsupported element type for image box: " +
                element.tagName,
            );
          }
          ...
          switch (styles.objectFit) {
            ...
            default:
              throw new Error(
                "EasyPointers: Unrecognized object-fit style value:" +
                  styles.objectFit,
              );
          }

          const stylePosMatch: RegExpMatchArray | null =
            styles.objectPosition.match(
              /(calc\([^\)]*\)|[^\(]+%|[^\(]*px)\s+(calc\([^\)]*\)|[^\(]+%|[^\(]*px)/,
            );
          const posLeft: number = _parseObjectPositionComponent(
            stylePosMatch?.[1] ?? "50%",
            availableWidth - renderedWidth,
          );
          const posTop: number = _parseObjectPositionComponent(
            stylePosMatch?.[2] ?? "50%",
            availableHeight - renderedHeight,
          );
          const contentLeft: number =
            parseFloat(styles.paddingLeft) + element.clientLeft;
          const contentTop: number =
            parseFloat(styles.paddingTop) + element.clientTop;
      ...
      case ElementBox.svgTransform: {
        throw new Error(
          "EasyPointers: svgTransform unsupported for HTML elements.",
        );
      } // break; // unreachable code because of the throw.
      default:
        throw new Error(
          "EasyPointers: Unrecognized element box: " + elementBox,
        );
    }
  }
...
const _elementsThatDoNotRenderChildren: Set<string> = new Set([
  "audio",
  "br",
  "canvas",
  "embed",
...
function _getBorderBoxMatrix(
  ...
  if (element instanceof SVGElement) {
    ...
    if (!svgMatrix)
      throw new Error(
        "EasyPointers: Could not get transform matrix of SVG Element.",
      );
    const domMatrix: DOMMatrix =
      (svgMatrix as any) instanceof DOMMatrix
        ? svgMatrix
        : new DOMMatrix([
            svgMatrix.a,
            svgMatrix.b,
  ...
  const offsetParent: HTMLElement | SVGElement | null = _getOffsetParent(
    position,
    element,
  );
  const parentStyle: CSSStyleDeclaration | null = offsetParent
  ...
  if (parentPerspective != "none") {
    ...
    const perspectiveOrigin: string[] = (
      parentStyle?.perspectiveOrigin ?? "0px 0px"
    ).split(" ");
  ...
  const zoom: string =
    element == document.documentElement ? "1" : (styles.zoom ?? "1");
  ...
  if (element == document.body && !usesTransforms) {
    ...
    const htmlStyles: CSSStyleDeclaration = getComputedStyle(
  ...
  if (mustDeriveTransformFromChildren) {
    ...
    if (!canDeriveTransformFromChildren) {
      throw new Error(
        "EasyPointers: Can't compute the transform of a the element: " +
          element.tagName +
          "#" +
          element.id +
          "." +
  ...
  if (hasReportedOffsets) {
    ...
    while (offsetAncestor && !domScopes.has(offsetAncestor.getRootNode())) {
      ...
      const ancestorPosition: string =
        getComputedStyle(offsetAncestor).position ?? "static";
  ...
  if (!usesTransforms) {
    matrix.translateSelf(
      parentPaddingX + offsetLeft,
      parentPaddingY + offsetTop,
    );
  } else {
    ...
    const negatedOrigin: string[] = transformOrigin.map((c) =>
    ...
    if (translate != "none") {
      ...
      if (components[0].charCodeAt(components[0].length - 1) == 37) {
        // "%"
        components[0] =
          (parseFloat(components[0]) / 100) * element.offsetWidth + "px";
      }
      if (
        components.length >= 2 &&
        components[1].charCodeAt(components[1].length - 1) == 37
      ) {
        // "%"
        components[1] =
          (parseFloat(components[1]) / 100) * element.offsetHeight + "px";
      }
