import { assetCacheName } from "../synth/asset-cache-config.js";
...
const worker: ServiceWorkerScope = self as unknown as ServiceWorkerScope,
  applicationCachePrefix = "goopbox-audio-worklet-",
  applicationCacheName = `${applicationCachePrefix}2`,
  permanentExternalResourceOrigins: ReadonlySet<string> = new Set([
...
worker.addEventListener("install", (event: ExtendableEvent): void => {
  event.waitUntil(
    caches.open(applicationCacheName).then(async (cache: Cache): Promise<void> => {
      await cache.addAll([
        "/",
        "/synth_worklet.js",
        ...
        cacheNames
          .filter(
            (cacheName: string): boolean =>
              cacheName.startsWith(applicationCachePrefix) && cacheName !== applicationCacheName,
          )
          .map((cacheName: string): Promise<boolean> => caches.delete(cacheName)),
      );
...
worker.addEventListener("fetch", (event: WorkerFetchEvent): void => {
  ...
  const requestOrigin: string = new URL(event.request.url).origin,
    isPermanentExternalResource: boolean = permanentExternalResourceOrigins.has(requestOrigin);
  ...
  event.respondWith(
    (async (): Promise<Response> => {
      ...
      if (isPermanentExternalResource) {
        const cachedResponse: Response | undefined = await applicationCache.match(event.request);
