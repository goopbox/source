...
import type { Event, EventPoint } from "../synth/synth.js";
...
export function clipEvent<T extends Event>(event: T, start: number, end: number): T | null {
  const clippedStart: number = Math.max(event.start, start),
    clippedEnd: number = Math.min(event.end, end);
  ...
  for (const point of event.points) {
    ...
    if (absoluteTime <= clippedStart || absoluteTime >= clippedEnd) {
      continue;
    }
...
export function deleteEventRange<T extends Event>(
  ...
  for (const event of events) {
    ...
    const before: T | null = clipEvent(event, event.start, start),
      after: T | null = clipEvent(event, end, event.end);
    if (before != null) {
...
export function resizeEventEnd<T extends Event>(event: T, end: number, pointCountMax: number): T {
  if (end <= event.end) {
    return clipEvent(event, event.start, end) ?? event.clone();
  }
  const points: EventPoint[] = event.points.map((point: EventPoint): EventPoint => point.clone()),
    last: EventPoint = points.at(-1)!;
...
export function editEventTime<T extends Event>(
  ...
  if (pointIndex !== 0 && pointIndex !== lastPointIndex) {
    const replacement: T[] = cloneEvents(events),
      originalTime: number = original.points[pointIndex]!.time,
      shiftedTime: number = Math.max(
        0,
      ...
      shiftedPoint: EventPoint = original.points[pointIndex]!.clone();
    shiftedPoint.time = shiftedTime;
    replacement[eventIndex]!.points = original.points
      .filter((point: EventPoint): boolean => point.time < skipStart || point.time > skipEnd)
  ...
  const shiftedEndpoint: number = (pointIndex === 0 ? original.start : original.end) + delta,
    others: T[] = events
  ...
  const held: T =
    pointIndex === 0
      ...
      : resizeEventEnd(
          original,
          Math.max(original.start + minDuration, Math.min(maximumEnd, original.end + delta)),
          pointCountMax,
        );
...
export function nearestEventPointIndex(
  ...
  if (lastPointIndex <= 0) {
    return Math.max(0, lastPointIndex);
  }
  const eventTime: number = absolutePart - event.start;
  if (Math.abs(eventTime - event.points[0]!.time) <= endpointRadius) {
    return 0;
  }
  if (Math.abs(eventTime - event.points[lastPointIndex]!.time) <= endpointRadius) {
    return lastPointIndex;
  }
  let nearestIndex = 0,
    nearestDistance: number = Number.POSITIVE_INFINITY;
...
export function bendEvent<T extends Event>(
  ...
  const replacement: T = event.clone(),
    time: number = Math.max(0, Math.min(event.end - event.start, absolutePart - event.start)),
    value: number = sanitize(event.getValueAt(event.start + time) + valueDelta);
  if (uniform) {
  ...
  if (existing !== undefined) {
    ...
    replacement.points.sort((a: EventPoint, b: EventPoint): number => a.time - b.time);
  }
  return replacement;
}
...
export function moveEventRange<T extends Event>(
  ...
  const newStart: number = range.start + delta,
    newEnd: number = range.end + delta;
  return deleteEventRange(deleteEventRange(events, range.start, range.end), newStart, newEnd)
    .concat(moved)
    .sort((a: T, b: T): number => a.start - b.start);
}
...
export function repeatEvents<T extends Event>(
  ...
  if (sourceDuration <= 0 || destination.start >= destination.end) {
    return [];
  }
  const repeated: T[] = [];
  for (let offset: number = destination.start; offset < destination.end; offset += sourceDuration) {
    for (const source of events) {
      const shifted: T = source.clone();
      shifted.start += offset;
      shifted.end += offset;
      const clipped: T | null = clipEvent(shifted, destination.start, destination.end);
      if (clipped != null) {
        repeated.push(clipped);
      }
    }
  }
  return repeated;
...
export function eventPath(event: Event, options: EventPathOptions): string {
  ...
  const points: EventPoint[] = event.points.map((point: EventPoint): EventPoint => point.clone()),
    duration: number = event.end - event.start,
    last: EventPoint = points.at(-1)!;
  ...
  const totalWidth: number = options.partWidth * duration,
    endOffset: number = 0.5 * Math.max(0, Math.min(2, totalWidth - 1)),
    position = (
      point: EventPoint,
      index: number,
    ...
    first = position(points[0]!, 0);
  let path = `M ${prettyNumber(first.x)} ${prettyNumber(first.center + first.size)} `;
  for (let index = 1; index < points.length; index++) {
    const previous = position(points[index - 1]!, index - 1),
      next = position(points[index]!, index);
