...
import { Channel, Instrument, Note, Pattern, makeNotePin } from "../synth/synth.js";
  ...
  return (
    presets.find(
      (preset: SoundFontPresetInfo): boolean =>
        ...
        (preset.bankMSB | (preset.isDrum ? 0x80 : 0) | (preset.bankLSB << 8)) === bank,
    )?.index ?? null
  );
}
...
function selectArrangement(project: FlpProject): FlpArrangement | undefined {
  if (project.currentArrangementId !== undefined) {
    const current: FlpArrangement | undefined = project.arrangements.find(
      (arrangement: FlpArrangement): boolean => arrangement.id === project.currentArrangementId,
    );
...
function velocityToNoteSize(velocity: number): number {
  ...
  return Math.max(
    1,
    Math.min(Config.noteSizeMax, Math.round((velocity * Config.noteSizeMax) / 127)),
  );
}
...
function bendIntervalAt(pins: readonly FlpBendPoint[], time: number): number {
  ...
  return pins.at(-1)!.interval;
}

function pushBendPoint(pins: FlpBendPoint[], time: number, interval: number): void {
  if (pins.length > 0 && pins.at(-1)!.time === time) {
...
function applyFlSlides(events: readonly ExpandedFlpNote[]): ExpandedFlpNote[] {
  const notes: ExpandedFlpNote[] = events.filter((event: ExpandedFlpNote): boolean => !event.slide),
    slides: ExpandedFlpNote[] = events
      .filter((event: ExpandedFlpNote): boolean => event.slide)
      .sort(
        (a: ExpandedFlpNote, b: ExpandedFlpNote): number => a.start - b.start || a.order - b.order,
      );
  ...
  for (const slide of slides) {
    ...
    const referencePitch: number = Math.max(
      ...
      targetInterval: number = slide.pitch - referencePitch;
    for (const note of affected) {
      const startInterval: number = bendIntervalAt(note.pins, slide.start),
        bendEnd: number = Math.min(slide.end, note.end),
        completion: number = (bendEnd - slide.start) / (slide.end - slide.start),
        endInterval: number = startInterval + completion * (targetInterval - startInterval),
        pins: FlpBendPoint[] = note.pins.filter(
...
function quantizeExpandedNote(
  ...
  const rawPins: FlpBendPoint[] = [
      ...
      ...note.pins.filter(
        (pin: FlpBendPoint): boolean => clippedStart < pin.time && pin.time < clippedEnd,
      ),
      { time: clippedEnd, interval: bendIntervalAt(note.pins, clippedEnd) },
    ],
    absolutePins: FlpBendPoint[] = [];
  for (const pin of rawPins) {
    const time: number = Math.max(start, Math.min(end, quantizeFlTickToPart(pin.time, ppq))),
      pitch: number = clampEditorPitch(note.pitch + Math.round(pin.interval));
    pushBendPoint(absolutePins, time, pitch);
  }
  if (absolutePins.at(-1)!.time < end) {
    pushBendPoint(absolutePins, end, absolutePins.at(-1)!.interval);
  }
...
function notesToChords(notes: readonly ArrangedFlpNote[]): ArrangedChord[] {
  const chords: ArrangedChord[] = [],
    groups = new Map<string, ArrangedChord[]>();
...
function splitOverlaps(chords: readonly ArrangedChord[]): ArrangedChord[][] {
  ...
  for (const chord of chords) {
    let laneIndex = -1,
      earliestAvailableEnd = Number.POSITIVE_INFINITY;
...
function makeChannel(
  ...
  for (const chord of lane) {
    ...
    const firstBar: number = Math.floor(chord.start / partsPerBar),
      lastBar: number = Math.ceil(chord.end / partsPerBar);
    for (let bar = firstBar; bar < lastBar && bar < barCount; bar++) {
      const barStart: number = bar * partsPerBar,
        start: number = Math.max(chord.start, barStart) - barStart,
        end: number = Math.min(chord.end, barStart + partsPerBar) - barStart;
      if (start >= end) {
        continue;
      }
      ...
      const absoluteStart: number = barStart + start,
        absoluteEnd: number = barStart + end,
        startInterval: number = Math.round(bendIntervalAt(chord.pins, absoluteStart)),
        note = new Note(chord.pitches[0]! + startInterval, start, end, chord.size);
      note.pitches = chord.pitches.map((pitch: number): number => pitch + startInterval);
      const segmentPins: FlpBendPoint[] = [
        ...
        ...chord.pins.filter(
          (pin: FlpBendPoint): boolean => absoluteStart < pin.time && pin.time < absoluteEnd,
        ),
        {
          time: absoluteEnd,
      ...
      note.pins = segmentPins.map((pin: FlpBendPoint) =>
        makeNotePin(Math.round(pin.interval) - startInterval, pin.time - absoluteStart, chord.size),
      );
  ...
  channel.octave = Math.max(0, Math.min(Config.pitchOctaves - 1, Math.floor(averagePitch / 12)));
  return channel;
}
...
function expandArrangement(
  ...
  const patternsById = new Map(project.patterns.map((pattern) => [pattern.id, pattern] as const)),
    notesByChannel = new Map<number, ArrangedFlpNote[]>(),
    tracksByChannel = new Map<number, Set<number>>();
  let arrangementEndTick = 0,
    order = 0;

  for (const clip of arrangement.clips) {
    const pattern = patternsById.get(clip.patternId);
    if (pattern === undefined || clip.length === 0) {
      continue;
    }
    ...
    const expandedNotes: ExpandedFlpNote[] = pattern.notes.map(
      (flNote, noteOrder: number): ExpandedFlpNote => {
        const start: number = clip.position + flNote.position - sourceStart,
          end: number = start + Math.max(1, flNote.length);
        return {
          start,
          end,
    ...
    for (const expanded of applyFlSlides(expandedNotes)) {
      ...
      const clippedStart: number = Math.max(expanded.start, clip.position),
        clippedEnd: number = Math.min(expanded.end, clip.position + sourceEnd - sourceStart);
      ...
      let channelNotes: ArrangedFlpNote[] | undefined = notesByChannel.get(expanded.channelId);
      if (channelNotes === undefined) {
        channelNotes = [];
        notesByChannel.set(expanded.channelId, channelNotes);
      }
      channelNotes.push(arranged);
      let channelTracks: Set<number> | undefined = tracksByChannel.get(expanded.channelId);
...
export function convertFlpProject(project: FlpProject): FlpSongImport {
  ...
  const beatsPerBar: number = Math.max(
    ...
    { notesByChannel, tracksByChannel, arrangementEndPart } = expandArrangement(
      project,
      arrangement,
    );
  ...
  for (const notes of notesByChannel.values()) {
    for (const note of notes) {
      finalNoteEnd = Math.max(finalNoteEnd, note.end);
    }
  }
  const songEndPart: number = Math.max(arrangementEndPart, finalNoteEnd, 1),
    barCount: number = Math.ceil(songEndPart / partsPerBar);
  if (barCount > Config.barCountMax) {
    throw new FlpImportError("corrupt", "FLP arrangement exceeds GoopBox's song length limit.");
  }
  ...
  const pitchChannels: Channel[] = [],
    channelSources: FlpChannelSource[] = [],
    sourceChannelsById = new Map<number, FlpChannel>(
  ...
  for (const channelId of sourceChannelOrder) {
    const sourceNotes: ArrangedFlpNote[] = notesByChannel.get(channelId)!,
      chords: ArrangedChord[] = notesToChords(sourceNotes),
      lanes: ArrangedChord[][] = splitOverlaps(chords),
      source: FlpChannel | undefined = sourceChannelsById.get(channelId),
      playlistTracks: number[] = [...(tracksByChannel.get(channelId) ?? [])].sort((a, b) => a - b),
      playlistTrackNames: string[] = playlistTracks.flatMap((trackIndex: number): string[] => {
  ...
  return {
    tempo: Math.max(Config.tempoMin, Math.min(Config.tempoMax, Math.round(project.tempo))),
    beatsPerBar,
    barCount,
    pitchChannels,
    noiseChannels: [],
...
export { FlpImportError } from "./flp-parser.js";
