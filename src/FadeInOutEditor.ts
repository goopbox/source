...
export class FadeInOutEditor {
  ...
  private _fadeInToX(fadeIn: number) {
    return (
      1.0 +
      ((this._editorWidth - 2.0) * 0.4 * fadeIn) / (Config.fadeInRange - 1)
    );
  }
  private _xToFadeIn(x: number) {
    return clamp(
      ...
      Config.fadeInRange,
      Math.round(
        ((x - 1.0) * (Config.fadeInRange - 1)) /
          (0.4 * this._editorWidth - 2.0),
      ),
    );
  }
  private _fadeOutToX(fadeOut: number) {
    return (
      1.0 +
      (this._editorWidth - 2.0) *
        (0.5 + (0.5 * fadeOut) / (Config.fadeOutTicks.length - 1))
    );
  }
    ...
    return clamp(
      ...
      Math.round(
        ((Config.fadeOutTicks.length - 1) *
          ((x - 1.0) / (this._editorWidth - 2.0) - 0.5)) /
          0.5,
      ),
    );
  }
