...
import { FlpImportError, type FlpSongImport, importFlp } from "./FlpImport.js";
...
export class ImportFile {
  private readonly _fileInput: HTMLInputElement = input({
    type: "file",
    accept:
      ".goop,application/octet-stream,.mid,.midi,audio/midi,audio/x-midi,.flp",
  });

  constructor(
    private _doc: SongDocument,
    private readonly _configureFlpImport?: (
  ...
  private _whenFileSelected = (): void => {
    ...
    if (extension == "goop") {
      ...
      reader.addEventListener("load", (_event: Event): void => {
        try {
          const change: ChangeSong = new ChangeSong(
            this._doc,
            new Uint8Array(<ArrayBuffer>reader.result),
          );
      ...
      reader.addEventListener("error", (): void =>
        this._reportGoopImportError(reader.error),
      );
      ...
      reader.addEventListener("load", (_event: Event): void => {
        try {
          const imported: FlpSongImport = importFlp(<ArrayBuffer>reader.result);
          if (this._configureFlpImport == undefined) {
            this._applyFlpImport(imported);
          } else {
            this._configureFlpImport(imported, (): void =>
              this._applyFlpImport(imported),
            );
          }
        } catch (error) {
  ...
  private _reportGoopImportError(error: unknown): void {
    console.error("Could not import goopbox song.", error);
    window.alert("Invalid or unsupported .goop file.");
  }

  private _reportFlpImportError(error: unknown): void {
    console.error("Could not import FLP file.", error);
    const kind: string | undefined =
      error instanceof FlpImportError ? error.kind : undefined;
    if (kind == "invalid") {
  ...
  private _applyFlpImport(imported: FlpSongImport): void {
  ...
  private _parseMidiFile(buffer: ArrayBuffer): void {
    ...
    while (reader.hasMore()) {
      ...
      if (chunkType == MidiChunkType.header) {
        ...
        const trackReader: ArrayBufferReader =
          reader.getReaderForNextBytes(chunkLength);
    ...
    const independentTracks: boolean =
      fileFormat == MidiFileFormat.independentTracks;
    ...
    const beatsPerMinute: number = Math.max(
      ...
      Math.min(
        Config.tempoMax,
        Math.round(microsecondsPerMinute / microsecondsPerBeat),
      ),
    );
    ...
    const songTotalBars: number = Math.ceil(
      currentMidiTick / midiTicksPerPart / partsPerBar,
    );
    ...
    for (let midiChannel: number = 0; midiChannel < 16; midiChannel++) {
      ...
      const isNoiseChannel: boolean =
        isDrumsetChannel ||
        (channelPreset != null && channelPreset.isNoise == true);
      ...
      const channelMaxPitch: number = isNoiseChannel
        ? Config.drumCount - 1
        : Config.maxPitch;
      ...
      if (isDrumsetChannel) {
        ...
        const presetValue: number =
          EditorConfig.nameToPresetValue("standard drumset")!;
        ...
        for (
          ...
          if (
            ...
            for (const pitch of heldPitches) {
              ...
              noteSize = Math.max(
                noteSize,
                Math.round(drum.volume * currentVelocity),
              );
            ...
            for (
              ...
              const heldPitch: number =
                drumFreqs[
                  pitchIndex +
                    Math.max(0, drumFreqs.length - Config.maxChordSize)
                ];
        ...
        for (let noteEvent of noteEvents[midiChannel]) {
          ...
          if (heldPitches.length > 0 && nextEventPart > prevEventPart) {
            ...
            for (let bar: number = startBar; bar < endBar; bar++) {
              const barStartPart: number = bar * partsPerBar;
              const barStartMidiTick: number =
                bar * beatsPerBar * midiTicksPerBeat;
              const barEndMidiTick: number =
                (bar + 1) * beatsPerBar * midiTicksPerBeat;

              const noteStartPart: number = Math.max(
                0,
                prevEventPart - barStartPart,
              );
              const noteEndPart: number = Math.min(
                partsPerBar,
                nextEventPart - barStartPart,
              ...
              const noteEndMidiTick: number = Math.min(
                barEndMidiTick,
                nextEventMidiTick,
              );
              ...
              if (noteStartPart < noteEndPart) {
                ...
                const preset: Preset | null =
                  presetValue == null
                    ? null
                    : EditorConfig.valueToPreset(presetValue);
                ...
                // zero volume.
                if (instrument != null) {
                  instrument.volume = Math.min(
                    instrument.volume,
                    currentInstrumentVolume,
                  );
                  instrument.pan = Math.min(
                    instrument.pan,
                ...
                const initialEditorPitch: number = Math.round(
                ...
                let firstPin: NotePin = makeNotePin(
                  0,
                  0,
                ...
                let prevPartPitch: number =
                  (shiftedHeldPitch + currentMidiInterval) / intervalScale;
                let prevPartSize: number =
                  currentVelocity * currentMidiNoteSize;
                for (
                  let part: number = noteStartPart + 1;
                  part <= noteEndPart;
                  part++
                ) {
                  ...
                  const keyPitch: boolean =
                    pitchIsNearInteger || pitchCrossedInteger;
                  ...
                  const keySize: boolean =
                    sizeIsNearInteger || sizeCrossedInteger;
                  ...
                  if (keyPitch || keySize || lastPart) {
                    ...
                    if (addPin) {
                      const toBePinned: PotentialPin =
                        potentialPins[addPinAtIndex];
                      note.pins.push(
                        makeNotePin(
                ...
                for (const notePin of note.pins) {
                  maxPitch = Math.min(
                    maxPitch,
                    channelMaxPitch - notePin.interval,
                  );
                ...
                for (
                  let pitchIndex: number = 0;
                  pitchIndex <
                  Math.min(Config.maxChordSize, heldPitches.length);
                  pitchIndex++
                ) {
                  let heldPitch: number =
                    heldPitches[
                      pitchIndex +
                        Math.max(0, heldPitches.length - Config.maxChordSize)
                    ] * midiIntervalScale;
                  if (
                    preset != null &&
    ...
    function compactChannels(channels: Channel[], maxLength: number): void {
      while (channels.length > maxLength) {
        ...
        for (
          let channelIndexA: number = 0;
          channelIndexA < channels.length - 1;
          channelIndexA++
        ) {
          for (
            ...
            for (
              ...
              barIndex < channelA.bars.length &&
              barIndex < channelB.bars.length;
              barIndex++
            ) {
        ...
        for (
          ...
          if (channelA.bars[barIndex] == 0 && channelB.bars[barIndex] != 0) {
            channelA.bars[barIndex] =
              channelB.bars[barIndex] + channelAPatternCount;
          }
        }
    ...
    class ChangeImportMidi extends ChangeGroup {
      constructor(doc: SongDocument) {
        ...
        song.key = key;
        song.scale =
          Config.scales.dictionary[isMinor ? "Minor" : "Major"].index;
        song.rhythm = 1;
        removeDuplicatePatterns(pitchChannels);
        removeDuplicatePatterns(noiseChannels);

        this.append(
          new ChangeReplacePatterns(doc, pitchChannels, noiseChannels),
        );
        song.loopStart = 0;
        song.loopLength = song.barCount;
        this._didSomething();
