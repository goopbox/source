...
test("song binary rejects the old uncompressed version 1 format", async (context) => {
  ...
  const song = new Song();
  song.tempo = 203;
  const legacy = encodeLegacySongBinary(encodeBinaryValue(song.toBinaryObject()));
...
test("song binary rejects the old compact song body", async (context) => {
  ...
  assert.throws(() => new Song(encodeSongBinary(compact)), /Invalid compact \.goop song data/);
});
...
test("compact song binary omits default instrument state and keeps links small", async (context) => {
  ...
  assert.ok(empty.length < 100, `default song should stay compact, got ${empty.length} bytes`);
  ...
  assert.ok(binary.length < 200, `64-note song should stay compact, got ${binary.length} bytes`);
...
test("compact song binary resolves SoundFont asset references through the asset table", async (context) => {
  const { Song, decodeSongBinary, encodeSongBinary, cleanup } = await loadSongModules();
  context.after(cleanup);
  const source = "https://example.com/soundfonts/test.sf2",
    songObject = new Song().toBinaryObject();
  ...
  const song = new Song(encodeSongBinary(songObject)),
    compact = decodeSongBinary(song.toBinary());
  assert.equal(compact[0], 2);
  const restored = new Song(song.toBinary());
  assert.equal(restored.channels[0].instruments[0].soundFontId, `asset:${source}`);
  assert.deepEqual(
...
test("song binary rejects unsupported, corrupt, and truncated files", async (context) => {
  ...
  const valid = new Song().toBinary(),
    unsupportedVersion = valid.slice();
  unsupportedVersion[4] = 255;
  assert.throws(() => new Song(unsupportedVersion), /Unsupported \.goop version: 255/);
...
test("song binary preserves Unicode asset and SoundFont identifiers", async (context) => {
  const { Song, encodeSongBinary, cleanup } = await loadSongModules();
  context.after(cleanup);
  const songObject = new Song().toBinaryObject();
  songObject.assets = ["https://example.com/音源.sf2", "!r61!https://example.com/échantillon.wav"];
  songObject.channels[0].instruments[0].soundFontId = "音源-é";
  songObject.channels[0].instruments[0].soundFontPreset = 0x1_23_45;
  const songData = encodeSongBinary(songObject),
    restored = new Song(songData);
  ...
  assert.equal(restored.channels[0].instruments[0].soundFontPreset, 0x1_23_45);
  assert.deepEqual(new Song(restored.toBinary()).toBinaryObject(), restored.toBinaryObject());
});
...
test("binary codec works without TextEncoder and TextDecoder globals", async (context) => {
  ...
  try {
    ...
    const { decodeBinaryValue, encodeBinaryValue, cleanup } = await loadSongModules();
    context.after(cleanup);
...
test("compact binary object schemas round-trip and reduce repeated field names", async (context) => {
  ...
  const value = {
      channels: Array.from({ length: 16 }, (_, index) => ({
        ...
        patterns: [
          {
            notes: [{ start: 0, pitches: [12, 16, 19], continuesLastPattern: false }],
          },
        ],
      })),
    },
    verbose = encodeBinaryValue(value),
    compact = encodeBinaryValue(value, true);
...
test("binary codec stores byte arrays without expanding them into number arrays", async (context) => {
  ...
  const bytes = Uint8Array.from({ length: 200 }, (_, index) => index),
    encoded = encodeBinaryValue(bytes);
...
test("binary values preserve a leading BOM and reject impossible array lengths", async (context) => {
  ...
  assert.throws(() => decodeBinaryValue(impossibleArray), /Truncated binary value/);
});
...
test("song binary rejects JSON and invalid signatures", async (context) => {
  ...
  assert.throws(() => new Song(invalidSignature), /Invalid \.goop file signature/);
});
