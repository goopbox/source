...
async function loadPreferences() {
  const directory = await mkdtemp(join(tmpdir(), "goopbox-preferences-test-")),
    outfile = join(directory, "preferences.mjs");
  await build({
...
test("master volume uses a perceptual curve from 5% to 1x", async (context) => {
  ...
  assert.ok(
    Math.abs(Preferences.masterVolumeToGain(50.5) - Math.sqrt(Preferences.masterVolumeMinGain)) <
      1e-12,
  );
  assert.ok(Math.abs(Preferences.masterVolumeToGain(Preferences.maxMasterVolume) - 1.0) < 1e-12);
