...
class NativeHistory {
  ...
  // oxlint-disable-next-line typescript/class-literal-property-style -- This is JavaScript.
  get length() {
    return 1;
  }
  replaceState(state, _unused, url) {
    if (this.failNextReplace) {
      this.failNextReplace = false;
      throw new DOMException("The history entry could not be updated.", "QuotaExceededError");
    }
    const href =
      url === undefined
...
async function loadSongHistory() {
  ...
  await build({
    stdin: {
      contents: [
        'export {SongDocument} from "./src/song-document.ts";',
        'export {encodeSongUrl, decodeSongUrl, decodeSongUrlHash} from "./src/song-url.ts";',
        'export {encodeSongBinary, decodeSongBinary, extractCompressedSongBody} from "./synth/song-binary.ts";',
        'export {Event, EventPoint, ChannelKind, Instrument, Note, Pattern, Song} from "./synth/synth.ts";',
        'export {Config} from "./synth/synth-config.ts";',
...
function makeLargeSong(Song, Pattern, Note, Config, ChannelKind) {
  ...
  while (song.pitchChannelCount < 3) {
    song.channels.splice(song.pitchChannelCount, 0, song.createChannel(ChannelKind.pitch));
    ...
    for (let patternIndex = 0; patternIndex < patternCount; patternIndex++) {
      ...
      for (let noteIndex = 0; noteIndex < noteCount; noteIndex++) {
        const noise = channelIndex >= song.pitchChannelCount,
          ...
          size = 1 + ((patternIndex * 19 + noteIndex * 23 + channelIndex * 29) % 10),
          note = new Note(pitch, noteIndex, noteIndex + 1, size),
          maximumPitch = noise ? Config.drumCount - 1 : Config.maxPitch;
        note.pins[1].interval = Math.max(
          -pitch,
          Math.min(maximumPitch - pitch, ((patternIndex + noteIndex * 3) % 5) - 2),
        );
...
function recordChange(browser, doc, change) {
  assert.equal(change.isNoop(), false, "history integration changes must produce an undo boundary");
...
test("undo history is durable, contiguous, exact, and crash resistant", async (context) => {
  ...
  await context.test("Automation channels survive history undo, redo, and reload", () => {
    const browser = new FakeBrowser(),
      doc = new SongDocument();
    new ChangeChannelCount(doc, doc.song.pitchChannelCount, doc.song.noiseChannelCount, 0);
    doc.updateCurrentHistoryEntry();
    const group = new ChangeGroup();
    group.append(
      new ChangeChannelCount(doc, doc.song.pitchChannelCount, doc.song.noiseChannelCount, 1),
    );
    ...
    group.append(new ChangeEvents(doc, pattern.automationEvents[0], [automationEvent]));
    assert.deepEqual(
      doc.song.channels[channelIndex].patterns.map(
        (candidate) => candidate.automationEvents.length,
      ),
      new Array(doc.song.patternsPerChannel).fill(
    ...
    assert.equal(
      restored.song.channels.at(-1).patterns[0].automationEvents[0][0].points[1].value,
      180.75,
    );
  });

  await context.test("adding channels at a type boundary records each undo step", () => {
    for (const kind of [ChannelKind.pitch, ChannelKind.noise, ChannelKind.automation]) {
      const browser = new FakeBrowser(),
        doc = new SongDocument(),
        channelIndex =
  ...
  await context.test("large histories survive undo, redo, reload, and branching", () => {
    const largeSong = makeLargeSong(Song, Pattern, Note, Config, ChannelKind),
      serializedSong = largeSong.toBinary(),
      sharedHash = encodeSongUrl(serializedSong);
    assert.ok(serializedSong.length > 8000, "fixture must exercise a large compressed song");
    assert.ok(sharedHash.length > 10_000, "fixture must exercise an unusually large shared URL");
    assert.match(
      sharedHash,
      /^#[A-Za-z0-9_-]+$/,
    ...
    assert.equal(reloadedDoc.song.tempo, 151, "reload should restore the current undo entry");
    assert.equal(reloadedDoc.hasRedoHistory(), true, "redo position should survive reload");
    reloadedDoc.redo();
    ...
    reloadedDoc.redo();
    assert.equal(reloadedDoc.song.tempo, 153, "redo must not resurrect the discarded branch");
    assert.deepEqual(
      historyTempos(storedHistory(browser), Song, decodeSongUrlHash),
      [150, 151, 153],
    );
    ...
    const standardBase64Hash = sharedHash.replaceAll("-", "+").replaceAll("_", "/");
    assert.notEqual(standardBase64Hash, sharedHash);
    browser.location.href = `http://localhost:8080/${standardBase64Hash}`;
    assert.equal(decodeSongUrl(), null, "standard Base64 song URLs must not be accepted");
  });

  await context.test("legacy JSON URLs and undo manifests are not migrated", () => {
    ...
    assert.equal(doc.song.tempo, 150, "legacy URL and native-history song data must be ignored");
  ...
  await context.test(
    ...
    () => {
      ...
      apply(new ChangeOctave(doc, doc.song.channels[0].octave, doc.song.channels[0].octave + 1));
      ...
      apply(new ChangeChorus(doc, instrument.chorus, (instrument.chorus + 1) % 4));
  ...
  await context.test(
    ...
    () => {
      ...
      const before = snapshotDocument(doc),
        sameChannel = new ChangeChannelOrder(doc, 1, 1, 0),
        sameBar = new ChangeBarOrder(doc, 3, 3);
      ...
      expectedChannelSong.channels.splice(2, 0, ...expectedChannelSong.channels.splice(0, 1));
      ...
      const afterChannel = snapshotDocument(doc),
        expectedBarSong = new Song(afterChannel.song);
  ...
  await context.test("undo preserves mute state and disabled effect settings", () => {
    const browser = new FakeBrowser(),
      doc = new SongDocument(),
      channel = doc.song.channels[0],
      instrument = channel.instruments[0];
    channel.muted = true;
    instrument.chorus = 1;
    instrument.reverb = 6;
    ...
    assert.deepEqual(doc.song.channels[0].instruments[0].toBinaryState(), exactSettings);
    doc.redo();
    assert.equal(doc.song.channels[0].muted, true);
    assert.deepEqual(doc.song.channels[0].instruments[0].toBinaryState(), exactSettings);
  });

  await context.test("quota trimming keeps a contiguous suffix of the newest progress", () => {
    const fourEntryBrowser = new FakeBrowser(),
      fourEntryDoc = new SongDocument();
    ...
    assert.equal(doc.song.tempo, 151);
    doc.undo();
    assert.equal(doc.song.tempo, 151, "history before the retained suffix should be unavailable");
    doc.redo();
    assert.equal(doc.song.tempo, 152);

    const middleBrowser = new FakeBrowser(),
      middleDoc = new SongDocument();
  ...
  await context.test(
    ...
    () => {
      ...
      doc.redo();
      assert.equal(browser.confirmations.length, 1);
      assert.match(browser.confirmations[0], /storage is full|storage is.*unavailable/i);
      ...
      assert.equal(currentSong.tempo, 152, "the public URL should retain the latest progress");
    },
  );
  ...
  await context.test(
    ...
    () => {
      const pendingBrowser = new FakeBrowser(),
        pendingDoc = new SongDocument();
      recordTempo(pendingBrowser, pendingDoc, ChangeTempo, 151, false);
      pendingDoc.undo();
      assert.equal(pendingDoc.song.tempo, 150, "undo should flush the pending boundary first");
      ...
      const metadataBrowser = new FakeBrowser(),
        metadataDoc = new SongDocument();
      recordTempo(metadataBrowser, metadataDoc, ChangeTempo, 152);
      const metadataHistory = storedHistory(metadataBrowser);
      metadataHistory.entries[metadataHistory.currentIndex].bar = 999_999;
      metadataHistory.entries[metadataHistory.currentIndex].channel = 999_999;
      metadataHistory.entries[metadataHistory.currentIndex].viewedInstruments = [999_999];
      metadataBrowser.sessionStorage.setItem(historyStorageKey, JSON.stringify(metadataHistory));
      ...
      assert.ok(
        clampedDoc.viewedInstrument.every((value, channel) =>
          clampedDoc.song.channels[channel].instruments.length === 0
            ? value === 0
            : value >= 0 && value < clampedDoc.song.channels[channel].instruments.length,
        ),
      );

      const beforeCorruption = snapshotDocument(clampedDoc),
        invalidHash = encodeSongUrl(encodeSongBinary({ channels: [] }));
      ...
      corruptHistory.entries[corruptHistory.currentIndex].snapshot = "not-a-song-hash";
      metadataBrowser.sessionStorage.setItem(historyStorageKey, JSON.stringify(corruptHistory));
      metadataBrowser.reload();
      ...
      const nativeFailureBrowser = new FakeBrowser(),
        nativeFailureDoc = new SongDocument();
      ...
      const detachedHashBrowser = new FakeBrowser(),
        detachedHashDoc = new SongDocument();
      ...
      const constructorFailureBrowser = new FakeBrowser(),
        constructorFailureDoc = new SongDocument();
      recordTempo(constructorFailureBrowser, constructorFailureDoc, ChangeTempo, 151);
      const staleConstructorHistoryId = storedHistory(constructorFailureBrowser).id;
      ...
      assert.equal(storedHistory(foreignBrowser).entries.at(-1).mutedChannels[0], true);
      ...
      const assetSong = new Song(),
        assetSongObject = assetSong.toBinaryObject();
      ...
      const assetBrowser = new FakeBrowser(
        ...
        assetDoc = new SongDocument(),
        assetWaveCount = Config.chipWaves.length;
