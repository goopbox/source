...
async function loadSynthController() {
  const directory = await mkdtemp(join(tmpdir(), "goopbox-synth-controller-test-")),
    outfile = join(directory, "module.mjs");
  await build({
    entryPoints: ["synth/synth-controller.ts"],
    bundle: true,
    platform: "node",
    format: "esm",
    outfile,
...
test("song synchronization compares binary contents and channel mutes", async (context) => {
  ...
  controller.syncSong();
  runSyncFrame();
  assert.equal(commands.length, 1, "an unchanged binary song should not be posted again");

  controller.song.tempo++;
  controller.syncSong();
  runSyncFrame();
  assert.equal(commands.length, 2, "changed binary song contents should be posted");
...
test("tieover playback history follows audio reports and resets on seeks", async (context) => {
  ...
  assert.equal(
    controller.getPreviousBar(5),
    4,
    "interpolation crossing a bar uses the adjacent bar",
  );
  report(2.1, 7);
  ...
  for (const seek of [
    ...
    seek();
    assert.equal(
      controller.getPreviousBar(Math.floor(controller.playhead)),
      null,
      "seeks clear stale tieover history immediately",
    );
  }
  report(4.1, 3);
  ...
  assert.equal(
    controller.getPreviousBar(5),
    4,
    "next-bar navigation matches the synth's continuation behavior",
  );
});
...
test("offline synths receive the controller's loaded samples and SoundFonts", async (context) => {
  ...
  const sampleData = {
    ...
    soundFontData = new Uint8Array([1, 2, 3]).buffer;
  ...
  const received = [],
    ...
    loading = controller.loadAssetsInto(target);
...
test("sample previews use raw PCM and loop controls without starting the song", async (context) => {
  ...
  controller.sampleAssets.set("recording", { samples, sampleRate: 32_000 });
  let endedCount = 0;
  ...
  assert.equal(
    commands.some((command) => command.type === "play"),
    false,
  );
  assert.equal(createdBuffers.length, 1);
  assert.equal(createdBuffers[0].channelCount, 1);
  assert.equal(createdBuffers[0].sampleRate, 32_000);
  ...
  assert.equal(createdSources[0].loopStart, 1 / 32_000);
  assert.equal(createdSources[0].loopEnd, 3 / 32_000);
  assert.deepEqual(createdSources[0].startArguments, [0, 2 / 32_000]);
  ...
  assert.equal(createdSources[1].loopStart, 1 / 32_000);
  assert.equal(createdSources[1].loopEnd, 3 / 32_000);
  assert.deepEqual(createdSources[1].startArguments, [0, 1 / 32_000, 2 / 32_000]);
