...
async function loadSynth() {
  const directory = await mkdtemp(join(tmpdir(), "goopbox-history-state-test-")),
...
test("valid dormant instrument settings round-trip exactly through raw binary", async (context) => {
  ...
  instrument.soundFontPreset = 0x3_ff_ff;
...
test("song-specific string and asset collection limits reject CRC-valid oversized data", async (context) => {
  ...
  const { Song, decodeSongBinary, encodeSongBinary } = synth,
    validSongObject = new Song().toBinaryObject(),
    corruptions = [
      [
        "asset count",
        (state) => {
...
test("CRC-valid unsafe numeric domains and note bends are rejected without poisoning synthesis", async (context) => {
  const { synth, cleanup } = await loadSynth();
  context.after(cleanup);
  const { Config, Note, Song, SynthEngine, decodeSongBinary, encodeSongBinary } = synth,
    song = new Song();
  song.channels[0].instruments[0].volume = 50;
  song.channels[0].bars[0] = 1;
  song.channels[0].patterns[0].notes.push(new Note(24, 0, 12, Config.noteSizeMax));
  const validSongObject = song.toBinaryObject(),
    unsafeMagnitude = Number.MAX_VALUE,
    validEnvelope = () => ({
    ...
    validFilterPoint = () => ({ type: 0, freq: 0, gain: 0 }),
    corruptions = [
      [
        "instrument type",
        (state) => {
      ...
      [
        "preset",
        (state) => {
          firstInstrumentState(state).preset = 0x1_00_00_00_00;
        },
      ],
      [
        "chip wave",
      ...
      [
        "string sustain type",
        (state) => {
          firstInstrumentState(state).stringSustainType = Config.sustainTypeNames.length;
        },
      ],
      [
        "SoundFont preset",
        (state) => {
          firstInstrumentState(state).soundFontPreset = 0x4_00_00;
        },
      ],
      [
        "algorithm",
      ...
      [
        "operator frequency",
        (state) => {
          firstInstrumentState(state).operators[0].frequency = Config.operatorFrequencyMax + 1;
        },
      ],
      [
        "operator amplitude",
        (state) => {
          firstInstrumentState(state).operators[0].amplitude = Config.operatorAmplitudeMax + 1;
        },
      ],
      [
        "operator wave",
        (state) => {
          firstInstrumentState(state).operators[0].wave = Config.chipWaves.length + 1;
        },
      ],
      [
        "spectrum",
      ...
      [
        "drum envelope type",
        (state) => {
          firstInstrumentState(state).drumsetEnvelopes[0] = Config.envelopes.length;
        },
      ],
      [
        "drum envelope speed",
      ...
      [
        "drum spectrum",
        (state) => {
          firstInstrumentState(state).drumsetSpectra[0][0] = Config.spectrumMax + 1;
        },
      ],
      [
        "first pin interval",
      ...
      [
        "high pin interval",
        (state) => {
          state.channels[0].patterns[0].notes[0].pins[1].interval = Config.maxPitch;
        },
      ],
      [
        "low pin interval",
...
test("corrupt dormant indices and empty song structures are rejected atomically", async (context) => {
  const { synth, cleanup } = await loadSynth();
  context.after(cleanup);
  const { Config, Note, Song, encodeSongBinary } = synth,
    song = new Song(),
    songObject = song.toBinaryObject();
  ...
  const validSongObject = song.toBinaryObject(),
    before = song.toBinary(),
    channelReference = song.channels[0],
    instrumentReference = channelReference.instruments[0],
    chipWavesReference = Config.chipWaves,
    corruptions = [
      [
        "empty channels",
        (state) => {
  ...
  for (const [name, corrupt] of corruptions) {
    ...
    assert.deepEqual(song.toBinary(), before, `${name} must not mutate the song`);
    assert.strictEqual(
      song.channels[0],
      channelReference,
...
test("a late instrument binary error does not partially apply earlier fields", async (context) => {
  ...
  context.after(cleanup);
  const { Song } = synth,
    instrument = new Song().channels[0].instruments[0],
    before = instrument.toBinaryState(),
    corrupt = structuredClone(before);
