...
async function loadChanges() {
  const directory = await mkdtemp(join(tmpdir(), "goopbox-undoable-changes-test-")),
...
test("dragging a partial note can undo and redo without leaving split-note corruption", async (context) => {
  ...
  const song = new Song(),
    pattern = new Pattern(),
    note = new Note(24, 0, 12, 4);
  note.pitches = [24, 31];
  note.pins = [makeNotePin(0, 0, 4), makeNotePin(3, 6, 8), makeNotePin(-1, 12, 5)];
  note.continuesLastPattern = true;
  pattern.notes.push(note);

  const originalPins = note.pins,
    originalPinObjects = [...originalPins],
    before = snapshotNotes(pattern),
    doc = {
      song,
      channel: 0,
    ...
    change = new ChangeDragSelectedNotes(doc, 0, pattern, 1, 0),
    after = snapshotNotes(pattern);
...
test("scale transposition follows the composing key", async (context) => {
  const { ChangeTranspose, Note, Pattern, Song, makeNotePin, cleanup } = await loadChanges();
  context.after(cleanup);
  ...
  const pattern = new Pattern(),
    note = new Note(4, 0, 12, 4); // E in D major.
  note.pins = [
    makeNotePin(0, 0, 4),
  ...
  const doc = {
    ...
    change = new ChangeTranspose(doc, 0, pattern, true);
...
test("uniform note value bends use the shared Event value handler", async (context) => {
  ...
  note.pins = [makeNotePin(0, 0, 4), makeNotePin(3, 6, 8), makeNotePin(3, 12, 5)];
  const doc = { notifier: { changed() {} } },
    change = new ChangeSizeBend(doc, note, 6, 7, 3, true);
  assert.deepEqual(
    note.pins.map((pin) => pin.size),
    [7, 7, 7],
  );
  assert.deepEqual(
    note.pins.map((pin) => pin.interval),
