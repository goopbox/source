...
async function loadCategoryHelpers() {
  const directory = await mkdtemp(join(tmpdir(), "goopbox-settings-category-test-")),
    outfile = join(directory, "category.mjs");
  await build({
    entryPoints: ["src/instrument-settings-category.ts"],
    bundle: true,
    platform: "node",
    format: "esm",
    outfile,
...
test("instrument settings category copies contain only their category", async (context) => {
  ...
  assert.deepEqual(copyInstrumentSettingsCategory(instrument, "effects").settings, {
    effects: ["reverb"],
    eqFilter: [],
    reverb: 50,
  });
  assert.deepEqual(copyInstrumentSettingsCategory(instrument, "envelopes").settings, {
    envelopes: [{ target: "noteVolume", envelope: "twang" }],
  });
});
...
test("category paste replaces only that category and removes stale fields", async (context) => {
  const { helpers, cleanup } = await loadCategoryHelpers();
  context.after(cleanup);
  const { copyInstrumentSettingsCategory, pasteInstrumentSettingsCategory } = helpers,
    chorusInstrument = { ...instrument, effects: ["chorus"], chorus: 75 };
  delete chorusInstrument.reverb;
  const copy = copyInstrumentSettingsCategory(chorusInstrument, "effects"),
    pasted = pasteInstrumentSettingsCategory(instrument, copy);
...
test("category clipboard validation requires the requested category", async (context) => {
  const { helpers, cleanup } = await loadCategoryHelpers();
  context.after(cleanup);
  const { copyInstrumentSettingsCategory, isInstrumentSettingsCategoryCopy } = helpers,
    copy = copyInstrumentSettingsCategory(instrument, "specific");
  ...
  assert.equal(
    isInstrumentSettingsCategoryCopy({ category: "specific", settings: {} }, "specific"),
    true,
  );
  assert.equal(isInstrumentSettingsCategoryCopy({ category: "specific" }, "specific"), false);
});
