...
function attachInputFile(createdElements, ImportFile, doc, file) {
  const previousCount = createdElements.length,
    importer = new ImportFile(doc),
    input = createdElements.slice(previousCount).find((element) => element.tagName === "INPUT");
...
test("ImportFile handles binary .goop songs atomically", async (context) => {
  const globalNames = [
    ...
    errorDescriptor = Object.getOwnPropertyDescriptor(console, "error");
  context.after(() => {
    ...
    if (errorDescriptor === undefined) {
      ...
      Object.defineProperty(console, "error", errorDescriptor);
    }
  });
  ...
  await context.test("the picker advertises .goop but not .json", () => {
    const { doc } = makeDocument(Song),
      { importer, input } = attachInputFile(createdElements, ImportFile, doc, {
        name: "unused",
        bytes: new Uint8Array(),
      }),
      acceptedTypes = input.getAttribute("accept").split(",");
  ...
  for (const fileName of ["song.goop", "song.GOOP"]) {
    await context.test(`${fileName} is read as binary and applied`, () => {
      ...
      const file = { name: fileName, bytes },
        { importer, input } = attachInputFile(createdElements, ImportFile, doc, file);
  ...
  await context.test("corrupt .goop data alerts without changing the current song", () => {
    FakeFileReader.reads.length = 0;
    alerts.length = 0;
    errors.length = 0;
    const { calls, doc } = makeDocument(Song);
    ...
    const before = doc.song.toBinary(),
      corrupt = before.slice();
    corrupt[corrupt.length - 1] ^= 1;
    const { importer, input } = attachInputFile(createdElements, ImportFile, doc, {
      name: "corrupt.goop",
      bytes: corrupt,
    });
  ...
  await context.test(".json files are unsupported", () => {
    ...
    doc.song.tempo = 181;
    const before = doc.song.toBinary(),
      json = new TextEncoder().encode('{"format":"goopbox-1"}'),
      { importer, input } = attachInputFile(createdElements, ImportFile, doc, {
        name: "legacy.json",
        bytes: json,
      });
