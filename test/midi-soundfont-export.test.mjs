...
async function loadModules() {
  const directory = await mkdtemp(join(tmpdir(), "goopbox-midi-soundfont-export-test-")),
  ...
  await build({
    stdin: {
      contents: [
        ...
        'export {InstrumentType} from "./synth/synth-config.ts";',
...
function readMidiPrograms(bytes) {
  ...
  while (chunkStart < bytes.length) {
    const chunkLength = view.getUint32(chunkStart + 4, false),
      state = { index: chunkStart + 8 },
      chunkEnd = state.index + chunkLength;
    while (state.index < chunkEnd) {
      ...
      if (status === 0xff) {
        ...
        if (eventType === 0xc0) {
          programs.push(bytes[state.index]);
        }
...
test("SoundFont instruments export to MIDI programs", async (context) => {
  ...
  assert.equal(module.getSoundFontMidiProgram([{ index: 7, program: 128 }], 7), 80);
});
