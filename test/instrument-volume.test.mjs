...
async function loadSynth() {
  const directory = await mkdtemp(join(tmpdir(), "goopbox-instrument-volume-test-")),
...
test("instrument volume uses a perceptual curve from 5% to 6x", async (context) => {
  ...
  const middleSetting = 50,
    ratio = (middleSetting - 1) / (Config.volumeRange - 2),
    expected = Config.volumeMinGain * (Config.volumeMaxGain / Config.volumeMinGain) ** ratio;
  assert.ok(Math.abs(Synth.instrumentVolumeToVolumeMult(middleSetting) - expected) < 1e-12);
...
test("instrument volume conversion still round-trips every setting", async (context) => {
  ...
  for (let setting = 0; setting < synth.Config.volumeRange; setting++) {
    assert.equal(
      synth.Synth.volumeMultToInstrumentVolume(synth.Synth.instrumentVolumeToVolumeMult(setting)),
      setting,
    );
  }
});
...
test("instrument default volume is near 1x on the new curve", async (context) => {
  ...
  assert.ok(Math.abs(synth.Synth.instrumentVolumeToVolumeMult(instrument.volume) - 1.0) < 0.01);
