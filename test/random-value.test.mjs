...
async function loadRandomValue(context) {
  ...
  await build({
    entryPoints: ["src/random-value.ts"],
    bundle: true,
    platform: "node",
    format: "esm",
    outfile,
...
test("random effect percentages use native whole display values", async (context) => {
  const { selectCurvedValue } = await loadRandomValue(context),
    values = samples.map((random) => selectCurvedValue(1, 100, 100 / 3, 100 / 3, 1, () => random));
  assert.ok(values.every((value) => Number.isInteger(value) && value >= 1 && value <= 100));
  assert.ok(new Set(values).size > 3, "reverb should not be limited to legacy storage steps");
});

test("random special effect values respect their display bounds and snapping", async (context) => {
  const { selectCurvedValue } = await loadRandomValue(context),
    pitchShiftValues = samples.map((random) => selectCurvedValue(-12, 12, 0, 2, 1, () => random));
  assert.ok(
    pitchShiftValues.every((value) => Number.isInteger(value) && value >= -12 && value <= 12),
  );
  ...
  assert.ok(
    echoDelayValues.every((value) => value >= 0.25 && value <= 2 && Number.isInteger(value * 4)),
  );
});
