  ...
  await build({
    stdin: {
      contents: [
        ...
        'export {TimeStretch} from "./synth/time-stretch.ts";',
        'export {Config, EffectType, InstrumentType, parseAssetDefinition} from "./synth/synth-config.ts";',
...
function addSampleAsset(module, song) {
  ...
  const waveIndex = module.Config.chipWaves.findIndex((wave) => wave.sampleId === asset.id);
...
function configureShortSong(module, instrumentType, pitch, tempo) {
  const { Config, InstrumentType, Note, Song } = module,
    song = new Song(),
    { asset, waveIndex } = addSampleAsset(module, song);
  song.tempo = 300;
  song.beatsPerBar = 1;
  song.barCount = 1;
  song.loopStart = 0;
  ...
  song.channels[0].patterns[0].notes.push(new Note(48, 0, Config.partsPerBeat, Config.noteSizeMax));
  return { asset, song };
}
...
function renderSignal(module, engine, length, chunk = length) {
  const output = new Float32Array(length),
    raw = [],
    apply = module.Synth.applyFilters;
  ...
  try {
    for (let offset = 0; offset < length; offset += chunk) {
      ...
      engine.synthesize(output.subarray(offset, offset + count), new Float32Array(count), count);
    }
  } finally {
...
function renderSourcePhase(module, instrumentType, pitch, tempo) {
  const { asset, song } = configureShortSong(module, instrumentType, pitch, tempo),
...
function configureCatchUpSong(module, instrumentType) {
  const { Config, makeNotePin } = module,
    { asset, song } = configureShortSong(module, instrumentType, 137, 83);
  song.tempo = 125;
  const instrument = song.channels[0].instruments[0],
    settings =
...
function renderCatchUpState(module, instrumentType, seekToMiddle) {
  ...
  const samples = new Float32Array(65_536);
  for (let i = 0; i < samples.length; i++) {
    samples[i] = Math.sin((i * Math.PI * 2) / 97);
  }
  engine.setAsset(asset.id, samples, 48_000);
  const seekFraction = 0.75,
    samplesBeforeSeek = engine.getSamplesPerBar() * seekFraction;
  assert.equal(Number.isInteger(samplesBeforeSeek), true);
  if (seekToMiddle) {
    engine.playhead = seekFraction;
  }
  engine.play();
  const renderedSamples = seekToMiddle ? 1 : samplesBeforeSeek + 1,
    { output, raw } = renderSignal(module, engine, renderedSamples);
...
function assertCatchUpApproximatesContinuousPlayback(module, instrumentType) {
  const continuous = renderCatchUpState(module, instrumentType, false),
    caughtUp = renderCatchUpState(module, instrumentType, true);
  assert.ok(
...
function configureExtendedCatchUpSong(module, instrumentType) {
  ...
  for (let bar = 0; bar < song.barCount; bar++) {
    const pattern = new Pattern(),
      note = new Note(pitch, 0, partsPerBar, Config.noteSizeMax);
    note.pins = [
      makeNotePin(0, 0, Config.noteSizeMax),
...
function assertExtendedCatchUpApproximatesContinuousPlayback(module, instrumentType) {
  const continuous = renderExtendedCatchUpState(module, instrumentType, false),
    caughtUp = renderExtendedCatchUpState(module, instrumentType, true);
  assert.ok(
...
test("pitch and notes are separate effect categories without a pitch/tempo effect", async (context) => {
  ...
  const pitch = Config.effectGroups.find((group) => group.name === "Pitch"),
    notes = Config.effectGroups.find((group) => group.name === "Notes");
  assert.notEqual(pitch, undefined);
  assert.notEqual(notes, undefined);
  assert.equal(
    Config.effectGroups.some((group) => group.name === "Pitch & Notes"),
    false,
  );
...
test("chip instruments and FM operators use the same decimal pitch/tempo settings", async (context) => {
  ...
  chip.chipWaveSettings.pitch = 12_345.6789;
  ...
  assert.equal(chipSettings.pitchPercent, 12_345.6789);
  ...
  assert.equal(restoredChip.chipWaveSettings.pitch, 12_345.6789);
...
test("chip-wave pitch/tempo keeps song version 1 and round-trips unrestricted binary values", async (context) => {
  ...
  instrument.chipWaveSettings.pitch = 123_456_789.125;
  instrument.chipWaveSettings.tempo = -98_765_432.875;
  instrument.operators[0].chipWaveSettings.pitch = -24_680.13579;
  instrument.operators[0].chipWaveSettings.tempo = 13_579.2468;
  ...
  assert.equal(restoredInstrument.chipWaveSettings.pitch, 123_456_789.125);
  assert.equal(restoredInstrument.chipWaveSettings.tempo, -98_765_432.875);
  assert.equal(restoredInstrument.operators[0].chipWaveSettings.pitch, -24_680.13579);
  assert.equal(restoredInstrument.operators[0].chipWaveSettings.tempo, 13_579.2468);
...
test("chip-wave sampler wraps at loop points and one-shot stops at loop end", async (context) => {
  ...
  assert.equal(Synth.interpolateChipWaveSample(wave, 6.25, 2, 6, false), 2.25);
  assert.equal(Synth.interpolateChipWaveSample(wave, 6.25, 2, 6, true), 0);
});
...
test("sample-backed chip and FM tones initialize at their configured offsets", async (context) => {
  ...
  for (const instrumentType of [module.InstrumentType.chip, module.InstrumentType.fm]) {
...
test("sample-backed chip and FM paths share one sampler and decouple source tempo from pitch", async (context) => {
  ...
  assert.ok(
    Math.abs(chipPitchA - chipPitchB) < 1e-9,
    "chip source tempo must not change with pitch",
  );
  const chipTempoB = renderSourcePhase(module, module.InstrumentType.chip, 70, 80);
  assert.ok(
    Math.abs(chipPitchA - chipTempoB) > 1e-4,
    "chip source position must change with tempo",
  );
...
test("extended chip waves catch up across 16 continued bars and their pitch bends", async (context) => {
  ...
  assertExtendedCatchUpApproximatesContinuousPlayback(module, module.InstrumentType.chip);
});
...
test("extended FM operator waves catch up across 16 continued bars and their pitch bends", async (context) => {
  ...
  assertExtendedCatchUpApproximatesContinuousPlayback(module, module.InstrumentType.fm);
});
...
test("song rendering includes loaded custom sample PCM", async (context) => {
  ...
  const { asset, song } = configureShortSong(module, module.InstrumentType.chip, 137, 83),
  ...
  assertAudioClose(
    renderer.outputSamplesL,
    renderSignal(module, engine, renderer.outputSamplesL.length, 37).output,
  );
});

function makeSignalEngine(module, type, pitch = 100, tempo = 100, oneshot = true) {
...
test("100% pitch and tempo bypass the stretcher and reproduce direct sample playback", async (context) => {
  ...
  context.after(module.cleanup);
  context.after(() => module.Config.configureAssets([]));
  module.TimeStretch.prototype.prepare = () => assert.fail("identity playback must never run DSP");
  for (const type of [module.InstrumentType.chip, module.InstrumentType.fm]) {
    const { engine, samples } = makeSignalEngine(module, type),
      { raw } = renderSignal(module, engine, 3000, 37),
      gain = raw[1] / samples[1];
    assert.ok(gain > 0);
    assertAudioClose(
      raw,
      Float32Array.from(samples.subarray(0, raw.length), (sample) => sample * gain),
    );
    const tone = engine.channels[0].instruments[0].activeTones.get(0);
  ...
  for (const type of [module.InstrumentType.chip, module.InstrumentType.fm]) {
    for (const [pitch, tempo] of [
      ...
      const { engine, samples } = makeSignalEngine(module, type, pitch, tempo),
        { raw } = renderSignal(module, engine, 12_000, 127);
      assert.equal(raw.length, 12_000);
      const expectedEnd = Math.ceil((samples.length * 100) / tempo),
        end = raw.findLastIndex((sample) => Math.abs(sample) > 1e-6) + 1;
      assert.ok(
        Math.abs(end - expectedEnd) <= 2,
        `${type}: ${pitch}/${tempo} ends at ${end}, expected ${expectedEnd}`,
      );
      const frequency = fundamental(raw.subarray(512, Math.min(expectedEnd - 256, 3000)));
      assert.ok(
...
test("stretch output stays continuous across arbitrary synthesize chunk sizes and loop wraps", async (context) => {
  ...
  for (const type of [module.InstrumentType.chip, module.InstrumentType.fm]) {
    const render = (chunk) => {
        const { engine, settings, instrument } = makeSignalEngine(module, type, 137, 83, false);
        settings.offset = 0.125;
        settings.loopStart = 0.25;
        settings.loopEnd = 0.75;
        ...
        return renderSignal(module, engine, 12_000, chunk).output;
      },
      reference = render(12_000);
    assert.ok(
      reference.subarray(10_000).some((sample) => Math.abs(sample) > 0.01),
      "loop keeps sounding",
    );
...
test("seeking a running engine clears previous DSP and FM feedback state", async (context) => {
  ...
  for (const type of [module.InstrumentType.chip, module.InstrumentType.fm]) {
    const makeEngine = () => {
        ...
        instrument.unison = 2;
        if (type === module.InstrumentType.fm) {
          instrument.feedbackAmplitude = 7;
        }
        return engine;
      },
      engine = makeEngine(),
      reference = renderSignal(module, engine, 16_000, 127).output,
      fresh = makeEngine();
    fresh.playhead = 0.25;
    ...
    engine.playhead = 0;
    assertAudioClose(renderSignal(module, engine, 2000, 53).output, reference.subarray(0, 2000));
  }
});
...
test("sample catchup respects one-shot endings, frozen tempo, and reverse loops", async (context) => {
  ...
  for (const type of [module.InstrumentType.chip, module.InstrumentType.fm]) {
    for (const [tempo, oneshot, expectedPhase] of [
      ...
      if (oneshot) {
        assert.ok(
          resumed.every((sample) => sample === 0),
          "finished one-shots stay silent",
        );
...
test("returning to 100% pitch and tempo immediately bypasses existing DSP state", async (context) => {
  ...
  for (const type of [module.InstrumentType.chip, module.InstrumentType.fm]) {
    ...
    try {
      assert.ok(renderSignal(module, engine, 4000).output.every(Number.isFinite));
    } finally {
...
test("zero and reverse rates preserve source transport and produce finite chunk-independent audio", async (context) => {
  ...
  for (const type of [module.InstrumentType.chip, module.InstrumentType.fm]) {
    for (const [pitch, tempo] of [
      ...
      const render = (chunk) => {
        const { engine, settings } = makeSignalEngine(module, type, pitch, tempo, false);
        settings.offset = 0.75;
        const result = renderSignal(module, engine, 2000, chunk),
          tone = engine.channels[0].instruments[0].activeTones.get(0);
        if (tempo === 0) {
          ...
          assert.ok(tone.phases[0] !== 0.75, "zero pitch must not freeze the source timeline");
        }
...
test("all FM sample operators retain independent stretch state through bends and feedback", async (context) => {
  ...
  const render = (chunk) => {
      ...
      const note = song.channels[0].patterns[0].notes[0];
      note.pins = [module.makeNotePin(0, 0, 10), module.makeNotePin(12, note.end, 6)];
...
test("sample catchup uses tempo automation at the seek destination without replay", async (context) => {
  ...
  for (const type of [module.InstrumentType.chip, module.InstrumentType.fm]) {
    ...
    channel.patterns[0].automationEvents[0] = [
      new module.Event(0, 192, [new module.EventPoint(0, 121), new module.EventPoint(192, 243)]),
    ];
    engine.playhead = 0.25;
    const { output, raw } = renderSignal(module, engine, 1);
    ...
    const tone = engine.channels[0].instruments[0].activeTones.get(0),
      sourceFrames = (((2 * 60) / 151.5) * 8000 + 1) * 0.83;
    assert.ok(
      Math.abs(tone.phases[0] - (sourceFrames % 4096) / 4096) < 1e-9,
...
test("AudioWorklet sample playback matches SynthEngine offline output", async (context) => {
  ...
  context.after(() => module.Config.configureAssets([]));
  for (const type of [module.InstrumentType.chip, module.InstrumentType.fm]) {
    const { engine, song, samples } = makeSignalEngine(module, type, 137, 83, false),
    ...
    for (let offset = 0; offset < output.length; offset += 128) {
      assert.equal(
        processor.process([], [[output.subarray(offset, offset + 128), new Float32Array(128)]]),
        true,
      );
    }
    assertAudioClose(output, renderSignal(module, engine, output.length, 37).output);
...
test("extreme stretch ratios preserve pitch and chunk-independent audio", async (context) => {
  ...
  context.after(() => module.Config.configureAssets([]));
  const render = (chunk) => {
      const { engine } = makeSignalEngine(module, module.InstrumentType.chip, 100, 0.1, false);
      return renderSignal(module, engine, 4000, chunk).raw;
    },
    reference = render(4000);
