  ...
  await build({
    stdin: {
      contents: [
        ...
        'export {Config, parseAssetDefinition} from "./synth/synth-config.ts";',
      ].join("\n"),
...
test("asset edits preserve sample references by identity", async (context) => {
  const { ChangeAssets, Config, Song, parseAssetDefinition, cleanup } = await loadModules();
  context.after(cleanup);
  const first = parseAssetDefinition("https://example.com/first.wav"),
    second = parseAssetDefinition("https://example.com/second.wav");
  assert.notEqual(first, null);
  assert.notEqual(second, null);
  const song = new Song(),
    doc = { song, notifier: { changed() {} } };
  new ChangeAssets(doc, [first, second]);
  const instrument = song.channels[0].instruments[0];
  instrument.chipWave = Config.chipWaves.find((wave) => wave.sampleId === second.id).index;
  ...
  assert.equal(Config.chipWaves[instrument.operators[0].wave - 1].sampleId, first.id);
