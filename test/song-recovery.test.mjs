...
async function loadSongRecovery() {
  const directory = await mkdtemp(join(tmpdir(), "goopbox-song-recovery-test-")),
    outfile = join(directory, "module.mjs");
  await build({
    stdin: {
      contents: [
        'export {SongRecovery, versionToKey} from "./src/song-recovery.ts";',
        'export {encodeSongUrl, decodeSongUrlHash} from "./src/song-url.ts";',
        'export {ExportPrompt} from "./src/export-prompt.ts";',
...
function withImmediateTimers(callback) {
  const setTimeoutDescriptor = Object.getOwnPropertyDescriptor(globalThis, "setTimeout"),
    clearTimeoutDescriptor = Object.getOwnPropertyDescriptor(globalThis, "clearTimeout");
...
test("song recovery ignores corrupt metadata and handles storage quota without crashing", async (context) => {
  ...
  const songData = new Song().toBinary(),
    storedSongData = encodeSongUrl(songData);

  await context.test("malformed and prototype-named recovery keys are safe", () => {
    const storage = new MemoryStorage(),
      validVersion = { uid: "valid", time: 20, work: 1000 },
      prototypeVersion = { uid: "__proto__", time: 10, work: 500 };
    ...
    storage.seed('songBinaryVersion: {"uid":"missing-numbers"}', storedSongData);
    storage.seed('songBinaryVersion: {"uid":"bad-time","time":null,"work":2}', storedSongData);
    storage.seed('songVersion: {"uid":"legacy","time":30,"work":1000}', storedSongData);
  ...
  await context.test("quota retries discard the oldest versions and preserve the newest", () => {
    const storage = new RetryQuotaStorage(2),
      oldest = { uid: "oldest", time: 10, work: 1000 },
      middle = { uid: "middle", time: 20, work: 1000 },
      newest = { uid: "newest", time: 30, work: 1000 };
    ...
    withImmediateTimers(() => {
      new SongRecovery(() => historyLossNotifications++).saveVersion("current", songData);
    });

    assert.deepEqual(storage.removedKeys, [versionToKey(oldest), versionToKey(middle)]);
    ...
    const recoveredSongs = SongRecovery.getAllRecoveredSongs(),
      recoveredUids = new Set(recoveredSongs.map((song) => song.versions[0].uid));
    assert.ok(recoveredUids.has("newest"));
    assert.ok(recoveredUids.has("current"));
    assert.ok(!recoveredUids.has("oldest"));
    assert.ok(!recoveredUids.has("middle"));
    const currentVersion = recoveredSongs.find((song) => song.versions[0].uid === "current")
        .versions[0],
      currentPayload = storage.getItem(versionToKey(currentVersion));
  ...
  await context.test("forced goop export works when export preference storage throws", async () => {
    const exportGlobalNames = [
        "window",
        "document",
        "URL",
        "Node",
      ...
      originalExportGlobals = new Map(
        exportGlobalNames.map((name) => [name, Object.getOwnPropertyDescriptor(globalThis, name)]),
      );
    let savedBlob = null,
      savedName = null,
      anchorClicked = false,
    ...
    try {
      ...
      withImmediateTimers(() => {
        const prompt = new ExportPrompt({ song, closePrompt: () => (promptClosed = true) }, "goop");
      ...
      for (const [name, descriptor] of originalExportGlobals) {
        if (descriptor === undefined) {
          delete globalThis[name];
        } else {
          Object.defineProperty(globalThis, name, descriptor);
