...
async function loadPatternEditor() {
  ...
  const directory = await mkdtemp(join(tmpdir(), "goopbox-pattern-editor-test-")),
    outfile = join(directory, "module.mjs");
  await build({
    entryPoints: ["src/pattern-editor.ts"],
    bundle: true,
    platform: "node",
    format: "esm",
    outfile,
...
async function loadHitAnimation() {
  const directory = await mkdtemp(join(tmpdir(), "goopbox-hit-animation-test-")),
    outfile = join(directory, "module.mjs");
  await build({
    entryPoints: ["src/note-hit-animation.ts"],
    bundle: true,
    platform: "node",
    format: "esm",
    outfile,
...
test("note hit effects start immediately, expire after 250ms, repeat on loops, and clear on stop", async (context) => {
  ...
  const editor = Object.create(PatternEditor.prototype),
    ...
    ghostDrawing = Object.create(drawing),
    expandingDrawing = Object.create(drawing),
    canvasFor = (drawingContext) =>
    ...
    note = {
      ...
      pitches: [0],
      pins: [
        { time: 0, interval: 0, size: 3 },
        { time: 24, interval: 0, size: 3 },
      ],
    };
  Object.assign(editor, {
    ...
    _doc: {
      ...
      song: {
        ...
        pitchChannelCount: 1,
        noiseChannelCount: 0,
        channels: [{ muted: false }],
        getPattern: () => pattern,
        getChannelIsPitch: () => true,
        getChannelIsNoise: () => false,
  ...
  assert.equal(
    editor._hitCopies.length,
    1,
    "a backward playhead correction does not repeat an active note hit",
  );
  ...
  for (const lastRenderedBar of [-1, 1, 7]) {
    ...
    assert.equal(
      editor._hitCopies.length,
      0,
      "ties survive hidden views, redraws, and missed frames",
    );
  }
  ...
  assert.equal(
    editor._hitCopies.length,
    0,
    "loop ties use the audio predecessor even without a rendered loop end",
  );
  ...
  assert.equal(
    editor._noteContinuesFromPreviousBar(note, 0, 0),
    false,
    "different pitches break the tie",
  );
  pattern.notes[0].pins[0].interval = -1;
  assert.equal(
    editor._noteContinuesFromPreviousBar(note, 0, 0),
    true,
    "pitch bends ending on the next pitch preserve the tie",
  );
  ...
  assert.equal(
    editor._hitCopies.length,
    1,
    "starting playback on a tied note still hits without an audio predecessor",
  );
  ...
  editor._animateNoteHits(1_000_003);
  ...
  editor._animateNoteHits(1_000_004);
  ...
  editor._animateNoteHits(1_000_010);
  assert.deepEqual(calls.get(drawing).fills, [mainPath], "main sustain uses the main layer");
  assert.deepEqual(
    calls.get(ghostDrawing).fills,
    [ghostPath],
    "ghost sustain uses the ghost layer",
  );
  ...
  editor._doc.synth.playing = false;
  editor._animateNoteHits(1_000_011);
...
test("deleting any channel during playback tolerates cached hit notes until redraw", async (context) => {
  ...
  for (const deletedChannel of [0, 1, 2]) {
    const editor = Object.create(PatternEditor.prototype),
      ...
      cacheNotes = () =>
        channels.map((channel, index) => ({
          note: channel.pattern.notes[0],
          channel: index,
          color: "#88aaff",
          path: {},
        }));
    Object.assign(editor, {
      _hitCanvas: makeCanvas(),
      _ghostHitCanvas: makeCanvas(),
      _expandingCanvas: makeCanvas(),
      container: { isConnected: true },
      _svgPlayhead: { getAttribute: () => "" },
      _editorWidth: 100,
      _editorHeight: 100,
      _barOffset: 0,
      _hitNotes: cacheNotes(),
      _hitCopies: [],
      _lastHitPosition: null,
      _lastHitBar: -1,
      _doc: {
    ...
    assert.equal(
      editor._hitCopies.length,
      2,
      "remaining channels still animate after the cache refreshes",
    );
