...
async function loadSongUrlModules() {
  ...
  await build({
    stdin: {
      contents: [
        'export {decodeSongUrlHash, encodeSongUrl} from "./src/song-url.ts";',
        'export {extractCompressedSongBody} from "./synth/song-binary.ts";',
...
test("song URLs are only the unpadded URL-safe Base64 compressed body", async (context) => {
  const { Song, decodeSongUrlHash, encodeSongUrl, extractCompressedSongBody, cleanup } =
    await loadSongUrlModules();
  context.after(cleanup);
  const songData = new Song().toBinary(),
    compressedBody = extractCompressedSongBody(songData);
  ...
  assert.equal(encoded.startsWith("#s"), false, "song URLs must not have the old s/version prefix");
  assert.deepEqual(new Uint8Array(Buffer.from(encoded.slice(1), "base64url")), compressedBody);
...
test("song URLs reject old markers, non-base64url characters, and malformed payloads", async (context) => {
  const { Song, decodeSongUrlHash, encodeSongUrl, cleanup } = await loadSongUrlModules();
  context.after(cleanup);
  ...
  assert.equal(decodeSongUrlHash("#AA="), null, "base64 padding must not match a song URL");
  assert.throws(
  ...
  assert.equal(decodeSongUrlHash("#j1.e30"), null, "old JSON URLs must be ignored");
});
