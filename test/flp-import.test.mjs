...
function blobEvent(opcode, payload) {
  return Buffer.concat([Buffer.from([opcode]), leb128(payload.length), payload]);
}
...
function playlistRecord({
  ...
  if (recordSize >= 72) {
    clip.writeDoubleLE(1, 64);
  }
  return clip;
}

function flpContainer(eventData, { ppq = 96, declaredDataLength = eventData.length } = {}) {
...
function makeFlp({
  ...
  for (const channel of channels) {
    ...
    if (channel.name != null) {
      events.push(blobEvent(0xcb, utf16(channel.name)));
    }
  }
  ...
  for (const pattern of patterns) {
    ...
    if (pattern.name != null) {
      events.push(blobEvent(0xc1, utf16(pattern.name)));
    }
    if (pattern.length != null) {
      events.push(u32Event(0xa4, pattern.length));
    }
    events.push(blobEvent(pattern.noteOpcode ?? noteOpcode, Buffer.concat(pattern.notes)));
  }
  if (includeArrangement) {
    if (includeArrangementMarker) {
      events.push(u16Event(0x63, arrangementId));
    }
    for (const track of tracks) {
      events.push(blobEvent(versionMajor >= 25 ? 0xee : 0xde, Buffer.alloc(0)));
      if (track.name != null) {
        events.push(blobEvent(versionMajor >= 25 ? 0xef : 0xdf, utf16(track.name)));
      }
    }
    if (clips.length > 0) {
      events.push(
...
const loadedModule = (async () => {
  ...
  await build({
    stdin: {
      contents: [
        'export {parseFlpProject} from "./src/flp-parser.ts";',
        'export {convertFlpProject, getFruitySoundFontPresetIndex, importFlp, FlpImportError} from "./src/flp-import.ts";',
  ...
  const { importFlp, parseFlpProject } = await loadedModule,
    buffer = makeFlp({
      ...
      patterns: [
        {
          id: 3,
          notes: [noteRecord({ channelId: 7, key: 60, length: 96, velocity: 127 })],
        },
      ],
      clips: [{ patternId: 3, position: 0, length: 384, track: 4 }],
    }),
    project = parseFlpProject(buffer);
...
test("keeps a simultaneous chord in one GoopBox note", async () => {
  const { importFlp } = await loadedModule,
    notes = [60, 64, 67].map((key) => noteRecord({ key, length: 192, velocity: 100 })),
    song = importFlp(
      makeFlp({
        patterns: [{ id: 1, notes }],
    ...
    song = importFlp(
      makeFlp({
        ...
        patterns: [
          {
            id: 1,
            notes: [noteRecord({ channelId: 2, key: 60 }), noteRecord({ channelId: 9, key: 72 })],
          },
        ],
      }),
    );
...
test("applies a nonzero playlist pattern start offset", async () => {
  ...
  assert.deepEqual(
    absoluteNotes(convertFlpProject(project).pitchChannels[0]).map(({ start, end, pitches }) => ({
      start,
      end,
      pitches,
    })),
...
test("preserves raw lengths and velocities and maps them deterministically", async () => {
  ...
  assert.deepEqual(
    absoluteNotes(convertFlpProject(project).pitchChannels[0]).map(({ start, end, size }) => ({
      start,
      end,
      size,
    })),
...
test("splits sustained notes across bars with continuation metadata", async () => {
  ...
  assert.deepEqual(
    absoluteNotes(song.pitchChannels[0]).map(({ start, end, continuesLastPattern }) => ({
      start,
      end,
...
test("splits staggered overlaps into exactly two lanes without splitting a chord", async () => {
  ...
  assert.ok(
    allPitches.some(
      (pitches) => pitches.length === 2 && pitches.includes(48) && pitches.includes(52),
    ),
  );
  assert.ok(allPitches.some((pitches) => pitches.length === 1 && pitches[0] === 55));
});
...
test("skips unknown events without losing note data", async () => {
  const { importFlp } = await loadedModule,
    song = importFlp(
      makeFlp({
        extraEvents: [u8Event(0x3f, 0xa5), blobEvent(0xfe, Buffer.from([1, 2, 3, 4, 5]))],
      }),
    );
...
test("keeps event alignment across FL 25's fixed-three-byte 0xAC event", async () => {
  const { parseFlpProject, importFlp } = await loadedModule,
    buffer = makeFlp({ extraEvents: [fixedThreeByteEvent(0xac, 1, 1, 0)] }),
    project = parseFlpProject(buffer);
...
test("rejects malformed and physically truncated note events as controlled corruption", async () => {
  ...
  await assertFlpError(() => parseFlpProject(flpContainer(truncatedEvent)), "corrupt");
  ...
  const { importFlp, parseFlpProject } = await loadedModule,
    buffer = makeFlp({
      ...
      extraEvents: [
        ...
        blobEvent(0xd5, vstWrapperInfo({ name: "Serum", vendor: "Xfer Records" })),
      ],
    }),
    project = parseFlpProject(buffer);
...
test("resolves the chosen Fruity Soundfont Player bank and program to a preset index", async () => {
  const { getFruitySoundFontPresetIndex, importFlp, parseFlpProject } = await loadedModule,
    statePath = String.raw`%SystemDrive%\Program Files\Image-Line\FL Studio\Data\Patches\Soundfonts\EarthBound.sf2`,
    buffer = makeFlp({
      ...
      extraEvents: [
        ...
        blobEvent(
          ...
          vstWrapperInfo({
            ...
            state: Buffer.from("C:\\Libraries\\Shreddage X\\Shreddage X.nki\0", "ascii"),
          }),
        ),
      ],
    }),
...
test("keeps malformed native plugin state opaque after reading its instrument name", async () => {
  ...
  const song = importFlp(buffer);
  assert.equal(song.channelSources[0].plugin.internalName, "Unsupported Generator");
  assert.equal(absoluteNotes(song.pitchChannels[0]).length, 1);
});

test("recognizes FL's instrument-typed sampler placeholder", async () => {
  const { parseFlpProject } = await loadedModule,
    project = parseFlpProject(
      makeFlp({
        extraEvents: [
          u8Event(0x15, 4),
...
test("decodes slide, note-color, fine-pitch, pan, and modulation fields", async () => {
  const { parseFlpProject, importFlp } = await loadedModule,
    ...
    note = parseFlpProject(buffer).patterns[0].notes[1];
  assert.deepEqual(
    {
      flags: note.flags,
      slide: note.slide,
...
test("converts FL slide notes into persistent GoopBox pitch-bend pins", async () => {
  const { importFlp } = await loadedModule,
    ...
    note = song.pitchChannels[0].patterns[0].notes[0];
...
test("supports an FL 24 D0 note stream and 60-byte playlist clips", async () => {
  ...
  assert.deepEqual(
    absoluteNotes(importFlp(buffer).pitchChannels[0]).map(({ start, end, pitches }) => ({
      start,
      end,
      pitches,
    })),
...
test("supports an FL 20 E0 note stream and 32-byte playlist clips", async () => {
  ...
  assert.equal(project.arrangements[0].clips.length, 1);
  assert.deepEqual(absoluteNotes(importFlp(buffer).pitchChannels[0])[0].pitches, [57]);
});
...
test("imports a legacy playlist with no explicit arrangement marker", async () => {
  ...
  assert.equal(project.arrangements[0].clips.length, 1);
  assert.deepEqual(absoluteNotes(importFlp(buffer).pitchChannels[0])[0].pitches, [55]);
});
...
test("recognizes legacy negative-one float playlist offsets as unset", async () => {
  const { parseFlpProject, importFlp } = await loadedModule,
    buffer = makeFlp({
      ...
      clips: [{ patternId: 1, reversedTrack: 197, unsetOffset: 0xbf_80_00_00 }],
    }),
    clip = parseFlpProject(buffer).arrangements[0].clips[0];
  assert.equal(clip.startOffset, null);
  assert.equal(clip.endOffset, null);
  assert.deepEqual(absoluteNotes(importFlp(buffer).pitchChannels[0])[0].pitches, [59]);
});
...
test("reports invalid FLP magic and projects with no arrangement data", async () => {
  ...
  await assertFlpError(() => importFlp(makeFlp({ includeArrangement: false })), "empty");
