...
async function loadRewriteHelper() {
  const directory = await mkdtemp(join(tmpdir(), "goopbox-asset-source-paste-test-")),
    outfile = join(directory, "asset-source-paste.mjs");
  await build({
    entryPoints: ["src/asset-source-paste.ts"],
    bundle: true,
    platform: "node",
    format: "esm",
    outfile,
...
test("paste rewrite preserves all unrelated text", async (context) => {
  ...
  assert.equal(
    helpers.rewritePastedAssetSource("prefix https://example.com/file.sf2?dl=0&other=1 suffix"),
