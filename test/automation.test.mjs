  ...
  await build({
    stdin: {
      contents: [
        ...
        'export { Config, EffectType, FilterType, InstrumentType } from "./synth/synth-config.ts";',
        'export { decodeSongBinary, encodeSongBinary } from "./synth/song-binary.ts";',
        'export { decodeBinaryValue, encodeBinaryValue } from "./synth/binary-codec.ts";',
        'export { SongRenderer } from "./src/song-renderer.ts";',
        'export { bendEvent, clipEvent, deleteEventRange, editEventTime, eventPath, nearestEventPointIndex, repeatEvents } from "./src/event-editing.ts";',
        'export { AutomationRowSelectionState } from "./src/automation-selection.ts";',
        'export { trackChannelKindsAreCompatible } from "./src/channel-compatibility.ts";',
        'export { encodeSongUrl, decodeSongUrlHash } from "./src/song-url.ts";',
        'export { createMidiExportTracks } from "./src/midi-export.ts";',
        ...
        'export { KeyboardLayout } from "./src/keyboard-layout.ts";',
        'export { MidiInputHandler } from "./src/midi-input.ts";',
        'export { SongPerformance } from "./src/song-performance.ts";',
      ].join("\n"),
...
function makeChangeDocument(song, channel = 0) {
  return {
    ...
    barScrollPos: 0,
    channelScrollPos: 0,
    viewedInstrument: new Array(song.getChannelCount()).fill(0),
    notifier: { changed() {} },
    selection: {
      resetBoxSelection() {},
...
test("new songs default to one channel of each kind", async (context) => {
  ...
  assert.deepEqual(
    ...
    [module.ChannelKind.pitch, module.ChannelKind.noise, module.ChannelKind.automation],
  );
});
...
test("old songs default to zero Automation channels", async (context) => {
  ...
  oldSong.channels.length = oldSong.pitchChannelCount + oldSong.noiseChannelCount;
...
test("Automation rows, surfaces, floating point events, and invalid references round-trip", async (context) => {
  ...
  context.after(module.cleanup);
  const song = new module.Song(),
    channel = addAutomationChannel(module, song, 2);
  ...
  assert.deepEqual(
    restored.channels
      .at(-1)
      .patterns[0].automationEvents[0][0].points.map((point) => [point.time, point.value]),
    [
      [0, 3.125],
      [7.5, 1.75],
      [24, 2.125],
  ...
  const structured = new module.Song(module.encodeSongBinary(song.toBinaryObject()));
...
test("row count grows empty rows at the bottom and removes bottom rows", async (context) => {
  ...
  context.after(module.cleanup);
  const song = new module.Song(),
    channel = addAutomationChannel(module, song, 3);
...
test("malformed Automation payloads reject finite, ordering, and allocation violations", async (context) => {
  ...
  context.after(module.cleanup);
  const song = new module.Song(),
    channel = addAutomationChannel(module, song);
  ...
  const source = song.toBinaryObject(),
    ...
    automationChannel = (value) => value.channels.at(-1),
    firstEvent = (value) => automationChannel(value).patterns[0].rows[0][0];

  rejects((value) => {
    firstEvent(value).points[0].value = Number.NaN;
  });
  rejects((value) => {
    firstEvent(value).points[0].value = Number.POSITIVE_INFINITY;
  });
  rejects((value) => {
    firstEvent(value).points[1].time = 0;
  });
  rejects((value) => {
    firstEvent(value).end = song.beatsPerBar * module.Config.partsPerBeat + 1;
  });
  rejects((value) => {
...
test("Automation interpolation and latching reconstruct through bar history and pattern reuse", async (context) => {
  ...
  const song = new module.Song(),
    channel = addAutomationChannel(module, song),
    channelIndex = song.getChannelCount() - 1;
  channel.automationRows[0].setSongTarget("tempo");
  setPatternEvents(channel, 0, 0, [
    event(module, 4, 20, [
      [0, 100],
...
test("later events replace held values and continuous loop latches differ from manual seeks", async (context) => {
  ...
  song.tempo = 120;
  song.barCount = 2;
  for (const musical of song.channels) {
    musical.bars.length = 2;
  }
...
test("active events in lower rows override holdovers", async (context) => {
  ...
  for (const row of channel.automationRows) {
    row.setSongTarget("tempo");
  }
  channel.patterns[0].automationEvents[0] = [
    event(module, 0, 4, [
      [0, 200],
  ...
  channel.patterns[0].automationEvents[1] = [
    event(module, 8, 12, [
      [0, 90],
      [4, 90],
    ]),
...
test("zero-valued enabled effects become active, echo allocation is bounded, and Automation owns no tone state", async (context) => {
  ...
  for (const effectType of effectTypes) {
    instrument.effects |= 1 << effectType;
  }
  ...
  for (let rowIndex = 0; rowIndex < targetValues.length; rowIndex++) {
    ...
    automation.automationRows[rowIndex].setInstrumentTarget(song, 0, 0, targetId);
  ...
  const maximumSamplesPerTick =
      8000 /
      ((module.Config.tempoMin / 60) * module.Config.partsPerBeat * module.Config.ticksPerPart),
    requiredEchoSamples =
      module.Config.echoDelayRange * module.Config.echoDelayStepTicks * maximumSamplesPerTick;
...
test("keyboard, MIDI recording, and performance input ignore Automation channels", async (context) => {
  ...
  const channel = song.getChannelCount() - 1,
    calls = { add: 0, remove: 0, record: 0, live: 0 },
    doc = {
      song,
      channel,
      performance: {
  ...
  assert.equal(module.KeyboardLayout.keyPosToPitch(doc, 0, 0, "pianoAtC"), null);
...
test("instrument targets stay independent and share modulation metadata", async (context) => {
  ...
  context.after(module.cleanup);
  const song = new module.Song(),
    base = song.channels[0].instruments[0];
  base.pan = 3;
  base.volume = 1;
  const channel = addAutomationChannel(module, song, 2);
  channel.automationRows[0].setInstrumentTarget(song, 0, 0, "pan");
  ...
  assert.equal(
    base.supportsEnvelopeTarget(module.Config.modulationTargets.dictionary.mixVolume.index, 0),
    true,
  );
});
...
test("all required instrument target families bind to effective runtime values", async (context) => {
  ...
  const song = new module.Song(),
    ...
    generalIndex = 0,
    general = musical.instruments[generalIndex];
  general.effects =
    (1 << module.EffectType.eqFilter) |
  ...
  const pwmIndex = makeInstrument(module.InstrumentType.pwm),
    stringIndex = makeInstrument(module.InstrumentType.pickedString),
    fmIndex = makeInstrument(module.InstrumentType.fm),
    supersawIndex = makeInstrument(module.InstrumentType.supersaw),
    bindings = [
    ...
    automation = addAutomationChannel(module, song, bindings.length);
  automation.bars[0] = 1;
...
test("cross-target numeric mapping snaps integer targets and preserves continuous fractions", async (context) => {
  ...
  const song = new module.Song(),
    automation = addAutomationChannel(module, song, 1),
    row = automation.automationRows[0];
  ...
  const mixTarget = module.Config.modulationTargets.dictionary.mixVolume,
    mixDomain = module.Config.getAutomationValueDomain(mixTarget),
    expected = automation.patterns[0].automationEvents[0][0].points.map((point) =>
      module.mapAutomationValueBetweenDomains(point.value, panDomain, mixDomain),
    ),
    doc = makeChangeDocument(song, song.getChannelCount() - 1);
  new module.ChangeAutomationTargetElement(doc, 0, "mixVolume", 0);
  assert.deepEqual(
    automation.patterns[0].automationEvents[0][0].points.map((point) => point.value),
    expected,
...
test("Automation pattern content, Track compatibility, and MIDI skipping use channel kind", async (context) => {
  ...
  const song = new module.Song(),
    automation = addAutomationChannel(module, song);
  automation.patterns[0] = pattern;
  automation.bars[0] = 1;
  const automationIndex = song.getChannelCount() - 1,
    tracks = module.createMidiExportTracks(song);
  assert.equal(
...
test("Automation time selections clip, split, and value-bend events", async (context) => {
  const module = await loadAutomationModules();
  context.after(module.cleanup);
  const source = event(module, 0, 24, [
      [0, 0],
      [12, 6],
      [24, 0],
    ]),
    clipped = module.clipEvent(source, 6, 18);
  assert.deepEqual(
  ...
  assert.deepEqual(
    split.map((automationEvent) => [automationEvent.start, automationEvent.end]),
    [
      [0, 6],
      [18, 24],
    ],
...
test("notes and Automation share the Event model and range handlers", async (context) => {
  ...
  // oxlint-disable-next-line unicorn/prefer-structured-clone -- Verify JSON serialization.
  assert.deepEqual(Object.keys(JSON.parse(JSON.stringify(note))).sort(), [
...
test("shared event edits clip collisions, flatten values, repeat, and render", async (context) => {
  ...
  const events = [
    ...
    extended = module.editEventTime(events, 1, 1, 6, 1, 24, 32);
  assert.deepEqual(
  ...
  assert.deepEqual(
    module
      .editEventTime(resizable, 0, 1, -4, 1, 24, 32)
  ...
  const shaped = event(module, 0, 12, [
      [0, 1],
      [6, 4],
      [12, 2],
    ]),
    flattened = module.bendEvent(
      shaped,
      6,
      2,
      (value) => Math.max(0, Math.min(10, value)),
  ...
  assert.deepEqual(
    flattened.points.map((point) => point.value),
    [6, 6, 6],
  );
...
test("target filtering follows instrument type, enabled effects, and filter points", async (context) => {
  ...
  const song = new module.Song(),
    instrument = song.channels[0].instruments[0],
    names = () =>
      module.Config.getAutomationTargetsForInstrument(instrument).map(({ target }) => target.name);
...
test("channel and instrument reference remapping preserves identity and marks deletion missing", async (context) => {
  ...
  context.after(module.cleanup);
  const song = new module.Song(),
    automation = addAutomationChannel(module, song, 2),
    first = automation.automationRows[0];
  ...
  assert.equal(
    second.targetChannelMissing,
    true,
    "a replacement at the same index does not inherit the surface",
  );
...
test("real channel, instrument, effect, and filter edits preserve or invalidate references", async (context) => {
  ...
  context.after(module.cleanup);
  const song = new module.Song(),
    doc = makeChangeDocument(song);
  new module.ChangeChannelCount(doc, song.pitchChannelCount, song.noiseChannelCount, 1);
  const automationIndex = song.getChannelCount() - 1,
    rows = song.channels[automationIndex].automationRows;
  ...
  new module.ChangeFilterAddPoint(doc, targetInstrument.eqFilter, insertedPoint, 0, false);
  assert.equal(rows[3].targetIndex, 2);
  const deletedPoint = targetInstrument.eqFilter.controlPoints[2];
  new module.ChangeFilterAddPoint(doc, targetInstrument.eqFilter, deletedPoint, 2, false, true);
  ...
  new module.ChangeFilterAddPoint(doc, targetInstrument.eqFilter, replacementPoint, 2, false);
  assert.equal(rows[3].targetElementMissing, true);
});
...
test("removing a note filter point keeps its disabled envelope serializable", async (context) => {
  ...
  const song = new module.Song(),
    doc = makeChangeDocument(song),
    instrument = song.channels[0].instruments[0],
    target = module.Config.modulationTargets.dictionary.noteFilterAllFreqs;
  ...
  const point = instrument.noteFilter.controlPoints[0],
    deletion = new module.ChangeFilterAddPoint(doc, instrument.noteFilter, point, 0, true, true);
...
test("Automation events survive beat stretching, splicing, overflow, and wrap movement", async (context) => {
  ...
  const stretchedSong = new module.Song(),
    stretchedChannel = addAutomationChannel(module, stretchedSong);
  ...
  const stretchedDoc = makeChangeDocument(stretchedSong, stretchedSong.getChannelCount() - 1);
  new module.ChangeBeatsPerBar(stretchedDoc, 2, "stretch");
  const stretched = stretchedChannel.patterns[0].automationEvents[0][0];
  assert.deepEqual([stretched.start, stretched.end, stretched.points[1].time], [4, 36, 32]);
  new module.ChangeBeatsPerBar(stretchedDoc, 1, "splice");
  assert.equal(stretchedChannel.patterns[0].automationEvents[0][0].end, module.Config.partsPerBeat);

  const wrappedSong = new module.Song(),
    wrappedChannel = addAutomationChannel(module, wrappedSong),
    partsPerBar = wrappedSong.beatsPerBar * module.Config.partsPerBeat;
  setPatternEvents(wrappedChannel, 0, 0, [
    event(module, partsPerBar - 8, partsPerBar, [
      [0, 1.25],
      [8, 2.75],
    ]),
  ]);
  const wrappedDoc = makeChangeDocument(wrappedSong, wrappedSong.getChannelCount() - 1);
  ...
  for (const channel of overflowSong.channels) {
    channel.bars.length = 2;
  }
  const overflowChannel = addAutomationChannel(module, overflowSong),
    overflowRowCount = overflowChannel.automationRows.length;
  overflowChannel.bars.length = 2;
  setPatternEvents(overflowChannel, 0, 0, [
    event(module, partsPerBar - 4, partsPerBar, [
      [0, 3.5],
      [4, 4.5],
    ]),
  ]);
  const overflowDoc = makeChangeDocument(overflowSong, overflowSong.getChannelCount() - 1);
  ...
  assert.ok(
    movedAutomation.patterns.some((pattern) =>
      pattern.automationEvents[0]?.some(
        (automationEvent) => automationEvent.start === module.Config.partsPerBeat - 4,
      ),
    ),
  );
...
test("growable rendering respects automation, clamping, and the non-automated duration", async (context) => {
  ...
  assert.equal(baselineLength, 16_000);
